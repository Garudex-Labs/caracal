...
//
// Unit tests for persistent zone-scoped Operator memory: recall, rendering, and governed deduplicated writes.
...
import {
  ...
  type ZoneMemoryEntry,
} from '../../../../apps/api/src/operator-zone-memory.js'

describe('recallZoneMemory', () => {
  it('reads the zone its memory newest first under a bounded limit', async () => {
    const rows: ZoneMemoryEntry[] = [{ text: 'Register the Son of Anton application', created_at: '2026-06-02T00:00:00Z' }]
    const query = vi.fn().mockResolvedValue({ rows })
    const entries = await recallZoneMemory({ query }, 'z1', 5)
    ...
    expect(params).toEqual(['z1', 5])
  })

  it('never reads fewer than one entry even for a non-positive limit', async () => {
    const query = vi.fn().mockResolvedValue({ rows: [] })
    await recallZoneMemory({ query }, 'z1', 0)
    expect(query.mock.calls[0][1]).toEqual(['z1', 1])
  })
})

describe('describeZoneMemory', () => {
  it('returns an empty string when a zone has no memory yet', () => {
    expect(describeZoneMemory(undefined)).toBe('')
...
describe('rememberAppliedChange', () => {
  it('writes a bounded, deduplicated memory of an applied change', async () => {
    const query = vi.fn().mockResolvedValue(undefined)
    await rememberAppliedChange({ query }, 'z1', 'conv-1', 'Register the Son of Anton application')
    ...
    expect(params[3]).toBe('Register the Son of Anton application')
  })
