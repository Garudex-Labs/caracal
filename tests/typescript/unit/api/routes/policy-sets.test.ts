...
describe('POST /v1/zones/:zoneId/policy-sets/:id/activate', () => {
  ...
  it('activates valid version, deactivates sibling sets, enqueues outbox row in TX, returns 202', async () => {
    ...
    expect(sqls.some((sql) => sql.includes('FOR UPDATE OF ps') && sql.includes('ps.archived_at IS NULL'))).toBe(true)
    expect(sqls.some((sql) => sql.includes('FOR UPDATE OF ps, psv'))).toBe(false)
