...
//
// Provider-connection route unit tests for stored connections, the OAuth browser flow, and revocation.
...
import { providerConnectionsRoutes } from '../../../../../apps/api/src/routes/provider-connections.js'
...
describe('GET /v1/zones/:zoneId/provider-connections', () => {
  it('lists stored provider connections with filters against provider_connections', async () => {
    const { app, db } = buildRouteApp(providerConnectionsRoutes)
...
describe('POST /v1/zones/:zoneId/provider-connections', () => {
  it('rejects invalid provider grant payloads and missing zones', async () => {
    const invalid = buildRouteApp(providerConnectionsRoutes)
    ...
    const missingZone = buildRouteApp(providerConnectionsRoutes)
  ...
  it('rejects unsupported provider kinds', async () => {
    const unsupported = buildRouteApp(providerConnectionsRoutes)
...
describe('OAuth provider grant browser flow', () => {
  ...
  it('handles expired, denied, and malformed callback state without provider calls', async () => {
    const expired = buildRouteApp(providerConnectionsRoutes)
    ...
    const denied = buildRouteApp(providerConnectionsRoutes)
