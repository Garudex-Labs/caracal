...
describe("delegationErrorMessage", () => {
  it("translates known coordinator error codes", () => {
    expect(delegationErrorMessage(new ConsoleApiError(503, "coordinator_not_configured"))).toBe(
      "Coordinator service not connected.",
    );
    expect(delegationErrorMessage(new ConsoleApiError(502, "upstream_unreachable"))).toBe(
      "Coordinator service unreachable.",
    );
  });

  it("humanizes an unknown ConsoleApiError code", () => {
    expect(delegationErrorMessage(new ConsoleApiError(400, "scope_not_granted"))).toBe(
      "scope not granted",
    );
  });
