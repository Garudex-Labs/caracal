...
//
// OAuthClient unit tests for exchange, cache isolation, cancellation, and approval holds.
...
describe('OAuthClient', () => {
  ...
  it('returns the granted target resource subset', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({
    ...
    expect(res.targetResources).toEqual(['resource://openai'])
  })
  ...
  it('supports application-principal exchanges with multiple resources', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock)
    const client = new OAuthClient('http://sts:8080', 'zone1', 'app1')
    const res = await client.exchange('', ['resource://b', 'resource://a', 'resource://b'], { clientSecret: 'secret-1' })
  ...
  it('sends Authority record, Session, and Delegation fields without actor credentials', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
  ...
  it('does not retry transient STS responses', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: false,
      status: 429,
  ...
  it('rethrows final fetch errors and times out expired deadlines', async () => {
    ...
    await expect(client.exchange('subject-tok', 'resource://api')).rejects.toThrow('network down')
