...
test.describe("Skill CRUD", () => {
  ...
  test("submit → get → approve → archive", async () => {
    const token = await getAccessToken();
    const headers = {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
    ...
    // Archive (components retire by archival, never hard delete)
    const archiveRes = await fetch(`${API_BASE}/api/v1/skills/${skillId}/archive`, {
      method: "PATCH",
      headers,
    });
...
test.describe("Hook CRUD", () => {
  ...
  test("submit → get → approve → archive", async () => {
    ...
    // Archive (components retire by archival, never hard delete)
    const archiveRes = await fetch(`${API_BASE}/api/v1/hooks/${hookId}/archive`, {
      method: "PATCH",
      headers,
    });
    expect(archiveRes.status).toBe(200);
...
test.describe("Sandbox CRUD", () => {
  ...
  test("submit → get → approve → archive", async () => {
    ...
    // Archive (components retire by archival, never hard delete)
    const archiveRes = await fetch(`${API_BASE}/api/v1/sandboxes/${sandboxId}/archive`, {
      method: "PATCH",
      headers,
    });
    expect(archiveRes.status).toBe(200);
