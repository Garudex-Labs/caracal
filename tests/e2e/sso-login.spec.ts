 ...
 *
 * Prerequisites:
 *   - API running on localhost:8080 with OAUTH_CLIENT_ID, OAUTH_CLIENT_SECRET,
 *     OAUTH_SERVER_METADATA_URL configured for a Microsoft Entra ID tenant
 *   - Frontend running on localhost:8000
 *   - FRONTEND_URL=http://localhost:8000 in the API environment
...
const SSO_EMAIL =
  process.env.SSO_TEST_EMAIL ?? "ryan@tx131.onmicrosoft.com";
const SSO_PASSWORD = process.env.SSO_TEST_PASSWORD ?? "TestCaracal@2026!";
...
test.describe("SSO Login Flow", () => {
  ...
  test("full SSO login flow authenticates user and redirects to home", async ({
    ...
    await page.waitForURL(
      (url) => url.origin === "http://localhost:8000" && url.pathname !== "/login",
      { timeout: 30_000 },
    );

    // Verify authentication state
    const role = await page.evaluate(() =>
      localStorage.getItem("caracal_user_role"),
    );
    expect(role).toBeTruthy();

    const apiKey = await page.evaluate(() =>
      localStorage.getItem("caracal_api_key"),
    );
