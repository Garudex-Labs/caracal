...
caracal self downgrade --version 0.7.0 --force
echo ""
...
if caracal self status 2>&1 | grep -q "No such command"; then
...
curl -s -I $API/api/v1/config/version \
    -H "X-Caracal-CLI-Version: 0.8.0" 2>&1 | grep -i "x-caracal"
echo ""

echo "CLI v0.5.0 → Server (degrades to min):"
curl -s -I $API/api/v1/config/version \
    -H "X-Caracal-CLI-Version: 0.5.0" 2>&1 | grep -i "x-caracal"
echo ""

echo "CLI v0.3.0 → Server (below min, still works):"
curl -s -I $API/api/v1/config/version \
    -H "X-Caracal-CLI-Version: 0.3.0" 2>&1 | grep -i "x-caracal"
echo ""

echo "No CLI header → defaults to server version:"
curl -s -I $API/api/v1/config/version 2>&1 | grep -i "x-caracal"
...
caracal server versions 2>&1 || echo "(server command may need compose dir)"
echo ""

echo "Server upgrade --dry-run:"
caracal server upgrade --dry-run 2>&1 || echo "(expected if already on latest)"
echo ""

echo "Server upgrade bad version (should fail fast):"
caracal server upgrade --version 99.0.0 --force 2>&1 || true
...
if [[ "$HTTP_CODE" == "200" ]]; then
    pass "Frontend serving on :8000 (HTTP $HTTP_CODE)"
else
...
OUTPUT=$(uv tool install --force --editable . >/dev/null 2>&1 && caracal ops list 2>&1 || true)
echo "$OUTPUT" | tail -3
if echo "$OUTPUT" | grep -q "Update available\|update available\|self upgrade"; then
    pass "Update notification banner shown"
else
    echo -e "${YELLOW}⚠ Banner not shown (cache may be fresh, check ~/.caracal/version_cache.json)${NC}"
fi
...
OUTPUT=$(caracal auth whoami 2>&1)
...
caracal config set update_check true
OUTPUT=$(CARACAL_NO_UPDATE_CHECK=1 caracal auth whoami 2>&1)
if echo "$OUTPUT" | grep -q "Update available"; then
    echo "FAIL: Banner shown despite env var"
else
    pass "No banner with CARACAL_NO_UPDATE_CHECK=1"
fi
...
mkdir -p ~/.caracal
echo '{"pid": '$$', "timestamp": '$(date +%s)'}' >~/.caracal/.cli-upgrade.lock

OUTPUT=$(caracal self downgrade --version 0.6.0 --force 2>&1 || true)
