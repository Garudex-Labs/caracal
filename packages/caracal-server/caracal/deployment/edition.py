...
Resolves the active execution model from persisted deployment signals.
"""
...
class EditionManager:
    ...
    1. CCL_GATEWAY_ENABLED runtime mode signal (if provided)
    ...
    def get_edition(self) -> Edition:
        ...
        Returns current edition (OPENSOURCE or ENTERPRISE).
          ...
          1. CCL_GATEWAY_ENABLED runtime signal (if provided)
    ...
    def _auto_detect_edition(self) -> Edition:
        ...
        gateway_enabled_raw = (os.environ.get("CCL_GATEWAY_ENABLED") or "").strip().lower()
    ...
    def _assert_execution_exclusivity(self, detected_edition: Edition) -> None:
        ...
        if detected_edition == Edition.ENTERPRISE and not gateway_url:
            raise EditionConfigurationError(
                "Enterprise execution requires a configured gateway URL. "
    ...
    def get_gateway_url(self) -> Optional[str]:
        ...
        value = (os.environ.get("CCL_GATEWAY_URL") or "").strip()
        if value:
            return value
        return None
