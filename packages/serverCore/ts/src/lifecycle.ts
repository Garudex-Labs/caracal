...
export class ShutdownRegistry {
  ...
  install(signals: NodeJS.Signals[] = ['SIGTERM', 'SIGINT']): void {
    ...
    for (const sig of signals) {
      process.once(sig, () => {
        void this.fire(sig, 0)
      })
    }
  }

  async fire(reason: string, code = 0): Promise<void> {
    if (this.firing) return
    this.firing = true
    this.opts.log('info', 'shutdown initiated', { reason, entries: this.entries.length })
    const exit = this.opts.exit ?? ((exitCode) => process.exit(exitCode))
    ...
    for (const entry of [...this.entries].reverse()) {
      ...
      try {
        await withTimeout(Promise.resolve(entry.fn()), remaining, `shutdown step ${entry.name} did not complete within ${remaining}ms`)
