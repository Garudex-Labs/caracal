...
class CaracalEvent:
    ...
    (``approval.wait``), or a coordinator call (``coordinator.call``). Every
    event carries the outcome and duration; ``status`` holds the HTTP status
    when a response arrived and ``code`` the platform error code when the
    operation failed with one. Bridge events to any metrics or tracing system;
    a hook that raises is ignored and never disturbs the operation that
    emitted the event."""
