...
import { publishTagPattern } from './lib/releaseSpec.mjs'

export function validateGitHubRelease(release, tag, channel) {
  if (!publishTagPattern.test(tag ?? '')) throw new Error(`invalid release tag: ${tag}`)
...
export async function fetchPublicGitHubRelease(url, tag, channel, fetcher = fetch) {
  ...
  for (let attempt = 1; ; attempt += 1) {
    ...
    if (response.ok) return validateGitHubRelease(await response.json(), tag, channel)
    if (response.status < 500 || attempt >= attempts) throw new Error(`public GitHub Release ${tag} returned HTTP ${response.status}`)
    await delay(5_000)
  }
}
