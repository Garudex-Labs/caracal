...
Configure a LiteLLM-compatible provider so Caracal can generate trace summaries, detect anomalies, and explain agent behavior in natural language.

## Get Insights running in 5 minutes

1. Get an API key from any provider supported by LiteLLM.
2. Go to **Operator > AI Engine** in the Caracal operator console.
3. Pick the provider. Caracal loads model options from the LiteLLM catalog.
