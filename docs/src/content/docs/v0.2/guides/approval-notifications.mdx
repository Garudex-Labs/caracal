...
description: Push approval lifecycle events to your own systems with signed,
  retried webhook sinks, and verify each delivery before acting on it.
slug: v0.2/guides/approval-notifications
---

Approval holds are decided fastest when the right person hears about them immediately. A notification sink is a zone-scoped webhook endpoint that Caracal pushes approval lifecycle events to: your relay turns the delivery into a page, a chat message, or a ticket in whatever system your team watches. Sinks are optional - a zone without one loses nothing except the push, since the web console badges pending holds and the [Approvals page](/v0.2/guides/step-up/#decide-as-an-operator) remains the decision surface.
...
Related pages: [Human Approval](/v0.2/guides/step-up/) and [Tail and Query the Audit Stream](/v0.2/guides/audit-stream/).
...
Create alerts for abandoned deliveries and test the operator decision path in [Human Approval](/v0.2/guides/step-up/).
