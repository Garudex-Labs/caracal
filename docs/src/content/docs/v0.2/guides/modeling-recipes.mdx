...
description: Map zones, applications, resources, providers, and scopes onto real
  deployments before creating production objects.
slug: v0.2/guides/modeling-recipes
---

Use this guide before creating production objects or when an existing zone has become hard to reason about. It turns deployment boundaries into a reviewed model; the [concept pages](/v0.2/concepts/) remain the canonical definitions.
...
Every recipe states the modeling decision, what maps to each noun, and the trade-off. Use the [Caracal Mental Model](/v0.2/concepts/model-overview/) as the vocabulary reference while you read. Three terms appear here before their own guides: a **grant** is policy data mapping application roles to resource scopes ([Resources and Grants](/v0.2/concepts/resource-grant/)); the **platform decision contract** is the fixed decision logic your policy data feeds ([Author Policy Data](/v0.2/guides/author-policy/)); a **DCR application** is a short-lived programmatically registered identity ([managed and DCR applications](/v0.2/concepts/principal/#managed-and-dcr-applications)).
...
Keep [resource identifiers](/v0.2/concepts/resource-grant/) stable across zones so policy, grants, and audit refer to the same target even when upstream URLs differ.
...
| Application | One managed application per durable service; one DCR application (a short-lived identity created through [Dynamic Client Registration](/v0.2/sdks/admin/#dynamic-client-registration-dcr)) per isolated, externally-launched identity (per tenant, job, or integration). |
...
Decide on the strongest isolation a customer actually requires, then pick the least complex model that satisfies it. Do not encode customer identity into [scope names](/v0.2/concepts/resource-grant/); keep it in the zone, principal, or policy input. For the end-to-end pattern of serving many customers from one shared zone, see [Serve Your Own Customers](/v0.2/guides/serve-customers/).
...
| Approval       | Hold the sensitive action for a human decision through [Human Approval](/v0.2/guides/step-up/).                    |
...
Policy tells work apart with the principal registration method, Session ID, and labels documented in the [policy input contract](/v0.2/concepts/policy/#policy-input-contract). Authorization inputs are the application, labels, roles, Delegation, and confinement - never the Subject identifier. See [Identities and Applications](/v0.2/concepts/principal/).
...
For connected accounts, Caracal owns `client_id`, `redirect_uri`, `state`, and PKCE; use the provider **Connect** action to mint a consent URL for a subject, and revoke to disconnect it. Reconnecting the same subject and provider replaces the active connection rather than duplicating it. The concrete field tables are in [Provider Recipes](/v0.2/guides/provider-recipes/).
...
* Run [Check Provider Readiness](/v0.2/examples/provider-preflight/) to verify resource-to-provider binding and that the active policy set returns `allow`.
...
Provision the reviewed model with [Define Resources and Providers](/v0.2/guides/resources-providers/), then author matching grant data with [Author Policy Data](/v0.2/guides/author-policy/).
...
* [Provider Recipes](/v0.2/guides/provider-recipes/)
* [Production Integration Patterns](/v0.2/guides/production-patterns/)
