...
description: Start child Sessions, attach typed constraints, inspect graph
  impact, and revoke safely.
slug: v0.2/guides/delegation
---
...
Python uses `Authority.narrow(...)`, `await caracal.delegate(...)`, and `async with caracal.accept_delegation(edge_id, validate=True)`. Go uses `AuthorityNarrow(...)`, `Delegate`, and `AcceptDelegation`. The [language SDK references](/v0.2/sdks/) define exact option types; the workflow and wire authority are equivalent.

## Narrow a child in the same application

Starting a child with less authority than its parent needs no explicit hand-off - pass `Authority.narrow(...)` when starting the child Session, exactly as shown in your [SDK guide](/v0.2/guides/sdk-typescript/#start-a-narrowed-child). This page's subject is the explicit flow between two Sessions that already exist.
...
Inspect the graph and request trace in [Tail and Query the Audit Stream](/v0.2/guides/audit-stream/).
