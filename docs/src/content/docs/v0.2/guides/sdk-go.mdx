...
description: Install the Go SDK, load a runtime profile, run governed sessions
  with context.Context, delegate authority, and inject Caracal headers.
slug: v0.2/guides/sdk-go
---

Use the Go SDK in an application that must carry governed authority in `context.Context`, create Sessions, narrow Delegation, or route HTTP through Gateway. A resource server that only verifies inbound mandates should use [the net/http adapter](/v0.2/guides/protect-nethttp/).
...
A rejected, expired, or already-consumed decision returns the original `*oauth.ApprovalRequiredError`; its `ApprovalID` lets you resume waiting later with `client.WaitForApproval(ctx, approvalID, timeout)`, which returns the typed final state. [Human Approval](/v0.2/guides/step-up/) covers the tiers and decision paths.
...
For exact exported types and options, use [Go SDK reference](/v0.2/sdks/go/).

## Next Step

Protect inbound handlers with [the net/http adapter](/v0.2/guides/protect-nethttp/) or implement [multi-agent delegation](/v0.2/guides/delegation/).
