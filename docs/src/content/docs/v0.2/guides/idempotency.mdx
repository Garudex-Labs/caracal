...
description: Understand what Caracal retries automatically, when to supply a
  stable operation identifier, and why external side effects still need
  destination-side protection.
slug: v0.2/guides/idempotency
---
...
:::caution[Failure point: Gateway calls]
Caracal's Coordinator receipt does not deduplicate an upstream HTTP mutation. Forward the stable operation ID to the destination's `Idempotency-Key` or use a transactional inbox/outbox.
:::

## Next Step

Encode replay, conflict, and destination-dedup cases in [Test Caracal Integrations](/v0.2/guides/testing/).
