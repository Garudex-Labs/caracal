...
This is the supported HTTP surface for trusted automation. Prefer [Admin Package](/v0.2/sdks/admin/) for typed pagination and errors. Never expose an admin token to an agent, browser, or protected upstream.
...
| Subject issuers      | `GET /v1/zones/:zoneId/subject-issuers`, `GET /v1/zones/:zoneId/subject-issuers/:id`                                                                                                                 | `POST /v1/zones/:zoneId/subject-issuers`, `PATCH /v1/zones/:zoneId/subject-issuers/:id`, `DELETE /v1/zones/:zoneId/subject-issuers/:id`                                                                                                   |
...
| Authority records | `GET /v1/zones/:zoneId/authority-records`. Filter by `authority_record_id`, `subject_id`, or `status`; use `format=csv` to export.                                                                                                                                                                                |
...
| Sessions          | `GET /v1/zones/:zoneId/sessions`. Filter by `status`, `lifecycle`, `label`, `parent_session_id`, or `application_id`; use `format=csv` to export.                                                                                                                                                                 |
| Approvals         | `GET /v1/zones/:zoneId/step-up-challenges`, `GET /v1/zones/:zoneId/step-up-challenges/:id`, `POST /v1/zones/:zoneId/step-up-challenges/:id/approve`, `POST /v1/zones/:zoneId/step-up-challenges/:id/reject`; `step-up-challenges` is the protocol path.                                                            |
...
* For scoped, non-interactive provisioning, drive the Control API with a control key. See the [supported Control management model](/v0.2/services/control/#supported-management-model) and [Bootstrap Control State](/v0.2/examples/control-bootstrap/).
* Prefer declarative reconciliation over scripting individual writes. Control's [`state plan|verify|apply` workflows](/v0.2/services/control/#supported-management-model) converge a Zone from desired state with dry-run and CI-friendly verification.
...
Continue to [Use Coordinator API](/v0.2/api/coordinator/) when automation needs Session, invocation, or Delegation endpoints.

## Related Pages

* [Admin Package](/v0.2/sdks/admin/)
* [Manage Product Objects](/v0.2/runtime-console/admin/)
* [Manage Product State](/v0.2/services/api/)
