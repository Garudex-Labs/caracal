...
description: Give an AI agent short-lived, policy-approved authority to call an
  LLM provider - without the agent ever holding the provider API key.
pageType: workflow
slug: v0.2/get-started/first-protected-call
---
...
* Complete [Install Caracal](/v0.2/get-started/install-caracal/), including the sign-in setup at the end of that page.
...
Open the web console at [http://localhost:3001](http://localhost:3001) and sign in with the method you configured during [installation](/v0.2/get-started/install-caracal/#enable-console-sign-in). A first sign-in walks through a short onboarding flow:
...
* The agent presents a Caracal mandate to the Gateway, never a provider key. If your agent's code asks for `OPENAI_API_KEY`, point it at the governed transport instead - the [next page](/v0.2/get-started/add-sdk-to-your-app/) and [Provider Recipes](/v0.2/guides/provider-recipes/) show how existing provider clients adopt it.
...
Your agent made one protected call from a throwaway script. Next, wire the same call into a real application with a configuration profile: [Add SDK to Your App](/v0.2/get-started/add-sdk-to-your-app/).
