...
description: Install the caracal CLI, verify Docker, and start the local stack
  that the rest of Get Started uses.
pageType: workflow
slug: v0.2/get-started/install-caracal
---
...
<Tabs syncKey="os">
  <TabItem label="Linux / macOS">
    ```sh
    curl -fsSL https://raw.githubusercontent.com/Garudex-Labs/caracal/main/install.sh | sh -s -- --version v0.2.0
    ```
  </TabItem>
...
The change applies immediately; no restart is needed. The same command family later manages suspension, restoration, and removal - see [Control Console Access](/v0.2/runtime-console/console-access/) for the full lifecycle.

An allowlisted email still needs a way to sign in. Pick one and add its settings to the operator env file the stack reads - `$CARACAL_HOME/caracal.env`, whose exact per-platform path and editor command are in [Configure Service Environment](/v0.2/operations/env-vars/#the-operator-env-file) - then rerun `caracal up`:
...
Continue with [First Protected Call](/v0.2/get-started/first-protected-call/).
