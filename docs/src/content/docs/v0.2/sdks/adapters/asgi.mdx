---
title: ASGI Adapter
description: Pure ASGI middleware that verifies Caracal mandates for FastAPI,
  Starlette, and Quart resource servers.
slug: v0.2/sdks/adapters/asgi
---

The ASGI adapter protects any Python ASGI application - FastAPI, Starlette, Quart, Django ASGI - with fail-closed Caracal mandate verification. It is pure ASGI: it imports no web framework and delegates every check to [Verify Package](/v0.2/sdks/verify/).
...
| `revocations`                                            | Revocation store consulted on every request. Use [Redis Revocation Store](/v0.2/sdks/backends/redis/) in production. |
...
The adapter verifies inbound mandates; it does not create Sessions or Delegations. Use the [Python SDK](/v0.2/sdks/python/) to create Caracal context before making outbound calls.
