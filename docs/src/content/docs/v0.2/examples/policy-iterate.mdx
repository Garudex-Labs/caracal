...
description: Diagnose a denied request, simulate a candidate policy-set version,
  regression-check expected decisions, and activate only when every gate passes.
pageType: workflow
slug: v0.2/examples/policy-iterate
---
...
Use it when a real request was denied and the fix must be proven safe before rollout: the candidate has to repair that denial and keep every regression case's decision. Do not use it as a substitute for the [policy authoring contract](/v0.2/guides/author-policy/) or [activation workflow](/v0.2/guides/activate-policy-set/); it automates their verification loop.
...
Continue to [Launch Research Agent](/v0.2/examples/research-agent/) to see runtime credential injection for a plain CLI agent.
