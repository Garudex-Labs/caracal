...
sidebar:
  order: 12
slug: v0.2/concepts/step-up
---

Read this page after [Policies and Policy Sets](/v0.2/concepts/policy/). An Approval pauses a sensitive authority request until an eligible human decides it. Policy data maps scopes to risk tiers and identifies whether an operator, the federated Subject, or either may decide.

Approval is an optional security primitive. A zone that declares no `approval_tiers` data never creates a hold, and nothing in the platform requires one. One naming rule carries through every surface: the SDKs call the decision's identifier `approvalId`, while the STS and Admin API call the same value `challenge_id` under `step-up` paths - see the [wire-name mapping](/v0.2/reference/interoperability-contracts/#product-to-wire-mapping).
...
Read [Session Delegation](/v0.2/concepts/delegation/) to understand how approved authority can be narrowed for another Session.
