---
title: Mandates
description: The short-lived, signed JWT that carries approved session or
  resource authority.
sidebar:
  order: 9
slug: v0.2/concepts/mandate
---

Read this page after [Policies and Policy Sets](/v0.2/concepts/policy/). A Mandate is the short-lived signed proof Caracal issues after policy allows a request. The Gateway or a verified service checks it before the action runs.
...
Resource servers should always verify a mandate at request time. Verification includes signature and claim checks plus revocation checks for the Authority record ID, Root authority record ID, Session ID, and Delegation ID when those claims are present. See the [parsed claim mapping](/v0.2/sdks/identity/#parsed-claim-names) for language-level and raw JWT names.
...
Read [Approvals](/v0.2/concepts/step-up/) to understand how sensitive requests wait for a human decision before a Mandate is issued.

## Related Pages

* [Sessions and Revocation](/v0.2/concepts/sessions-revocation/)
* [Protect an MCP Server](/v0.2/guides/protect-mcp/)
* [Run an Agent with caracal run](/v0.2/guides/runtime-run/)
