---
title: Providers
description: A provider defines the credential Caracal attaches to an upstream
  target after access is approved.
sidebar:
  order: 7
slug: v0.2/concepts/provider
---

A credential Provider answers one question: after Caracal approves a call, what credential does the upstream target receive? Read [Resources and Grants](/v0.2/concepts/resource-grant/) first. Every Gateway-routed Resource binds one Provider; one Provider can serve many Resources.
...
Read [Policies and Policy Sets](/v0.2/concepts/policy/) to understand how requests against a Resource are allowed, denied, or held for Approval.
