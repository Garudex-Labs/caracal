...
4. On the same page, bind an environment variable to each resource the workload needs and select the scopes each policy decision should evaluate. The page then shows the exact launch commands. See [Configure Workloads](/runtime-console/config-file/).
...
Run [Launch Research Agent](/examples/research-agent/) for the fixed example, or migrate a long-lived process to the matching SDK guide ([TypeScript](/guides/sdk-typescript/), [Python](/guides/sdk-python/), [Go](/guides/sdk-go/)).
