...
The `interaction_required` error carries the approval id, the tier, the expiry, and a `binding` - a hash of the exact resource and scope set held. The SDK runs the whole flow - catch the hold, wait for the decision, retry with the approval id so the retried mint consumes it - in one call:
...
const mandate = await caracal.withApproval((approvalId) =>
  caracal.mintMandate("resource://pipernet", ["pipernet:refund"], { approvalId }),
);
```
...
An approval is **single-use**: the first mint that presents the approval id spends it. When two workers race one approval - a retried queue message, a redeployed worker resuming the same persisted id - one wins and the other fails with the hold in state `consumed`. Consumed is not an error to wait out: the operation was already performed once, so check for its effect before re-requesting a fresh approval.
