...
description: Declare approval tiers in policy data, hold the exchange, and decide it from the Console, the Admin API, or your application's own end user.
---
...
| `approver` | Who may decide: `operator` (approve-capable admin credential), `subject` (the application's own end user), or `any`. Defaults to `operator`. |
...
Use `/reject` to settle the hold terminally. Deciding requires an admin token minted with the `approve` capability - `write` alone cannot decide a hold, so day-to-day automation credentials never carry approval authority. The approver is recorded from the authenticated actor, never from the request body, and a hold declared `"approver": "subject"` refuses every operator decision with `subject_approval_required`.

## Decide as the application's end user

A hold declared for `subject` (or `any`) is decided by the application's own authenticated end user through the STS, using the session mandate the application already holds:

```bash
curl -X POST "$CARACAL_STS_URL/step-up/$CHALLENGE_ID/decision" \
  -H "Authorization: Bearer $SESSION_MANDATE" \
  -H "Content-Type: application/json" \
  -d '{"decision": "approved", "binding": "'$BINDING'", "reason": "refund confirmed"}'
```

The decision must echo the hold's binding exactly, so an approval prompt that does not know the precise held resource and scope set cannot decide it. The session that raised the hold cannot approve itself, and the tier's privacy mode governs what the decision record keeps of the approver: `identified` stores the subject verbatim, `pseudonymous` a stable zone-scoped pseudonym, `anonymous` a redaction marker. The approver's session id is always retained as the forensic anchor.
...
| `subject_approval_required` on approve | The policy reserves this decision for the application's end user; no zone credential overrides it. |
...
Related pages: [Step-Up Challenges](/concepts/step-up/) and [Author Policy Data](/guides/author-policy/).
