...
To make agents distinguishable in policy and audit, pass `capabilities` when you spawn. These become `input.principal.capabilities`, so several agents under one application stay separable without one application per agent. `capabilities` are descriptive labels for policy and audit, not grants; authority always comes from scopes and delegation. The session lifecycle is handled for you: `spawn()` retires the session when the block exits, and the coordinator derives the right kind from the application (ephemeral for DCR, instance for managed).

```python
async with caracal.spawn(capabilities=["refund-agent"]):
    headers = caracal.headers()
```
...
```python
svc = await caracal.service(capabilities=["billing-worker"])
try:
    while running:
        await svc.heartbeat()
