...
The reconstructed input is redaction-safe: actor and subject claims are never written to audit, so add any claim-dependent fields before simulating. The [Iterate Policy Safely example](/examples/policy-iterate/) wraps this loop as a runnable script.
...
Run [Iterate Policy Safely](/examples/policy-iterate/) for the first real denial, or [Debug Authorization Decisions](/guides/authorize-access/) when live behavior differs from simulation.
