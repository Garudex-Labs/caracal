...
The migration from Caracal's earlier mutable-row keys to durable receipts requires Coordinator replicas to restart without mixed old/new overlap. The Helm chart therefore uses the `Recreate` strategy for Coordinator. Existing runtime stacks already restart Coordinator as one Compose service during upgrade. Expect a brief pause in Coordinator mutations while the replacement becomes ready; runtime mandates and Gateway enforcement continue independently.
