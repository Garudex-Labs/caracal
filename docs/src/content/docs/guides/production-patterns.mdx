...
description: Choose and validate production enforcement, identity, propagation, and audit boundaries.
pageType: workflow
---
...
Use [Protect a Gateway-Routed HTTP API](/guides/protect-gateway-http/) for the default HTTP path. Use the framework-specific protection guides when enforcement must live in the resource server.
...
| Approval | Gated scopes reach the intended operator or Federated user decision plane. |
...
Complete the focused guide for the selected enforcement boundary, then use [Operate Caracal](/operations/) to deploy and monitor it.