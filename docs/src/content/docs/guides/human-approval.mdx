...
| `ttl_seconds` | How long the hold stays decidable. Subject-decidable holds (`subject` or `any`) default to 15 minutes and cap at 24 hours; operator-only holds default to 4 hours and cap at 7 days. The floor is 60 seconds everywhere. |
...
An approval's lifetime is its own: it never inherits or extends a session's lifetime. A subject decision is an interactive consent moment, so its window is short by default; an operator decision is an administrative review and gets hours. Tune `ttl_seconds` to the tier's real decision window: it bounds how long an approved-but-unconsumed hold stays spendable, so a shorter TTL shrinks the window in which a leaked approval could be consumed, while a TTL shorter than your approvers' actual response time just converts legitimate requests into expiries. Expiry is cheap by design - an expired hold never resumes anything, the requester simply raises a fresh one, and terminal rows age out of the store automatically.
