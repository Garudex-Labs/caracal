...
Labels are policy input, so one bundle-wide rule can confine every customer-labeled agent to the customer-record surface — whatever its role would otherwise allow:
...
customer_confined if { not customer_labeled }
...
Add `customer_confined` to the conditions of your mint rule and a worker spawned for one customer can never mint authority outside that surface, even by accident. The [Lynx Capital example](/examples/lynx-capital/) ships this rule with tests.
