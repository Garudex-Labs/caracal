...
| Agent session | One agent run, labeled and attributed to one customer | Yes - one per customer task |
...
Start the session for a customer's request inside the one zone and stamp the customer id into the session's metadata. The metadata travels with the agent session and makes per-customer attribution a direct lookup instead of a guess.
...
Agent metadata travels with the session and lands in audit, but it is **not** part of policy input. The platform decision contract decides from the agent's labels (`input.principal.labels`) and the delegation edge - never from the metadata map. Use metadata to *find* a customer's activity; use labels when policy must *restrict* it.
:::
...
When policy must confine the work itself - a billing sweep, a dunning cycle, any job acting on one customer's records - carry the customer on the agent session as a label:
...
Every decision is written to the zone audit ledger with the application, session, agent session, and delegation chain. The customer id rides in the agent metadata and labels you set in Steps 2 and 3, so you can answer "what did this customer do, with what authority" from the audit trail. See [Audit and Request Traces](/concepts/audit-ledger/).
