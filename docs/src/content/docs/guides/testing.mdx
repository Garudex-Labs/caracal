...
| Call | Respond with |
| --- | --- |
| `POST /zones/{zone}/agents` | `{"agent_session_id": "agent-1"}` (add `heartbeat_deadline_at` for service sessions) |
| `POST /zones/{zone}/delegations` | `{"delegation_edge_id": "edge-1"}` |
...
Return an error status to exercise failure paths: a `503` drives the idempotent spawn retry, `429` with `Retry-After` drives hinted backoff, and a `401` from an `interaction_required` body raises the approval hold.
...
Two seams make assertions precise without reaching into SDK state. The fake transport records every request, so you can assert the wire contract your integration depends on - an `idempotency-key` header on spawns, the delegation body, the terminate on session exit. And `onEvent` reports every control-plane operation as data, so a test can subscribe and assert that a flow produced exactly the expected `coordinator.call`, `token.exchange`, or `delegation.accept` events, the same way production observability consumes them.
