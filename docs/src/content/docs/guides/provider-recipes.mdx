...
Provider recipes live in the documentation, not the web console. The Console stays focused on the fields a given setup needs; this page is the reference you keep open beside it.
...
Bindings can also live in the runtime profile as `[[credentials]]` entries; see [Configuration File](/runtime-console/config-file/). Provider clients still require an API key argument at construction - pass a placeholder such as `caracal-gateway`. The transport replaces the `Authorization` header on Gateway-routed calls, so the placeholder never reaches the provider. The examples use `inference:invoke`; register and grant that scope on each resource, or replace it with the resource's configured operation scope.

Each recipe below assumes a constructed SDK client (`const caracal = new Caracal()`, `caracal = Caracal()`, `client, _ := caracal.New()`) and runs inside a governed Session context.
