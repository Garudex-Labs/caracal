...
Stored connections follow the upstream token's own lifecycle. A grant whose provider returned no expiry is treated as non-expiring and served as stored; a grant with a refresh token is refreshed inside STS shortly before expiry, so the Gateway is never handed an upstream token about to lapse; a lapsed grant without a refresh token transitions to `expired` and the caller receives a precise `credential_expired_not_renewable` denial rather than a generic policy error. The provider's Connections panel reads this state live - `GET /v1/zones/{zoneId}/provider-grants` is the same listing for automation - and reconnecting the same user, resource, and provider replaces the active grant. Providers that rotate refresh tokens on every use are supported: each refresh stores the newest rotation under optimistic locking so concurrent refreshes cannot resurrect a superseded token.
