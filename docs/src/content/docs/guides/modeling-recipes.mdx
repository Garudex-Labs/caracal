...
The principal is always your application; agent sessions carry its labels and delegation context. The real fork is where the upstream credential lives.
...
| Audit attribution | Application, agent session, and delegation chain. | The same, plus the provider connection the exchange used. |

Policy tells work apart with `input.principal.registration_method`, `input.principal.agent_session_id`, and `input.principal.labels`. Authorization inputs are the application, labels, roles, delegation, and confinement - never the subject identifier. See [Identities and Applications](/concepts/principal/).
