...
await caracal.session(async () => {
  await caracal.session(
    async () => {
      await caracal.fetch(resourceId, '/tickets', { scopes: ['tickets:read'] })
    },
    {
...
await caracal.session(async () => {
  await caracal.session(
    async () => {
      await caracal.fetch('resource://pipernet', '/tickets', { scopes: ['tickets:read'] })
    },
    {
      authority: Authority.narrow(['tickets:read'], {
...
```ts
await caracal.fetch('resource://pipernet', '/tickets', {
  scopes: ['tickets:read'],
})
```
