...
A cycle costs five control-plane calls (a lifecycle bootstrap mint, two session starts, one delegation, the final mint) and runs roughly **once per mandate TTL** - 15 minutes by default - with concurrent requests sharing one in-flight cycle. Every request in between is served from cache, so the steady-state overhead is effectively zero; tune `mandateTtlSeconds` only when policy requires shorter-lived authority.
