...
The outer `session()` creates the lifecycle parent. The narrowed child creates the positive-TTL Delegation required for resource authority, and `fetch()` mints a one-shot Gateway mandate for the protected call. Both Sessions terminate when their callbacks exit.
...
await caracal.session(async () => {
  await caracal.session(
    async () => {
      await caracal.fetch('resource://pipernet', '/reports', { scopes: ['pipernet:read'] })
    },
    {
      authority: Authority.narrow(['pipernet:read'], {
...
The SDK retries selected Coordinator and STS transient failures with stable operation identity. It does not retry Gateway side effects. Use [Safe Retries and Idempotency](/guides/idempotency/) before retrying a mutation.
:::

For exact constructors, options, and return types, use [TypeScript SDK reference](/sdks/typescript/).

## Next step

Add [multi-agent delegation](/guides/delegation/) only if one Session must hand a narrower slice to another; otherwise validate the resource boundary with [Protect a Gateway-Routed HTTP API](/guides/protect-gateway-http/).
