...
To make agents distinguishable in policy and audit, pass `capabilities`. These become `input.principal.capabilities`, so several agents under one application stay separable without one application per agent. `capabilities` are descriptive labels for policy and audit, not grants; authority always comes from scopes and delegation. The session lifecycle is handled for you: the coordinator derives the right kind from the application (ephemeral for DCR, instance for managed).

```ts
await caracal.spawn(async () => {
  const headers = await caracal.headersAsync();
  await fetch("https://api.example.com/tickets", { headers });
}, { capabilities: ["refund-agent"] });
```
...
```ts
const svc = await caracal.service({ capabilities: ["billing-worker"] });
try {
  while (running) {
    await svc.heartbeat();
