...
2. `caracal.toml` in the OS Caracal config directory, when present.
...
config directory. Setting `CARACAL_ENV=production` disables that auto-detection
and requires explicit service URLs. Cloud and custom deployments can provide
explicit `CARACAL_APP_CLIENT_SECRET_FILE`, `CARACAL_RUN_CREDENTIALS`,
