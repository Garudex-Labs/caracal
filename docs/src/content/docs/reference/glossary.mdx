...
| Sign-in | Authenticated subject session a user or service establishes in a zone; sessions act on behalf of a sign-in and reference it for attribution. |
...
- Use `session` for the governed execution record and `sign-in` for the authenticated subject session; never use one for the other.
