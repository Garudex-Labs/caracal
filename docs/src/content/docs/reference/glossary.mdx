...
Contrast with [Per-call mandate](#per-call-mandate).
...
The general term for a JWT issued by the STS that grants a principal authority to perform operations. Mandates come in two forms: [per-call](#per-call-mandate) (15-minute TTL, `"use": "per_call"`) and [ambient](#ambient-mandate) (60-minute TTL, `"use": "ambient"`). Both are signed with the zone's ECDSA P-256 key and carry the full delegation chain.
...
Contrast with [Ambient mandate](#ambient-mandate).
