...
<div id="faq-registry" className="faqRegistry">
    ...
    <details id="faq-022" className="faqEntry faqEntryImportant" data-faq-entry data-faq-category="model" data-faq-id="FAQ-022" data-faq-title="If spawned children inherit the parent's application, when is a DCR application actually used?" data-keywords="DCR ephemeral agent lifecycle managed authenticate as client credentials root session per tenant isolation use case orchestrator">
      ...
      <p>The credential boundary between durable managed identities and short-lived DCR identities is named by <code>registration_method</code> (managed vs DCR), not by an agent's <code>lifecycle</code>. A session's <code>lifecycle</code> is either <code>agent</code> (the default) or <code>service</code> (heartbeat-leased), and a DCR application cannot host a <code>service</code> session, so its one session is always an <code>agent</code>. A short-lived worker is therefore <em>not</em> a separate lifecycle — it is an ordinary <code>agent</code> session with a TTL (see <a href="#faq-023">FAQ-023</a>).</p>
    ...
    <details id="faq-025" className="faqEntry faqEntryImportant" data-faq-entry data-faq-category="model" data-faq-id="FAQ-025" data-faq-title="What is the difference between an agent and a service lifecycle — and how do I model a task-and-die worker versus a time-limited one?" data-keywords="agent service lifecycle task scoped ttl heartbeat lease worker search agent die descriptor registration method">
      <summary className="faqEntryHeader">
        <span className="faqNumber">FAQ-025</span>
        <span className="faqTag faqTagIdentity">Identity</span>
        <h3>What is the difference between an agent and a service lifecycle — and how do I model a task-and-die worker versus a time-limited one?</h3>
      ...
      <p>There are only <strong>two runtime lifecycles</strong>, and they map directly onto the two SDK primitives:</p>
      <ul>
        <li><strong>Task-scoped agent</strong> — created with <code>spawn()</code>, recorded as <code>lifecycle = "agent"</code>. It lives for the duration of its task: when its block exits it is terminated automatically. This single behavior covers <em>both</em> of the cases you are distinguishing. A "do one task and die" worker (for example a search sub-agent) is just <code>spawn()</code> whose block returns when the task is done. A "live up to N seconds then expire" worker is the same <code>spawn()</code> with <code>ttl_seconds=N</code>, which adds a hard wall-clock cap enforced by the TTL sweeper. The difference between "task-and-die" and "time-limited" is <strong>whether you set a TTL</strong>, not a different lifecycle.</li>
        <li><strong>Service agent</strong> — created with <code>service()</code>, recorded as <code>lifecycle = "service"</code>. It is the genuinely different lifecycle: it outlives any single task and is kept alive by a heartbeat lease, so the runtime can detect and reap it if it stops reporting.</li>
      </ul>
      <p>The stored <code>lifecycle</code> column carries exactly these two values, and <strong>only <code>service</code> changes runtime behavior</strong> (the heartbeat lease). An earlier <code>instance</code>/<code>ephemeral</code> split has been collapsed into <code>agent</code> because it was inert — it shared the same TTL sweeping, the same heartbeat rules, and the same enforcement, and merely mirrored <code>registration_method</code>, which still names the managed-vs-DCR credential boundary on its own. So how long a worker lives is expressed by task scope and an optional TTL, not by a separate lifecycle. Reach for <code>service()</code> when you need a durable, heartbeat-leased agent; otherwise use <code>spawn()</code> and let task scope (optionally plus a TTL) express how long the worker should live.</p>
    ...
    <details id="faq-026" className="faqEntry" data-faq-entry data-faq-category="model" data-faq-id="FAQ-026" data-faq-title="Can two DCR applications have different policies, and can a DCR agent spawn children?" data-keywords="DCR policy per application labels application id different policy spawn parent leaf one session bind registration method lifecycle">
      ...
      <p><strong>Which sessions can be parents:</strong> the coordinator imposes no lifecycle-based restriction on being a parent, so any session under a managed application — whether its <code>lifecycle</code> is <code>agent</code> or <code>service</code> — can spawn children via <code>spawn()</code>. The only structural non-parent is the DCR session described above. So the practical answer is that managed sessions spawn and DCR sessions do not.</p>
