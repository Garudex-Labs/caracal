...
<div id="faq-registry" className="faqRegistry">
    ...
    <details id="faq-022" className="faqEntry faqEntryImportant" data-faq-entry data-faq-category="model" data-faq-id="FAQ-022" data-faq-title="If spawned children inherit the parent's application, when is a DCR application actually used?" data-keywords="DCR ephemeral agent kind derived managed instance authenticate as client credentials root session per tenant isolation use case orchestrator">
      <summary className="faqEntryHeader">
      ...
      <p>A DCR application is used by <strong>authenticating as it</strong>, not by spawning under it. This is the point that usually causes confusion, so start from how <code>agent_kind</code> is decided: the coordinator derives it from the application — a session under a managed application is <code>instance</code>, and a session under a DCR application is <code>ephemeral</code>. Since <code>spawn()</code> always runs under the caller's own application, a spawned child of a managed application is <code>instance</code>; through the SDK, <code>spawn()</code> does not produce an <code>ephemeral</code> session, and a DCR application — bound to exactly one session — cannot be a spawn parent. So an <code>ephemeral</code> agent exists as the single root session of a DCR application.</p>
      <p><code>instance</code> and <code>ephemeral</code> are <strong>descriptors, not runtime behaviors</strong>. They share the same heartbeat rules, the same TTL sweeping, and the same enforcement; the split only mirrors <code>registration_method</code> (managed vs DCR) so policy and audit can name the credential boundary. The only <code>agent_kind</code> with distinct runtime behavior is <code>service</code> (it holds a heartbeat lease). A short-lived worker is therefore <em>not</em> an <code>ephemeral</code> kind — it is an ordinary <code>instance</code> session with a TTL (see <a href="#faq-023">FAQ-023</a>).</p>
      <p>That root session is created by a workload that holds the DCR application's credentials. The control plane registers the DCR application through the Admin API DCR endpoint, which returns a one-time client secret and a short expiry; the SDK never registers applications. An orchestrator injects those credentials into an independently launched workload — for example a per-tenant container — and that workload, configured with the DCR <code>application_id</code> and secret, authenticates with the <code>client_credentials</code> grant and creates its one ephemeral session. The application then expires (one hour or less) and is archived by DCR cleanup.</p>
    ...
    <details id="faq-024" className="faqEntry faqEntryImportant" data-faq-entry data-faq-category="security" data-faq-id="FAQ-024" data-faq-title="If A narrows a grant to B, and B spawns C without narrowing, does C stay bounded by B's restriction?" data-keywords="delegation chain inherit narrow grant escalation A B C subset application boundary policy least privilege subtree hop">
      ...
      <p>No — and this is the most important thing to understand about delegation. A delegation edge bounds the <strong>token issued for that edge</strong> and <strong>any further edge chained from it</strong>; it does <strong>not</strong> lower the application's authority ceiling. Take A spawns B with <code>grant=narrow([tickets:read])</code>, then B spawns C:</p>
      <ul>
        <li>If B spawns C with <strong>inherit</strong> (the default, no grant), <strong>no edge is recorded for C</strong>. C authenticates as the application and re-derives the application's authority bounded by policy — <em>not</em> B's <code>tickets:read</code> slice. B did not "grant" anything; C simply runs as the app.</li>
      ...
      <p>So the rule "an agent can never grant what it does not have" is enforced <strong>for edges</strong> — but <code>inherit</code> creates no edge, so it is not a grant and is not bounded by an ancestor's narrowing. The hard, server-enforced boundary that contains <em>every</em> session is the <strong>application plus policy</strong> (default-deny). Delegation edges are a least-privilege tool <em>within</em> that boundary. To contain an entire subtree below the application, narrow at <strong>every hop</strong>, or place the subtree under a different application with <code>delegate(to=peer)</code> — a real cross-application boundary that requires receiver consent. Do not rely on a single narrowed intermediary to contain its descendants: all sessions of one application share a credential and a trust domain by design.</p>
    ...
    <details id="faq-025" className="faqEntry faqEntryImportant" data-faq-entry data-faq-category="model" data-faq-id="FAQ-025" data-faq-title="What is the difference between an instance, ephemeral, and service agent — and how do I model a task-and-die worker versus a time-limited one?" data-keywords="agent kind instance ephemeral service lifecycle task scoped ttl heartbeat lease worker search agent die descriptor registration method">
      <summary className="faqEntryHeader">
        <span className="faqNumber">FAQ-025</span>
        <span className="faqTag faqTagIdentity">Identity</span>
        <h3>What is the difference between an instance, ephemeral, and service agent — and how do I model a task-and-die worker versus a time-limited one?</h3>
      ...
      <p>There are only <strong>two real runtime lifecycles</strong>, and they map directly onto the two SDK primitives:</p>
      <ul>
        <li><strong>Task-scoped agent</strong> — created with <code>spawn()</code>. It lives for the duration of its task: when its block exits it is terminated automatically. This single behavior covers <em>both</em> of the cases you are distinguishing. A "do one task and die" worker (for example a search sub-agent) is just <code>spawn()</code> whose block returns when the task is done. A "live up to N seconds then expire" worker is the same <code>spawn()</code> with <code>ttl_seconds=N</code>, which adds a hard wall-clock cap enforced by the TTL sweeper. The difference between "task-and-die" and "time-limited" is <strong>whether you set a TTL</strong>, not a different kind of agent.</li>
        <li><strong>Service agent</strong> — created with <code>service()</code>. It is the genuinely different lifecycle: it outlives any single task and is kept alive by a heartbeat lease, so the runtime can detect and reap it if it stops reporting.</li>
      </ul>
      <p>The stored <code>agent_kind</code> column today carries the values <code>instance</code>, <code>ephemeral</code>, and <code>service</code>, but <strong>only <code>service</code> changes runtime behavior</strong> (the heartbeat lease). <code>instance</code> and <code>ephemeral</code> are <em>descriptors</em>: they share the same TTL sweeping, the same heartbeat rules, and the same enforcement, and the split simply mirrors <code>registration_method</code> (a managed application's session is labelled <code>instance</code>, a DCR application's session is labelled <code>ephemeral</code>). So "ephemeral" in the data does <strong>not</strong> mean "task-and-die" — task scoping is already how every <code>spawn()</code> behaves regardless of the label. Reach for <code>service()</code> when you need a durable, heartbeat-leased agent; otherwise use <code>spawn()</code> and let task scope (optionally plus a TTL) express how long the worker should live.</p>
    ...
    <details id="faq-026" className="faqEntry" data-faq-entry data-faq-category="model" data-faq-id="FAQ-026" data-faq-title="Can two DCR applications have different policies, and can an ephemeral or DCR agent spawn children?" data-keywords="DCR policy per application labels application id different policy spawn parent ephemeral leaf one session bind registration method">
      <summary className="faqEntryHeader">
        <span className="faqNumber">FAQ-026</span>
        <span className="faqTag faqTagIdentity">Identity</span>
        <h3>Can two DCR applications have different policies, and can an ephemeral or DCR agent spawn children?</h3>
      ...
      <p><strong>Can a managed "ephemeral"-labelled session spawn children:</strong> the coordinator imposes no kind-based restriction on being a parent, so in principle a managed-application session can spawn regardless of its descriptor label. But through the SDK you never create an <code>ephemeral</code>-labelled managed session in the first place (the SDK does not expose <code>kind</code>), and the label has no runtime effect (see <a href="#faq-025">FAQ-025</a>) — so the practical answer is that managed sessions spawn via <code>spawn()</code> and DCR sessions do not spawn at all.</p>
