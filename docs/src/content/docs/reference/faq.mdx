...
<section className="faqSearchPanel" aria-label="Search FAQs">
  <div className="faqSearchMeta">
    <span id="faq-search-status" className="faqSearchStatus" aria-live="polite">22 results</span>
  </div>
...
<div id="faq-registry" className="faqRegistry">
    ...
    <details id="faq-022" className="faqEntry faqEntryImportant" data-faq-entry data-faq-category="model" data-faq-id="FAQ-022" data-faq-title="If spawned children inherit the parent's application, when is a DCR application actually used?" data-keywords="DCR ephemeral agent kind derived managed instance authenticate as client credentials root session per tenant isolation use case orchestrator">
      ...
      <p>A DCR application is used by <strong>authenticating as it</strong>, not by spawning under it. This is the point that usually causes confusion, so start from how <code>agent_kind</code> is decided: the coordinator derives it from the application alone — a session under a managed application is always <code>instance</code>, and a session under a DCR application is always <code>ephemeral</code>. Since <code>spawn()</code> always runs under the caller's own application, a spawned child of a managed application is always <code>instance</code>. <code>spawn()</code> cannot produce an <code>ephemeral</code> session, and a DCR application — bound to exactly one session — cannot be a spawn parent. So an <code>ephemeral</code> agent only ever exists as the single root session of a DCR application.</p>
