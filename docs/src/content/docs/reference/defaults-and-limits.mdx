...
| Session start (spawn) | up to 2 retries | 5xx/network only, under an idempotency key: retrying resumes the created session instead of duplicating it. |
| Delegation create | 1 retry | Same idempotency contract. |
