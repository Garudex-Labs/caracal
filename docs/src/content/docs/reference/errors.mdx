...
| `hop_count_exceeded` | 403 | The delegation chain has more hops than the `max_hops` constraint allows | Reduce delegation depth; check `max_hops` constraints on parent delegation edges |
...
| Metric | Trigger |
| --- | --- |
| `caracal_gateway_denials_missing_auth_total` | No `Authorization` header |
| `caracal_gateway_denials_bad_bearer_total` | Bearer token malformed |
| `caracal_gateway_denials_expiring_total` | Mandate TTL expired |
| `caracal_gateway_denials_bad_routing_total` | No matching binding for the request path |
| `caracal_gateway_denials_path_traversal_total` | Path traversal detected in the request URI |
| `caracal_gateway_denials_signature_total` | JWT signature verification failed |
| `caracal_gateway_denials_jti_replay_total` | JTI already seen — replay attempt or clock skew |
| `caracal_gateway_denials_revoked_total` | Session revoked |
| `caracal_gateway_denials_binding_total` | Scope or resource binding does not match |
...
```go
import "github.com/garudex-labs/caracal/packages/core/go/errors"
