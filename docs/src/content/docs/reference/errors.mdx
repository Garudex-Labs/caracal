...
| `agent_identity_required` | A verifier requires an agent identity. | In-process verifier or verify engine. | Run the call from an agent session, not a bare subject session. |
...
| `CoordinatorError` | Session, delegation, and heartbeat calls | `status`, `method`, `path`, `retryAfterSeconds` | Branch on `status`: 401 means a rejected bearer (the SDK already refreshed once), 404/409 means the session is gone - re-attach or start fresh. |
