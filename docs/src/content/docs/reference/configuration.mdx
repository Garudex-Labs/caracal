...
flowchart TD
  ...
  Need -->|caracal run launch| Identity[Workload identity env plus console Run bindings]
...
| Key | Meaning |
| --- | --- |
| `CARACAL_APPLICATION_ID` | Confidential application ID; required by `caracal run`. |
| `CARACAL_APP_CLIENT_SECRET` | Inline local-development client secret. |
| `CARACAL_APP_CLIENT_SECRET_FILE` | Cloud/custom mounted client-secret file path. |
| `CARACAL_STS_URL` | Cloud/custom STS URL override. |

Credential bindings, zone, TTL, and failure behavior come from the application's run manifest, authored in the web console. Local dev and stable launches can omit both secret variables and read the owner-only file at `<Caracal config dir>/runtime/<application_id>/client-secret`.
