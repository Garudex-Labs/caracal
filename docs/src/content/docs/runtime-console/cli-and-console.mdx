...
human product management              -> caracal web (web console)
...
| Open the web console | `caracal web` |
...
| Trace one known request ID | **Request Traces** |
...
| Symptom | Check |
| --- | --- |
| The web console cannot reach product data | Run `caracal up` and confirm `caracal status --ready`; use `caracal web --allow-offline` only for sign-in or UI-only checks. |
