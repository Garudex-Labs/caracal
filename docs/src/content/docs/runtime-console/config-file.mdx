...
`CARACAL_APPLICATION_ID`. The runtime and SDKs then read the client secret and
credential manifest from these paths under the OS Caracal config directory:
...
You create these files yourself. The web console shows the one-time client secret
at application creation but never writes to your machine:

```bash
mkdir -p ~/.config/caracal/runtime/<zone_id>/<application_id>
printf '%s' '<client secret>' > ~/.config/caracal/runtime/<zone_id>/<application_id>/client-secret
chmod 600 ~/.config/caracal/runtime/<zone_id>/<application_id>/client-secret
```

The `credentials.json` manifest at the same path holds `credentials` and
`optional_credentials` arrays with the same entry fields as the TOML schema below.
These paths are read only in local dev and stable modes; production launches
(`NODE_ENV=production`) require explicit secret-file configuration.
...
- Store local credential files at the runtime path with owner-only permissions for dev and stable launches.
...
local runtime paths with `CARACAL_APP_CLIENT_SECRET_FILE`,
...
Author the runtime profile from the values you created in the web console: the zone, application, and resource identifiers, the application's one-time client secret, and a credential mapping for each resource. Save it as `caracal.toml` at the resolved config path with owner-only permissions, or point `CARACAL_CONFIG` at it.
