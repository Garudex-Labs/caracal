...
Edit the host file from any OS with `caracal config`:

```bash
caracal config set CARACAL_WORKLOAD_ID fiona
caracal config list          # secret values are masked
caracal config path          # print the file location
```
