...
The Lynx Capital example is a complete Python reference lab at `examples/lynxCapital/` that runs a realistic weekly payout cycle: 4,200 invoices, 5 geographic regions, approximately $8.5M USD in payments, multi-currency settlement, regional tax compliance, vendor contract verification, and threshold-based routing across banking rails. Caracal is real in this example: the Python SDK, STS token exchange, coordinator sessions, gateway resource routing, Console, and control API are the actual integration path. Local provider fixtures are isolated under `_mock/`; application code treats providers through normal clients and transports.
...
### Start Caracal through Console + control API

Install the released Runtime and Console, then start the Caracal stack:
...
In the Console, open **Control**, confirm **enable endpoint**, then create or inspect:
...
After creating the Lynx zone, control key, and resources in the Console, use the Console-generated runtime profile contract. Enable **write files** when you want the Console to create the profile and one-time secret file locally; otherwise write the control key secret to an owner-only file and write `~/.config/caracal/caracal.toml` from the Console values. Add one SDK resource binding per Lynx provider:
...
All 14 external providers run through deterministic local fixtures in Docker. They are reference-lab fixtures, not Caracal product defaults. The provider network joins `caracalData` so the real Caracal Gateway can forward REST resource calls to provider aliases such as `mercury-bank.mock`:
...
`caracal-console` remains open for live session inspection.
...
configuration after the user creates the Console objects and writes
`caracal.toml`.
