...
| Policy set | `lynx-finance-ops`: default-deny base, generated bindings and grants data, and one policy per application allowing exactly its roles' mandate mints and gateway calls. |
...
resource-view grants; one policy per application allows its roles' mandate mints (scope ∩
delegation edge, role label granted, view owned by the caller) and gateway uses (mandate
target includes the view). Expected access behavior is documented in
`policies/README.md`.
