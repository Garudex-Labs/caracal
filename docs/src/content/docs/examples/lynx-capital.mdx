...
### Start Caracal through Terminal + control API

Install the released runtime shell and terminal, then start the Caracal stack:
...
In the Terminal, open **Control**, confirm **enable endpoint**, then create or inspect:
...
`~/.config/caracal/caracal.toml` from the Terminal values. The generated config
...
`caracal-terminal` remains open for live session inspection.
...
configuration after the user creates the Terminal objects and writes
`caracal.toml`.
