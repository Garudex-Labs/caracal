...
Then send a request through the Gateway with the SDK transport from [First Protected Call](/get-started/first-protected-call/#the-agents-first-protected-call), pointing it at this resource. With the application identity from guided setup in the environment, the call has this shape:
