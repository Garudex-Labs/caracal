...
Then open the console's **Run** page, select the application, and add three launch bindings, each with the **Provider token** credential type:
...
Set the credential TTL to 900 seconds. The Run page then shows the exact launch command and confirms this all-provider-token profile needs no Gateway.

## Store the client secret

The launcher needs one local file: the application's one-time client secret at the runtime secret path.

```bash
export CARACAL_APPLICATION_ID="app_support_research_agent"
mkdir -p ~/.config/caracal/runtime/$CARACAL_APPLICATION_ID
printf '%s' '<one-time client secret>' > ~/.config/caracal/runtime/$CARACAL_APPLICATION_ID/client-secret
chmod 600 ~/.config/caracal/runtime/$CARACAL_APPLICATION_ID/client-secret
```
