...
Use it to evaluate one-shot runtime credential injection for an existing CLI that cannot accept an SDK transport. The launcher limits described in [Run an Agent with caracal run](/guides/runtime-run/) apply: do not use it for a long-running service or as proof of request-level Gateway enforcement.
