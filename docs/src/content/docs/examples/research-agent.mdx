...
Then open the application's **Run** section and add three launch bindings, each with the **Provider token** credential type:
...
Set the credential TTL to 900 seconds. The Run section then shows the exact launch command and confirms this all-provider-token profile needs no Gateway.
