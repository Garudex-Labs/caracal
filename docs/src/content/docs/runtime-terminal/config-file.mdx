...
`caracal.toml` is the configuration file for the `caracal run` and `caracal credential read` commands. It stores the application credentials (zone, application, and client secret) used for token exchange, and declares which scoped tokens to inject into subprocess environments.
...
The terminal interface resolves the config path in this order, stopping at the first match:
...
# Declare scoped tokens to inject into subprocesses
[[credentials]]
...
| --- | --- | --- | --- |
| `zone_url` | string | Yes | STS base URL. The token exchange endpoint is at `{zone_url}/oauth/2/token`. |
...
Declares a required resource-scoped token. Every credential in this array is exchanged before the subprocess is spawned. If any exchange fails and `continue_on_failure` is `false`, the subprocess is not started.
...
Declares an optional resource-scoped token. Exchange failures produce a warning but do not block the subprocess.
...
| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `mode` | `"block"\|"log"` | `"block"` | `"block"` exits with code 1 before spawning. `"log"` logs and continues. |

---

## Environment variable overrides

These environment variables are read by the terminal interface. Most of them override what is in `caracal.toml` or the defaults applied in development mode:
