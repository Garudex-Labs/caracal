...
description: Navigate the caracal-terminal — launch, screen layout, key bindings, views, and live audit streaming.
pageType: reference
---

`caracal-terminal` is a terminal UI for inspecting all Caracal entities without leaving the shell. It fetches from the same control-plane API as the terminal interface and renders into a full-screen terminal buffer with keyboard navigation.
...
On startup, the Terminal enters full-screen alt buffer mode and hides the cursor. Press `q` or `ctrl-c` at any time to exit and restore the terminal.
...
```
┌──────────────────────────────────────────────────────────────────┐
│ Caracal Terminal                                   http://localhost:3000│  banner
...
Press `enter` on an audit event to open its full detail view, showing the complete `AuditDetail` record including `determining_policies_json` and `diagnostics_json`. This is the Terminal equivalent of `caracal explain <request_id>`.

---

## Error display

API errors are shown in the status line in red. The Terminal does not exit on API errors — press `r` to retry, or press `h` to go back and try a different view.
