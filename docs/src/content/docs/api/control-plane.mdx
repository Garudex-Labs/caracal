...
Providers define upstream credential metadata used by STS/Gateway brokered forwarding. OAuth2/OIDC provider grants are forwarded as bearer-style credentials and may be refreshed from `token_endpoint`; API-key grants are forwarded using `header_name` or `auth_header`; workload providers are metadata-only in the current OSS runtime. The `config_json` and `secret_config` fields in the request body are merged — fields matching `/secret|password|token|api[_-]?key|private[_-]?key|credential|passphrase/i` are extracted and encrypted under the zone's DEK before storage. Decrypted secrets are never returned in responses; only `secret_config_keys` indicates which secrets are stored.
