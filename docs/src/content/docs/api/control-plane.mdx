...
Tokens are stored with a SHA-256 lookup fingerprint and an Argon2id verifier hash in the `admin_tokens` table. Each token carries a scope:
...
| --- | --- | --- |
| `invalid_admin_token` | 401 | Token missing, malformed, unknown, or verifier hash does not match |
...
```json
{
  "approver_subject_id": "user_approver_123"
}
```

The approver must not be the same subject that triggered the challenge.

**Response 200:** `{ "id": "challenge-uuid", "satisfied_at": "2026-05-11T20:01:00Z", "approver_subject_id": "user_approver_123" }`

**Error 400:** `invalid_request` when the body is missing or invalid.

**Error 409:** `challenge_not_satisfiable` when the challenge is expired, already satisfied, not found in the zone, or self-approved by the triggering subject.
