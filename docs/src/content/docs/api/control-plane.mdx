...
| Authority records | `GET /v1/zones/:zoneId/sessions`; `/sessions` is the Admin API protocol path for STS exchange records.                                                                                                                                                                                                             |
...
| Sessions          | `GET /v1/zones/:zoneId/agent-sessions`; `agent-sessions` is the Admin API protocol path for governed Sessions. Filter by `status`, `lifecycle`, `label`, `parent_id`, or `application_id`; use `format=csv` to export.                                                                                             |
