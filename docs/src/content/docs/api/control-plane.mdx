...
| Subjects | `GET /v1/zones/:zoneId/subjects`, `GET /v1/zones/:zoneId/subjects/overview?subject_id=`, `POST /v1/zones/:zoneId/subjects/revoke` (the kill switch: revokes every live session record the subject holds, terminates the governed sessions riding them, revokes their delegations and the subject's provider connections, and feeds the revocation stream; the subject id always travels as a query parameter or body field, never a path segment) |
