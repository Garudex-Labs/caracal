...
| `POST` | `/approvals/{id}/decision`              | Subject-plane approval decision, authenticated with a federated user session mandate. |
...
`GET /approvals/{id}?wait={seconds}` returns `pending`, `approved`, `rejected`, `expired`, or `consumed`; the server bounds the wait. The Subject decision endpoint requires the federated Subject mandate and exact approval binding.
