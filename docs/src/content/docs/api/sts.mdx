...
1. **Client authentication** — Verifies `client_secret` (scrypt hash match) or `client_assertion`. Public clients are not supported. Authenticated Gateway broker exchanges use the internal Gateway-to-STS service signature and must carry a `subject_token`.
...
6. **Delegation edge validation** — If `delegation_edge_id` present: verifies edge is active; checks target session matches `agent_session_id`; validates delegation constraints (scope budget, TTL, max hops); traverses the delegation graph to verify path integrity and absence of cycles; re-validates all edges on the path.
...
| `auth_header` | string | Header name the upstream expects. Required and explicit for API-key provider directives. |
| `auth_scheme` | string | Token prefix (e.g., `Bearer`) |
| `provider_token` | string | Present only on authenticated Gateway broker exchanges when `auth_mode` is `provider_oauth` or `provider_apikey`; the actual upstream credential is not returned to normal STS clients |
...
```json
{
  ...
  "delegation_chain": [
    { "application_id": "app1", "agent_session_id": "agent1", "delegation_edge_id": "edge1" }
  ],
  "hop_count": 1,
...
Claims for agent session and delegation are present only when those parameters were provided in the exchange request. For delegated exchanges, mandate expiry is capped by the shortest effective authority bound: the normal STS TTL cap, the delegation edge expiry, and any typed `ttl_seconds` delegation constraint.
...
| `access_denied` | 403 | Delegation edge inactive, expired, or revoked; edge target mismatch; delegation path invalid or cyclical; delegation constraints violated |
