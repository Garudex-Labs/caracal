...
When an SDK starts a Session with `subjectSessionId`, `subject_session_id`, or `SubjectSessionID`, that compatibility field supplies a **Subject authority record ID**. It links the Session to the Authority record for attribution and lifecycle. It does not, by itself, make later resource mandates carry that record's federated `sub`.
