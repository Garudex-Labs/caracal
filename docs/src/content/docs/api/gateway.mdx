...
### 6. Session revocation check

Extracts the `sid` and `agent_session_id` claims and checks the in-memory revocation store. Returns `401 InvalidToken` if the subject session or agent session is revoked.
