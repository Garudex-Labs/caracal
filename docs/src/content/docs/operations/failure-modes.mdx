...
| STS/Gateway audit loss | The affected process becomes NotReady; liveness remains healthy for investigation. | Preserve logs and replay storage, stop risky traffic, scope missing evidence, then replace the affected pod. |
| Coordinator unhealthy | Session and Delegation views and lifecycle management fail.                       | Restore Coordinator readiness, token, DB, Redis, and sweeper health.                                           |
...
An `audit_replay_unavailable` readiness reason indicates that replay storage cannot currently accept durable spills. An `audit_evidence_lost` reason is process-lifetime definitive: Redis delivery and durable persistence both failed for one or more events. Restoring the replay path does not clear that loss latch; preserve evidence and replace the process after incident scoping.
