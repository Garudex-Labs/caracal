...
| 1 / 512 MB *(shipped default)* | 12.7 (single agent) | 86 ms | Unstable under concurrency* |
| 2 / 2 GB | 23.4 | 333 ms | 8-minute soak, 100% success |
| 4 / 4 GB | 41.8 | 584 ms | Stable |

\* The 512 MB default is memory-bound under concurrent load; see [Resource Usage](#resource-usage-and-stability).

Two ceilings apply together:

- **Per resource:** a single `(zone, resource, application)` pair is capped at **16.6 req/s** (1000 mints/minute) by a built-in rate limit. Spread load across resources or applications to exceed it.
...
The STS hashes credentials with Argon2id, which allocates **64 MB per in-flight verification**. On the shipped **512 MB** default, sustained multi-agent load exhausts the container memory limit and the STS is restarted by the container runtime (fail-closed, automatic, ~2-5 s). Raising the STS memory limit to **2 GB eliminated this entirely** in validation. Provision **at least 1.5-2 GB of STS memory** for any concurrent workload.
:::
...
| Audit retention | Enforced ceiling of 365 days |
...
| --- | --- | --- |
| STS mint rate, per `(zone, resource, app)` | 1000 / minute (16.6 req/s) | Denied, fail-closed; recorded as `rate_limited` in audit |
| STS repeated authentication failures, per application | 60 / minute | Temporary block |
| Coordinator requests, per client IP | 600 / minute | `429 rate_limited` |
| Live sessions per zone | 50 | `429 session_zone_limit_exceeded` |
| Live sessions per application | 200 | `429 session_limit_exceeded` |
| Control API calls, per client | 60 / minute | `429` |

To operate at the throughput and concurrency in this report, raise the per-zone session cap and the applicable rate limits to your target, then confirm the change with [Scale Capacity](/operations/scale-capacity/). Configure limits and container resources in [Configure Service Environment](/operations/env-vars/).
...
The shipped default (STS 1 vCPU / 512 MB) is suitable for a single low-rate agent only. Every other profile raises STS memory above the default for the reason in [Resource Usage](#resource-usage-and-stability). Postgres used up to roughly one vCPU-equivalent and 461 MB with 50 connections at 24 req/s; provision it with headroom above that and prefer SSD or NVMe storage.

## Production Recommendations

- **Give the STS at least 1.5-2 GB of memory.** This is the single most important sizing input; the 512 MB default is not safe under concurrency.
- **Raise the per-zone session cap** to your concurrent-agent count before load; the default is 50.
- **Shard across resources or applications** to exceed 16.6 req/s per resource; then add STS replicas to scale beyond one instance.
