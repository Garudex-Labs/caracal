...
| `CaracalGatewayAuditEvidenceLost` | critical | Stop risky Gateway traffic and preserve evidence for the affected pod. |
| `CaracalSTSAuditEvidenceLost` | critical | Stop token issuance through the affected STS pod and preserve evidence. |
...
Gateway irrecoverably lost at least one audit event after both Redis delivery and durable replay persistence failed. The process latches the loss counter and remains NotReady with reason `audit_evidence_lost`; a replay-path-only failure uses `audit_replay_unavailable`. Preserve pod logs and replay-volume state, stop risky traffic through the affected pod, determine the missing-event window, and replace the pod only after the incident scope is recorded.

## CaracalSTSAuditEvidenceLost

STS irrecoverably lost at least one audit event after both Redis delivery and durable replay persistence failed. The process latches the loss counter and remains NotReady with reason `audit_evidence_lost`; a replay-path-only failure uses `audit_replay_unavailable`. Preserve pod logs and replay-volume state, stop token issuance through the affected pod, determine the missing-event window, and replace the pod only after the incident scope is recorded.
