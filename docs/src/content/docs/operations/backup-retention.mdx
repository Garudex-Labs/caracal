...
| -------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Postgres database                | Product state, policies, grants, Authority records, Sessions, audit events, Delegations, outboxes.      |
...
| Coordinator data | `SESSION_RETENTION_DAYS` and `DELEGATION_RETENTION_DAYS` (90 days by default), `OUTBOX_RETENTION_DAYS`, `IDEMPOTENCY_RETENTION_SECONDS` (seven days by default), and sweeper intervals. Terminal Session trees are removed leaf-first after their retention window; long-term evidence remains in Audit. Restores also restore live replay receipts, preventing pre-backup operations from being silently recreated. |
...
5. Confirm audit query, policy-set activation state, resource routing, Authority records, Sessions, and Delegations.
