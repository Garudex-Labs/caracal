...
The control service is unmounted by default and managed through the engine. Use CLI or TUI controls instead of operating the Compose profile directly:

```bash
caracal control mount
caracal control enable
caracal control status
caracal control disable
caracal control unmount
```

The TUI exposes the same flow under **Control**. Use `mount` to prepare Control without exposing the endpoint, `enable` and `disable` for fast logical on-off control, and `unmount` to remove the optional runtime for long-term idle state. The engine persists lifecycle state and includes the optional profile only when the endpoint should run.

**Required inside the managed service:** `CARACAL_CONTROL_ENABLED=true`, `STS_JWKS_URL`, `STS_ISSUER_URL`, `CONTROL_AUDIENCE`, `CARACAL_API_URL`, `CONTROL_API_TOKEN`.
