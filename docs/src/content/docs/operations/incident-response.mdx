...
| `CaracalGatewayAuditEvidenceLost` or `CaracalSTSAuditEvidenceLost` | Critical evidence-loss incident. |
...
| STS/Gateway audit evidence lost | Critical | Stop risky traffic through the affected pod; keep liveness available for evidence collection. | Pod logs, dropped metric, replay volume, Redis health, request timeline. | Missing-event window is scoped, evidence is preserved, and the affected process is replaced. |
...
- Request IDs, zone IDs, resource IDs, policy-set versions, Session IDs, Delegation IDs.
