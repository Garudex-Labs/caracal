...
`smokeTest.sh` probes API `/ready` and `/live`, Gateway `/ready`, STS `/ready`, Audit `/ready`, and Coordinator `/ready`.
...
Use Console `diagnostics` for API health, readiness, zone diagnostics, and local preflight checks. Use Console `audit`, `explain`, and `credential` views for decision and token investigation.
...
| Smoke test fails only for API `/live` | Confirm the API exposes `/live` in the deployment shape being tested. |
