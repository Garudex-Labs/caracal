...
In published builds (`CARACAL_MODE=rc` or `stable`), every metrics endpoint fails closed: it returns `401` unless `METRICS_BEARER` is set and the scraper presents `Authorization: Bearer <token>`. This applies to API, STS, Gateway, Audit, and Coordinator, all of which are reachable on the internal service network. Provision `METRICS_BEARER` (or `METRICS_BEARER_FILE`) and configure the scraper with the same token to enable collection. In `dev` mode metrics stay open for local inspection.
