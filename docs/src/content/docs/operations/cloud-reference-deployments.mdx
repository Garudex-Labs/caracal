...
| --- | --- |
| `caracal/postgres` | `username`, `password`, `host`, `port`, `database`, plus an auth-session database named `caracal_auth`. |
...
| `caracal/auth-secret`, `caracal/auth-database-url` | Better Auth signing secret and the dedicated web session database URL. |
...
The `ExternalSecret` composes `databaseUrl`, `authDatabaseUrl`, and `redisUrl` and materializes the exact keys the chart mounts, so `secrets.create` stays `false`.
