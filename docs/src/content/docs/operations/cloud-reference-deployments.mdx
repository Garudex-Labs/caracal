...
- `values.cloud-managed.yaml` - production overlay with externalized state, cert-manager TLS web ingress, ServiceMonitor, and tightened egress.
...
| [cert-manager](https://cert-manager.io/)                  | Issues TLS certificates for the web console ingress and any optional API or Gateway ingress. |
...
The `ExternalSecret` composes `databaseUrl`, `redisUrl`, and the per-service connection strings - each service connects as its own least-privilege database role, with passwords derived from the administrative password and provisioned by the migration job - and materializes the exact keys the chart mounts, so `secrets.create` stays `false`.
...
4. Confirm the web Ingress loads the console at the `services.web.publicUrl` origin and API/Gateway Ingress are disabled unless explicitly required.
...
Use [Harden Production](/operations/tls-hardening/) before opening the web console or any optional API/Gateway ingress to production traffic.
