...
| `CARACAL_OPERATOR_EMAILS` / `CARACAL_OPERATOR_EMAILS_FILE` | production yes | none | Comma-separated allowlist of operator emails and `@domain` suffixes permitted to register and sign in. Without it, registration is open in development and closed in production. |
| `CARACAL_OPEN_REGISTRATION` | no | `true` in development, `false` in production | Overrides the no-allowlist registration default. An explicit allowlist always wins. |
