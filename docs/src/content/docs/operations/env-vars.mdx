...
Every `caracal` command loads one operator env file: `$CARACAL_HOME/caracal.env` on an installed host (override with `CARACAL_ENV_FILE`), or `.env` at the repository root in development, where `infra/docker/dev.env` holds the Compose defaults. A variable already set in the process environment always wins over the file; open the file in an editor to change values. For a supported secret, `NAME_FILE` is resolved before `NAME`.
