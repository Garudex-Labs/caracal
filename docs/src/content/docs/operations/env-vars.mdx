...
`caracal run` reads only workload identity; its credential bindings come from the application's run manifest in the web console:

| Variable | Meaning |
| --- | --- |
| `CARACAL_APPLICATION_ID` | Confidential application ID. Required. |
| `CARACAL_APP_CLIENT_SECRET` | Inline local-development client secret. |
| `CARACAL_APP_CLIENT_SECRET_FILE` | Cloud/custom mounted client-secret file. |
...
owner-only file at `<Caracal config dir>/runtime/<application_id>/client-secret`.
