...
| Service | Variables |
| --- | --- |
| API | `CARACAL_ADMIN_TOKEN`, `SECRET_STORE_KEK`, `CARACAL_SECRET_BACKEND`, `GATEWAY_STS_HMAC_KEY`, `STS_URL`, `API_BODY_LIMIT_BYTES`, `API_V1_RATE_LIMIT_PER_MIN`, `API_READY_OUTBOX_DEAD_MAX`, `TRUST_PROXY`, `CARACAL_CONTROL_ENABLED`, `AUDIT_HMAC_KEY`, `API_OPERATOR_*` |
| STS | `ISSUER_URL`, `SECRET_STORE_KEK`, `CARACAL_SECRET_BACKEND`, `GATEWAY_STS_HMAC_KEY`, `STS_ADMIN_TOKEN`, `MAX_GRANT_TTL_SECONDS`, `OPA_POLL_SECONDS`, `AUDIT_REPLAY_DIR` |
...
SDK profile loading reads:

| Variable | Meaning |
| --- | --- |
| `CARACAL_CONFIG` | Explicit SDK profile path. |
| `CARACAL_CONFIG_HOME` | Overrides the OS Caracal config directory used for profile and credential auto-detection. |
| `CARACAL_ENV` | Set to `production` to require explicit service URLs and disable local credential auto-detection. |
| `CARACAL_ALLOW_INSECURE_CONFIG_URLS` | Set to `true` to allow plaintext `http` control-plane URLs in production; the SDK logs a warning banner while it is active. |
...
| `CARACAL_BOOTSTRAP_TOKEN` | Static bootstrap subject token for deployments that do not exchange a client secret. Expired or `alg: none` JWTs are rejected at startup. |
...
| `CARACAL_APP_RESOURCES` | Comma-separated resource audiences for client-secret exchanges when no credential manifest supplies them. |
...
| `CARACAL_DEFAULT_TTL_SECONDS` | Default TTL for task spawns that do not pass an explicit TTL. Positive integer. |

Local dev and stable SDK loads auto-detect credential files from the OS
