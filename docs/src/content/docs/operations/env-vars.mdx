...
| `API_OPERATOR_AUTOPILOT_ENABLED` | no | `false` | Master switch for autopilot auto-approval. |
| `API_OPERATOR_AUTOPILOT_CAPABILITIES` | no | empty | Allowlist of low-risk capabilities autopilot may auto-approve. |
| `API_OPERATOR_AUTOPILOT_MAX_STEPS` | no | `1` | Per-plan step limit for autopilot. |
| `API_OPERATOR_AUTOPILOT_WINDOW_SEC` | no | `3600` | Rolling auto-approval budget window in seconds. |
| `API_OPERATOR_AUTOPILOT_WINDOW_MAX` | no | `10` | Maximum auto-approvals per window. |
