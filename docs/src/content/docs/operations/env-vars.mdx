...
| `CARACAL_STS_URL` or `CARACAL_ZONE_URL` | STS URL for token exchange. |
| `CARACAL_GATEWAY_URL` | Gateway URL for SDK transports. |
| `CARACAL_ZONE_ID` | Zone identifier. |
| `CARACAL_APPLICATION_ID` | Confidential application ID. |
| `CARACAL_APP_CLIENT_SECRET_FILE` | Preferred workload secret file. |
| `CARACAL_RUN_CREDENTIALS_FILE` | JSON credential manifest. |
