...
description: Complete reference for zone, app, resource, provider, policy, policy-set, grant, session, and manifest commands — flags, API calls, and output formats.
pageType: reference
---

Admin commands manage control-plane entities and ecosystem contracts. Control-plane commands require `CARACAL_ADMIN_TOKEN`. Offline commands such as `manifest validate` do not call the API. Zone-scoped commands require a zone ID, resolved in order: `--zone` flag → `CARACAL_ZONE_ID` environment variable → `zone_id` in `caracal.toml`.

All commands accept `--json` to emit structured JSON and `--help` for command-specific help.

---

## Manifest commands — `caracal manifest`

`manifest validate` checks public extension manifests offline before a provider, exporter, policy pack, verifier, upstream, or connector is published.

```bash
caracal manifest validate --file gateway-upstream.json
caracal manifest validate --file provider-plugin.json --kind provider-credential-plugin --json
```
...
| Kind | Contract |
| --- | --- |
| `gateway-upstream` | Gateway-compatible upstream manifest |
| `provider-credential-plugin` | Provider credential plugin manifest |
| `audit-exporter` | Audit exporter manifest |
| `policy-pack` | Policy pack manifest |
| `resource-verifier` | Resource verifier manifest |
| `agent-connector` | Agent framework connector manifest |

The validator enforces interoperability-critical invariants: provider credentials must remain Gateway-only, resource verifiers must check target/scope/use and revocation anchors, connectors must label enforcement mode, and audit/exporter manifests must preserve replay and redaction requirements.
...
### `caracal policy template`

Use built-in policy templates as the default starting point.

```bash
caracal policy template list
caracal policy template get baseline-scopes
caracal policy template use baseline-scopes --name "Gateway Scope Policy"
```

| Form | Description |
| --- | --- |
| `template list` | Lists built-in templates such as `baseline-scopes`, delegation constraints, and step-up triggers |
| `template get <id>` | Prints the Rego content for inspection or editing |
| `template use <id>` | Creates a policy in the selected zone from the template |

`template use` accepts `--name`, `--description`, `--owner-type`, `--zone`, and `--json`.
...
Validate a policy-set version and an optional input fixture without changing active bindings.
...
The simulation endpoint validates rollout contract, schema compatibility, manifest integrity, public-application invariants, and optional input shape. It does not execute STS OPA evaluation.
