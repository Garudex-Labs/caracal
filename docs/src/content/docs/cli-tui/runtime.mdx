...
description: caracal run, credential read, and credential inspect — inject resource mandates, print mandates, and decode token claims for manual use.
pageType: reference
---

Runtime commands bind Caracal mandates to processes. `caracal run` and `caracal credential read` require `caracal.toml` and perform OAuth 2.0 token exchange directly with the STS. `caracal credential inspect` is offline and does not require `caracal.toml` or `CARACAL_ADMIN_TOKEN`.
