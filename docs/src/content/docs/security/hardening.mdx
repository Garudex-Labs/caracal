...
Zone signing keys are rotated by the Control-Plane API. The JWKS endpoint returns the two most recent keys; all verifying services pick up the new key within the 5-minute JWKS cache TTL. Rotation does not invalidate outstanding session mandates — existing mandates remain valid until their TTL expires.
...
Redis provides:
- JTI replay detection (resource mandate single-use enforcement)
...
Without Redis, or with `JTI_FAIL_OPEN=true`, resource mandate replay protection is disabled. This allows an attacker who captures a resource mandate to reuse it any number of times until it expires.
...
Admin tokens use a SHA-256 lookup fingerprint and an Argon2id verifier hash in the `admin_tokens` table. Token entropy remains required because bearer tokens grant direct administrative access when presented.
...
- Keep `ADMIN_AUTH_FAIL_LIMIT_PER_MIN` enabled and add a network-layer rate limit at the ingress or API gateway.
...
- `caracal_gateway_denials_signature_total` — possible key rotation issue or token forgery attempt.
- `caracal_gateway_denials_jti_replay_total` — possible token replay attack.
- `caracal_gateway_denials_revoked_total` — unusually high revocation activity.
- `caracal_gateway_sts_exchange_errors_total` — STS availability or policy evaluation issues.
