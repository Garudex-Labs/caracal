...
Caracal's security requirements are met under an explicit assurance case maintained in [`governance/THREAT_MODEL.md`](https://github.com/Garudex-Labs/caracal/blob/main/governance/THREAT_MODEL.md). It argues the claim from four pillars:

- **Threat model:** threats T1–T14 each carry an owner, a required mitigation, and a verification step.
- **Trust boundaries:** every boundary names what is untrusted and where mediation happens (see [Enforce Boundaries](/architecture/trust-boundaries/)).
...
| Area                     | Scope                                                                                               |
| ------------------------ | --------------------------------------------------------------------------------------------------- |
| API                      | Control-plane routes, admin auth, product state, step-up APIs, admin audit, AI Operator routes.     |
| Coordinator              | Session lifecycle, Delegation, invocation, TTL, retention, outbox.                                  |
...
| Web console and auth BFF | Sign-in, session cookies, origin enforcement, proxied admin calls, derived tokens.                  |
| Relay and streams        | Coordinator lifecycle relay, Redis Streams signatures, dedupe, pending-entry handling.              |
| Packages and infra       | SDKs, adapters, backends, Docker, Helm, migrations, installers, release artifacts.                  |
...
| Availability loss disables enforcement      | Bounded request bodies, timeouts, rate limits, health and readiness gates, restart policies; readiness fails when PostgreSQL, Redis, or STS is down.                                               |
...
| Admin foothold expands or hides its tracks  | Intent derived from authenticated identity instead of headers, authenticated metrics, fail-closed tamper-evident admin audit, read-only and zone-scoped admin tokens, row-level security backstop. |
| Browser session riding against the web tier | Same-origin serving, origin allowlist on writes, strict security headers, hardened cookies, allowlisted registration, derived read/write tokens, rate-limited credential endpoints.                |
| AI Operator misuse or prompt injection      | Plans require explicit approval, per-step authority re-checks, role-bounded reserved identities, opt-in zone reach, server-side plan secrets, capped model calls.                                  |

## Validation

Use targeted suites for the changed boundary: API/Coordinator TypeScript tests, auth and web BFF tests, STS/Gateway/Audit Go tests, Control/Engine tests, stream consumer tests, release checks, and broader `pnpm run ci` when shared packages, crypto, config, release, or infra change.

## Next Step

Use [Harden Security Posture](/security/hardening/) to translate the threat model into deployment checks.
