...
- **Pinning.** GitHub Actions are pinned to commit SHAs, container base images are pinned to digests, and language dependencies are resolved from committed lockfiles (`pnpm-lock.yaml`, `go.sum`, Python `*.lock`). Reviewer guidance for dependency changes is in the repository [contributing guide](https://github.com/Garudex-Labs/caracal/blob/main/CONTRIBUTING.md).
