...
Redis stream groups are provisioned by the Redis bootstrap script before Redis reports ready, and database migrations run through a dedicated one-shot migration job before application traffic starts. The Coordinator container also starts its Go relay sidecar. All ports are bound to localhost in development; production deployments place services behind network policies appropriate to the deployment target.
...
| `GET /metrics.json` | Compatibility JSON metrics snapshot |
...
**Connections:** Postgres (SELECT: zones, resources, providers, gateway_resource_bindings), Redis (JTI deduplication, stream signature verification, `caracal.sessions.revoke` via `gateway-revocation`), STS (JWKS fetch, resource mandate exchange).
...
The Redis bootstrap path runs `provision-streams.sh` once per Redis container start. It creates all nine Redis consumer groups with `XGROUP CREATE … MKSTREAM`, ignores existing groups, and writes a readiness marker consumed by the Redis healthcheck.

The database migration path runs `caracal-migrate` as a one-shot Compose service or Helm migration Job. It records applied migration versions in `schema_migrations` and serializes concurrent runners with a Postgres advisory lock.
...
```
postgres ───────────────► dbMigrate (exits)
   │                          │
   └──────────────┬───────────┘
                  ▼
redis bootstrap ─► redis healthy
                  │
      ┌───────────┼───────────┐
      ▼           ▼           ▼
     STS        Audit        API
...
Each service waits for a `healthy` signal from its dependencies before starting. Gateway waits for both API and STS healthy. Coordinator waits for STS healthy. API, STS, Audit, and Coordinator wait for database migrations and Redis readiness.
...
| **Audit**       | ✓ | ✓ | — | — | — | — | — |
| **Coordinator** | ✓ | ✓ | ✓ | — | — | — | — |
| **terminal management interface/SDK**     | — | — | ✓ | ✓ | ✓ | — | ✓ |
