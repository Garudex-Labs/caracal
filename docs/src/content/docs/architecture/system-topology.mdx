...
Caracal runs five core application services plus Postgres and Redis. One-shot initialization containers provision Redis streams and credentials, then exit. The Coordinator container also starts its Go relay sidecar. All ports are bound to localhost in development; production deployments place services behind network policies appropriate to the deployment target.
