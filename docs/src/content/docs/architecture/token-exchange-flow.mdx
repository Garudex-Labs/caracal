...
| `application_id` and `client_secret`                   | Authenticates the calling application.                                    |
...
RFC 8693 `actor_token`, `client_assertion`, and `client_assertion_type` are not supported; STS rejects requests that carry them. The calling application's authenticated identity is published to policy input as `input.context.actor_claims.caracal_client_id`.
...
Gateway exchanges with STS using a request signature, timestamp, and independent random nonce over the exact form body bytes STS consumes. Query parameters and duplicate singleton fields are rejected before the signed request is trusted. The request id, W3C trace id, mandate `jti`, and HMAC nonce remain separate correlation and replay domains.
