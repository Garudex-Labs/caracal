...
| `subject_token` | Session mandate JWT (the caller's proof of identity) |
...
[3]  Validate subject_token (if present)
       ...
       → audience == IssuerURL (rejects resource mandates as subject)
       → zone_id claim matches request zone_id
       → use claim == "session" (RFC 8693 subject-confusion mitigation)
       → HTTP 401 on failure

[4]  Validate session mandate session
...
[9]  Validate delegation references (if delegation_edge_id present)
       ...
       → full delegation path validated (max hops, scope containment, TTL constraint)
       → HTTP 403 on failure
```
...
[12] Validate ttl_seconds (if present)
       → session mandates: max 3600 seconds (60 minutes)
       → resource mandates: max 900 seconds (15 minutes)
...
| `aud` | []string | Issuer URL for session; resource identifiers for resource |
