...
┌───────────────────────▼─────────────────────────────┐
│  TRUST BOUNDARY: Client credentials + session mandate │
│                                                     │
│  STS                                                │
...
- The STS trusts only ES256-signed session mandates with `use = "session"` — unsigned input cannot produce a mandate
...
Revocation propagates from the Coordinator through the Redis stream to the STS and Gateway consumers in near real time. However, a resource mandate already issued before revocation completes will have a valid signature and unexpired `exp` for up to 15 minutes. During that window:
...
The 15-minute resource TTL is the intentional upper bound on revocation lag. Session mandates (60-minute TTL) cannot be presented directly to upstreams — they must be exchanged for a resource mandate at the STS, where the session status is checked before issuance.
