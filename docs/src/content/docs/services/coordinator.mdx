...
**`agent_sessions`** — `id`, `zone_id`, `application_id`, `subject_session_id` (parent user session), `parent_id` (self-referential FK), `status` (`active`|`suspended`|`terminated`), `depth`, `capabilities` (array), `ttl_seconds`, `spawned_at`, `suspended_at`, `terminated_at`, timestamps. Indexed on `(zone_id, application_id)` and `(zone_id, status)`.
