...
description: Understand when to use the API service and how its storage, outbox, and authentication failures appear to operators.
pageType: reference
---
...
Use [Use the Admin API](/api/control-plane/) for route and auth reference rather than treating this service page as an endpoint catalog. Service environment variables are cataloged in [Configure Service Environment](/operations/env-vars/).
...
[Coordinate Session State](/services/coordinator/) or [Manage Product Objects](/runtime-console/admin/).
