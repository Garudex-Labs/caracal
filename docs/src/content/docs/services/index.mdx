...
Caracal is composed of five core application services. Three are written in Go (`sts`, `gateway`, `audit`); two are written in TypeScript (`api`, `coordinator`). The stack also includes a coordinator relay sidecar and an optional control API surface. All core services share a PostgreSQL database and a Redis instance but own distinct schema responsibilities and domain boundaries.
...
| --- | --- | --- | --- |
| Control API | TypeScript | 8087 | Programmatic CLI/TUI command invocation; disabled unless the control profile is enabled |
