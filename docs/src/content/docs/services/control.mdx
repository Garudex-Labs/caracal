...
1. Create a control key once in Console from the Control menu. Console returns a
   `client_id` and `client_secret` and records the `control:<command>:<verb>`
   scopes the key may request, along with an optional maximum token TTL. The key
   is bound to the zone where it was created; its secret is held sealed in the
   Secret Store and stays retrievable from the Applications page through an
   audited reveal.
...
STS resolves the bound zone from the authenticated control key; automation must
not provide a separate zone id. Each token is short-lived, replay-protected,
rate-limited, and audited, so
  ...
  "objects": [
    {
      ...
      "spec": {
        ...
        "kind": "oidc",
        "config": { "issuer": "https://login.hooli.example" }
      }
    },
...
```json
{
  ...
  "result": {
    ...
    "prune": false,
    "zoneId": "<zone>",
    "outcomes": [{ "kind": "resource", "identity": "resource://pipernet", "action": "create", "applied": true, "id": "..." }],
...
Authority records and governed Sessions use distinct canonical command nouns:

| Command            | Purpose                          |
| ------------------ | -------------------------------- |
| `authority-record` | List STS Authority records.      |
| `session`          | Manage governed Sessions.        |

The corresponding Admin SDK namespaces are `authorityRecords` and `sessions` in TypeScript, `authority_records` and `sessions` in Python, and `AuthorityRecords` and `Sessions` in Go.
...
| `control_disabled`  | `503` | The runtime gate is off; control is not accepting invokes.                     |
| `unauthorized`      | `401` | The token is missing, malformed, or failed verification.                       |
| `token_replay`      | `401` | The token was already used; mint a fresh token per invoke.                     |
| `rate_limited`      | `429` | The subject or source address exceeded the invoke rate limit.                  |
| `audit_unavailable` | `500` | The invoke could not be durably audited, so it was not executed.               |

The successful shape is unchanged: `{ "ok": true, "result": ... }`. Correlate any
response with the `x-request-id` response header in the web console **Audit** decision trace.
...
- Each command is a group shown as `control:<command>:*` (for example
  `control:session:*`). Toggling a group grants every action under that command.
...
Control keys are also shielded from the surface they authorize. The `app`
command's `patch`, `rotate-secret`, and `delete` verbs, and declarative
`ensure`/`state` reconciliation, refuse to touch an application that is a
control key, so one automation key can never mutate, rotate, or delete another.
The `caracal-control` token audience is mintable only by a control key
exchanging its own client credentials; no grant or delegation can produce a
control-audience token for any other identity. Manage the keys themselves only
in Console.

## Safety Behavior

Every failure returns the structured envelope described in [Structured Errors](#structured-errors), including the checks that run before dispatch: `control_disabled` (`503`) when the gate is off, `unauthorized` (`401`) for authentication failures, `token_replay` (`401`) when a token is presented twice, `rate_limited` (`429`) for subject or source throttling, and `audit_unavailable` (`500`) when the invoke cannot be durably audited. Request bodies are capped at 256 KB and rejected with `413` beyond that.
...
- [Use the Admin API](/api/control-plane/)
