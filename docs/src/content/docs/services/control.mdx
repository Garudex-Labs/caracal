...
1. Create a control key once in Console from the Control menu. Console returns a
   ...
   scopes the key may request, along with an optional maximum token TTL.
...
Each token is short-lived, replay-protected, rate-limited, and audited, so
...
- [Control bootstrap example](https://github.com/Garudex-Labs/caracal/tree/main/examples/controlBootstrap)
