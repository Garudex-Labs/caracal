...
A provider answers one question: after Caracal approves a call, what credential does the upstream target receive? Every Gateway-routed resource binds exactly one provider. One provider can serve many resources.
...
| OAuth 2.0 authorization code | A per-user delegated token. | The upstream needs user consent. |
| OAuth 2.0 client credentials | A service-to-service token. | The upstream uses machine-to-machine OAuth. |
...
Gateway strips the caller's authorization and forwards the provider credential instead. For OAuth modes, STS obtains and refreshes the upstream tokens; delegated user consent is stored as a provider grant.
...
Read [Policies and Policy Sets](/concepts/policy/) to understand how requests against a resource are allowed, denied, or held for step-up.
