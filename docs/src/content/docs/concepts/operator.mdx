...
Console-managed endpoint metadata is shared through PostgreSQL. After a successful create, metadata or model update, enable or disable, or delete, the API replica that handled the change advances a shared Redis registry version. Every API replica checks that version every five seconds and rebuilds its in-process endpoint registry when it changes, so a healthy multi-replica deployment converges within five seconds without repeating control-plane provisioning. Key rotation updates the shared sealed credential directly and does not require a metadata refresh. `GET /v1/operator/ai/status` exposes each replica's `registry_sync` state, acknowledged and observed versions, pending-publish flag, refresh interval, timestamps, and bounded error class. A `publish_failed`, `version_read_failed`, or `refresh_failed` state means convergence is not currently assured; the replica logs the failure and retries rather than reporting success silently.
