...
In agent mode you can engage **autopilot**, which lets Caracal auto-satisfy approval for eligible plans in the conversation. Engaging it is an explicit opt-in to acting without a human in the loop; it is off by default and per conversation, and a platform-level master switch must also be on. Auto-approval never widens authority - the governed execute path still enforces the capability allowlist, the least-privilege executor token, and zone isolation on every apply. Autopilot requires the independent guardian review to complete successfully, but the review's alignment verdict and findings do not grant permission; a failed review can only leave the plan waiting for a human. Autopilot also defers while a plan still needs credentials from the console's secure prompt and stops when a preview shows the plan cannot apply. A deployment can bound an engaged conversation with a **write budget**: once the cumulative auto-approved write operations would exceed it, autopilot pauses, records the pause in the conversation ledger, and the plan waits for explicit human approval. Every automatic approval records its write count and cumulative write total even when the deployment does not enforce a budget. The master switch is a single kill switch that stops all auto-approval on the next turn.
...
The settings page can edit model endpoint metadata, rotate the sealed key, delete a model endpoint, and run a real connectivity check. Multiple model endpoints and models participate in failover. Each logical completion gives its current model endpoint one SDK-managed retry for transient failures, including rate limits and server errors; after that retry is exhausted, it moves to the next endpoint. A terminal HTTP rejection such as 400, 401, 404, or 413 stops immediately and reports the rejecting endpoint and status instead of replaying the same invalid request across the failover chain. The endpoint timeout bounds the retry as well as the initial request. Direct API-process `API_OPERATOR_AI_*` configuration also remains implemented, but the packaged-runtime workflow is console management; see [Configure Service Environment](/operations/env-vars/#api-operator-and-control) for that narrower path.

Console-managed endpoint metadata is shared through PostgreSQL. After a successful create, metadata or model update, enable or disable, or delete, the API replica that handled the change advances a shared Redis registry version. Every API replica checks that version every five seconds and rebuilds its in-process endpoint registry when it changes, so a healthy multi-replica deployment normally converges in about five seconds plus the time required to read and validate the refreshed registry. Key rotation updates the shared sealed credential directly and does not require a metadata refresh. `GET /v1/operator/ai/status` exposes each replica's `registry_sync` state (`unknown`, `healthy`, or `error`), `local_version`, `observed_version`, pending-publish flag, refresh interval, timestamps, and `last_error_class`. When `state` is `error`, a `last_error_class` of `publish_failed`, `version_read_failed`, or `refresh_failed` means convergence is not currently assured; the replica logs the failure and retries rather than reporting success silently. The last error class remains available after recovery for diagnosis even when the state returns to `healthy`.
