...
- **TTL**: 15 minutes
- **Purpose**: authorizes one Gateway-proxied request, or a direct connector call when that connector owns replay and revocation checks
...
| `root_sid` | string | Root authority session ID inherited by derived per-call mandates |
...
The `root_sid` claim keeps the authority graph simple: ambient mandates set `root_sid` to their own `sid`, and per-call mandates inherit the same value. Revoking the root authority session lets the Gateway reject every derived mandate even when each per-call token has its own `sid`.
...
The term "per-call" describes the authority boundary, not a guarantee that every SDK method invocation always calls STS. The safe behavior depends on where enforcement happens:

| Path | Exchange behavior | Replay behavior |
| ---- | ----------------- | --------------- |
| Gateway-proxied HTTP | Gateway performs an STS exchange for each inbound request and forwards only after replay and revocation checks pass | Gateway rejects a repeated per-call JTI before the upstream sees it |
| SDK-managed direct call | SDK may cache a token for the same subject session, agent session, delegation edge, resource, and scope context until the token expires | The connector or resource-side verifier must enforce replay/revocation if the Gateway is not in the path |
| Ambient-to-STS exchange | SDK re-presents the ambient token to STS to mint narrower per-call authority | Ambient tokens are not accepted by Gateway as upstream authorization |
...
The ambient token is the session credential. It is exchanged at session start and stored in the agent's execution environment (injected by `caracal run` as `CARACAL_SUBJECT_TOKEN`). The SDK exchanges this ambient token for per-call tokens as needed for outbound calls. Per-call STS sessions are linked to their parent session, and mandates carry `root_sid` for deterministic descendant revocation. Delegated per-call mandates are capped by the shortest effective authority bound: STS TTL limit, delegation edge expiry, and any typed `ttl_seconds` constraint on the edge.
...
A mandate is not a session cookie, an API key, or a long-lived secret. It expires within minutes, is bound to a specific zone, and (for per-call tokens) can be used exactly once via the Gateway. An agent that loses its mandate requests a new one only if its STS session, agent session, delegation edge, and policy still allow it.
