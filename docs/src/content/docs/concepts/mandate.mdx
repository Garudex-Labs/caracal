...
Resource servers should always verify a mandate at request time. Verification includes signature and claim checks plus revocation checks for the Authority record ID (`sid`), Root authority record ID (`root_sid`), Session ID (`agent_session_id`), and Delegation ID (`delegation_edge_id`) when those raw JWT claims are present.
