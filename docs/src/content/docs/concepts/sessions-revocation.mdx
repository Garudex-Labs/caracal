...
| Authority record      | One STS exchange record, stored internally in `sessions`. Its Authority record ID is the raw JWT `sid`.                                                         |
| Root authority record | Root of an STS exchange ancestry chain. Its ID is the raw JWT `root_sid`.                                                                                       |
| Session               | Governed Coordinator execution, stored internally in `agent_sessions`. A delegated Session is a Session with an inbound Delegation, not a separate object type. |
...
A Session may attach that record through the SDK compatibility field `subjectSessionId`, `subject_session_id`, or `SubjectSessionID`. This Subject authority record ID supports attribution and lifecycle linkage. It does **not** by itself cause later resource mandates minted for the Session to carry the federated user's `sub`; callers that need the user's own mandate must present the federated mandate on the supported exchange or approval path.

## Revocation Anchors

Resource servers check every relevant anchor:

- Authority record ID, carried in the raw JWT `sid` claim;
- Root authority record ID, carried in the raw JWT `root_sid` claim;
- Session ID, carried in the raw JWT `agent_session_id` claim;
- Delegation ID, carried in the raw JWT `delegation_edge_id` claim.
