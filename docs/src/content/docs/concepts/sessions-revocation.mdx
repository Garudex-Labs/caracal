...
Caracal does not generate subject identities, and the *Subjects* page is not a login surface. The `sub` on a subject session is taken verbatim from the subject token the application exchanged: the application's own identity system (Auth0, Keycloak, Better Auth, a custom JWT issuer) authenticates the user or service, and the exchange carries that identity into Caracal. To federate end users directly, a zone registers the identity system as a **subject issuer** (issuer, JWKS URL, audience); the application then exchanges each user's identity token for a user subject session. Caracal treats the identifier as opaque - it does not need to know whether it names a user id, an employee id, or a customer id - and requires only that it is unique and stable for the same identity over time, so revocation and audit history stay attributable. Everything below the subject session is keyed by that `sub`: derived sessions, delegations, provider connections, approvals, and every audit event.
