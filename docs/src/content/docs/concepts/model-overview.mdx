...
| Subject     | The identity work is done for: the Application itself by default, or a Federated user from your identity provider. |
...
One application backs many Sessions. A long-running service registers **one managed application**, then starts, delegates, and fans out as many Sessions as it needs under that single credential. You do not create an application per AI agent; per-execution attribution comes from the Session, not a new application. See [Should I create one application per agent?](/reference/faq/#faq-006).
