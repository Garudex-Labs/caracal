...
## Scope resolution and identity binding

Delegation edges carry provider-agnostic authority:

- The edge stores scope strings and an optional resource ID (for example, `resource://payments`).
- The mandate is the signed capability. The STS issues a Caracal JWT with the OAuth-style `scope` claim plus `target` resource identifiers, and it returns an `upstreams` map resolved from the zone's resource definitions.
- Rotating providers or upstream URLs does not require reissuing the delegation edge.

Delegation identity is bound to agent sessions, not to a runtime key:

- The edge is keyed to `source_session_id` and `target_session_id`.
- The STS only honors the edge when the requesting session ID matches the stored `target_session_id`.
- If an agent rotates its local credentials but keeps the same Coordinator session, the edge remains valid.
- If the session is closed and reopened (new session ID), a new delegation edge is required.
...
Downstream delegations are therefore valid only while their upstream edge and session remain active. When a parent edge is revoked, all descendant edges are revoked and their sessions terminated, so mandates tied to those sessions are rejected even if their TTL has not expired.
