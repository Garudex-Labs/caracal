...
An edge bounds **the token issued for that edge** and **any further edge chained from it**. It does **not** lower the application's authority ceiling. This distinction is the whole security model, so make it explicit:
...
- When a child is created with **inherit** (the default), **no edge is recorded**. The child authenticates as the application and re-derives the application's authority, bounded only by policy — it is **not** bounded by an ancestor's narrowing.

Worked example: A spawns B with `grant=narrow([tickets:read])`, then B spawns C.

- If B spawns C with **inherit**, C presents no edge and receives the **application's** authority (subject to policy) — *not* B's `tickets:read` slice. B did not "grant" anything; C simply runs as the app.
...
The consequence: the **application plus policy** is the hard, server-enforced boundary every session is contained by. Delegation edges are a **least-privilege tool within that boundary** — to bound an entire subtree below the application, narrow at **every hop** (or move the subtree under a different application via `delegate()`, which is a real cross-application trust boundary requiring receiver consent). All sessions of one application share a credential and a trust domain by design; do not rely on a single narrowed intermediary to contain its descendants.
