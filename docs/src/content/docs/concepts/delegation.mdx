...
- To carry authority **across applications**, when a receiving Session belongs to a different application that must consent.
...
Delegation is authored **at runtime through the SDK**. There is no Console form for creating one. Call `session(authority=…)` or `delegate(to=…)`; Coordinator records and enforces the graph edge. Audit records the Delegation ID, hop chain, and scopes. The raw audit and Coordinator protocols name the ID `delegation_edge_id`.
