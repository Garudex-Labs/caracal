...
The platform decision contract owns the `result` object in `package caracal.authz`. It is deny by default and only allows an exchange that matches one of its vetted rules: a bootstrap mandate, a delegated mint narrowed to the Delegation, or a mandate presented at the Gateway. You never write `result`; you supply the data the contract reads.
...
| `confinement` | `[{label_prefix, scopes}]`                           | Caps every Session whose principal carries a matching label prefix to a fixed scope set. Optional; omitting it confines nothing. |
...
| `input.delegation_edge`               | Present when authority was delegated from another Session.  |
...
| `input.principal.lifecycle`           | string   | `task` or `service` for Sessions.                                                                                                                                                  |
| `input.principal.labels`              | string[] | Role descriptors set at `session(labels=[...])`; drive least-privilege role checks.                                                                                                |
| `input.principal.agent_session_id`    | string   | Protocol field carrying the Session ID that initiated the exchange, when present.                                                                                                  |
...
| `input.context.actor_claims`          | object   | Verified claims of the optional actor credential; recorded for audit, not read by any allow rule.                                                                                  |
| `input.context.subject_claims`        | object   | Verified claims of the presented subject token; the contract uses them for structural binding checks, never for scope authority.                                                   |
...
When `input.delegation_edge` is present it carries: `id`, `source_session_id`, `target_session_id`, `issuer_application_id`, `receiver_application_id`, `resource_id`, `scopes` (the narrowed grant), `edge_version`, `path` (the full delegation chain), `graph_epoch`, and `constraints_json`. The platform contract reads the edge identity and scopes for structural and scope-subset checks; it does not read `constraints_json`. Native STS validation enforces typed resource, expiry, TTL, hop, and distinct-scope budget limits before policy evaluation.
...
Approval gates are declared as data: `risk` names a tier for each sensitive scope and `approval_tiers` declares which tiers require a human decision. When a mint requests a gated scope, the STS creates a durable hold and releases the mandate only after an authorized approver decides it. See [Step-Up Challenges](/concepts/step-up/).
...
Read [Step-Up Challenges](/concepts/step-up/) to understand how policies hold sensitive exchanges for a human decision.
