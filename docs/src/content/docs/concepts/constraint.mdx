...
description: Resource, scope, lifetime, and hop limits plus audit metadata carried by a Delegation.
sidebar:
  order: 11
---

Read [Session Delegation](/concepts/delegation/) first. Constraints state how far, how long, and for what a Delegation may be used. Caracal validates them when the Delegation is created and again before issuing a Mandate through it.
...
Delegation constraints bound authority, lifetime, and propagation. They are not quotas. Use a domain-specific store when calls, work, or cost must be consumed atomically. Approval metadata and broad-reason fields are audit annotations; they do not authorize the Delegation.
