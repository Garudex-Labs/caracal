...
The Runtime and Terminal provide two views:

**Live tail** (the terminal management interface Audit view): streams events from the Redis stream in near real-time. Filter by decision (`allow` / `deny`), pause and resume the stream.

**Request trace** (the terminal management interface audit detail view): retrieves all audit events for a specific `request_id`, showing the full OPA evaluation — which resources were requested, which were allowed, which were denied, and which policy versions determined each decision.
