...
- A `task` parent cannot start a `service` child. The protocol error remains `task_agent_cannot_spawn_service`. A `service` parent may start either lifecycle.
- A DCR Session is a **leaf**: it cannot parent or be a child Session. The protocol errors remain `dcr_application_cannot_spawn` and `dcr_application_cannot_be_child`.
...
Every node in that tree is a `task` Session under one managed application, distinguished by its Session ID, labels, and delegation context. On the Coordinator wire, Session ID is named `agent_session_id`. "Disposable worker" is expressed by TTL and narrowed authority, not a separate lifecycle.
...
Policies receive protocol fields `input.principal.registration_method`, `input.principal.agent_session_id`, `input.principal.lifecycle`, and `input.principal.labels`. Here `agent_session_id` is the Session ID. Audit metadata records the application, registration method, Session ID, parent and Delegation context, requested scopes, and resource.

## Telling Sessions Apart

Every Session has one canonical Session ID. The Coordinator protocol calls it `agent_session_id`; SDK context exposes it as `sessionId`, `session_id`, or `SessionID`. It is returned when `session()` or `startSession()` starts the Session and is stamped onto its token exchanges and audit events.
...
The Admin API audit protocol filters by `agent_session_id` for one Session or `label` for a role across many Sessions.
