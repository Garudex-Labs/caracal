...
| --- | --- | --- |
| Managed | One durable service, orchestrator, Gateway, or agent runtime — including the runtime that spawns and fans out child agents. | Created in the Console (or Admin API) and reused across many agent sessions for the same workload. Spawned children run as `agent` sessions under this same application. |
