...
`v0.2.0` is the first versioned documentation release. Until that stable release is cut, continue editing `docs/src/content/docs/`; do not create `docs/src/content/docs/v0.2/` manually. The stable release command snapshots that exact source as `v0.2`, marks it current, and enables the version selector.
