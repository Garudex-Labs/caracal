...
Every Caracal release artifact shares one Semantic Version from `release.config.json` `product.version`. The current source state is `0.2.0-rc.1`; successive release candidates increment `-rc.N`. `scripts/release.sh stamp` propagates the configured version to owned artifact metadata.
...
| RC publish     | `scripts/release.sh rc publish`                                                |
| Stable prepare | Set `product.version` to `X.Y.Z`, then `scripts/release.sh stable prepare`   |
...
Run `pnpm release:plan` and `pnpm release:stamp:check` before publication. Preparation generates a source-neutral release plan and the docs release record. The publish command atomically creates the root tag and every nested Go module tag at the same commit, then explicitly dispatches CI from that tag. CI rejects occupied registry versions and finalizes the customer manifest with the full tag commit. Do not author release evidence as an independent source of truth.
...
The root release workflow publishes npm and PyPI packages only from a complete, verified tag set. Every release publishes all publishable packages at the shared version. Existing versions are errors, and Python distributions publish registry attestations. A partial registry publication consumes that version and requires a roll-forward release rather than an in-place retry. Manual package workflows are dry-run only, and the local publisher supports TestPyPI only.
