...
`v0.2.0` is the first versioned documentation release. Until that stable release is cut, continue editing `docs/src/content/docs/`; the site serves those pages from shareable `/v0.2/` routes, but no `v0.2` source snapshot exists yet. Do not create `docs/src/content/docs/v0.2/` manually. The stable release command snapshots that exact source as `v0.2` and marks it current without changing its public routes.
