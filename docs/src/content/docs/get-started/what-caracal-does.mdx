...
| Agent run | One tracked execution of that workload. |
...
Delegation lets one agent run give another run narrower permission. The invariant is:
...
Use the Console **audit** view to search events and **explain** to inspect a request ID. The Console is the product surface for audit inspection; the runtime shell does not expose product-management audit commands.
