...
This page covers installing the `caracal` CLI, the `caracal-terminal` terminal UI, and the SDK packages for TypeScript, Python, and Go.

## Install the CLI or Terminal

The CLI and Terminal are distributed as separate self-contained binaries for Linux, macOS, and Windows. No runtime is required. Run both installers to install both tools, or run only the Terminal installer when you want `caracal-terminal` without `caracal-terminal`.
...
```sh
caracal --version
caracal-terminal --version   # CLI installer
...
```powershell
# CLI
irm https://raw.githubusercontent.com/Garudex-Labs/caracal/main/install-cli.ps1 | iex
