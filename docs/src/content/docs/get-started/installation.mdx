---
title: Installation
description: Install the terminal management interface, run the OSS stack with Docker, or pull SDK packages.
pageType: workflow
---

This page covers installing the `caracal` terminal management interface, the `caracal-terminal` terminal UI, and the SDK packages for TypeScript, Python, and Go.

## Install the runtime shell or terminal

The Runtime and Terminal are distributed as separate self-contained binaries for Linux, macOS, and Windows. No runtime is required. Run both installers to install both tools, or run only the Terminal installer when you want `caracal-terminal` without `caracal-terminal`.
...
2. Downloads the matching `caracal` shell plus either `caracal-terminal` or `caracal-terminal` from the latest GitHub release.
...
```sh
caracal --version
caracal-terminal --version   # terminal management interface installer
caracal-terminal --version   # Terminal installer
```
...
```powershell
# terminal management interface
irm https://raw.githubusercontent.com/Garudex-Labs/caracal/main/install-terminal.ps1 | iex
...
# Output: apps/runtime/dist/caracal-terminal-<os>-<arch>
```
...
Once the terminal management interface is installed, go to [Quickstart](./quickstart/) to bring up the full stack.
