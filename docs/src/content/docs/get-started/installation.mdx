---
title: Installation
description: Install the terminal interface, run the OSS stack with Docker, or pull SDK packages.
pageType: workflow
---

This page covers installing the `caracal` terminal interface, the `caracal-terminal` terminal UI, and the SDK packages for TypeScript, Python, and Go.

## Install the runtime shell or terminal

The runtime shell and terminal are distributed as separate self-contained binaries for Linux, macOS, and Windows. No runtime is required. Run both installers to install both tools, or run only the Terminal installer when you want `caracal-terminal` without `caracal-terminal`.
...
```sh
caracal --version
caracal-terminal --version   # terminal interface installer
...
Once the terminal interface is installed, go to [Quickstart](./quickstart/) to bring up the full stack.
