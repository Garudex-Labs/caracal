...
| Agent session | Execution session | Coordinator, authority sessions, audit. |
| Mandate | Caracal access token | STS, Gateway, SDK transport. |
| Resource | Resource identifier | Gateway routing, policy. |
...
An agent app is the workload identity. It authenticates to Caracal with an app secret, starts agent sessions, requests scoped mandates, and appears in audit records. The app secret is not a provider credential.

### Resource

A resource is the protected target. It includes scopes, an upstream URL, and a Gateway application binding. Stable scope names such as `calendar:read` or `payments:submit` are easier to govern than prose-like scopes.

Gateway resources can also bind to a provider when the upstream API requires provider-native authentication. Caracal-aware resources use the same Gateway path with the Caracal mandate forwarded to the upstream verifier; provider-native credentials stay inside the trusted Gateway/STS boundary.
