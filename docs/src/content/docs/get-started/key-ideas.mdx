...
A resource is the protected target. In the default path it includes scopes and, when Gateway routing is needed, an upstream URL prefix. Stable scope names such as `calendar:read` or `payments:submit` are easier to govern than prose-like scopes.
...
| Provider credential | Gateway or a contract-complete adapter. | Authenticates to the upstream provider. |

Provider-native credentials should not be copied into agent code. The Gateway-mediated path keeps them behind the enforcement boundary.
