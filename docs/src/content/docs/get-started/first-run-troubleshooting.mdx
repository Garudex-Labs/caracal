...
| Symptom | Check |
| --- | --- |
| Sign-up is rejected with `registration_not_permitted` | The packaged console closes registration by default. Run `caracal allowlist add <email>` on the runtime host to permit the email, and make sure a sign-in method is configured in `$CARACAL_HOME/caracal.env` - see [Sign In and Create Your First Zone](/get-started/first-protected-call/#sign-in-and-create-your-first-zone). |
| Sign-in fails with `account_locked` or `sign_in_not_permitted` | The email's allowlist entry is locked or was removed. On the runtime host, run `caracal allowlist list` to inspect entries, then `caracal allowlist unlock <email>` or `caracal allowlist add <email>` to restore access. |
...
| Symptom | Check |
| --- | --- |
| `workload identity not found` | Export `CARACAL_WORKLOAD_ID` and provide a workload secret source. |
...
The workload secret is shown once when the workload is created. If you lose it, rotate the secret from the web console **Launcher** page and update your secret file or environment with the new value.
