...
| Symptom | Check |
| --- | --- |
| Sign-up is rejected with `registration_not_permitted` | The packaged console closes registration by default. Run `caracal invite <email>` on the runtime host for a one-time bootstrap code, or configure a sign-in method in `$CARACAL_HOME/caracal.env` - see [Sign In and Create Your First Zone](/get-started/first-protected-call/#sign-in-and-create-your-first-zone). |
