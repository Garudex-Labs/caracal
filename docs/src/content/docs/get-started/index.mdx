...
Caracal gives each part of that flow a name. Six terms cover everything in Get Started. Each one maps to something you already know:
...
Caracal is not an LLM framework, prompt router, agent scheduler, static config store, general-purpose API gateway, or identity provider. If your only need is human users behind a normal login, use an identity provider instead.

If you already use adjacent tools, here is where Caracal differs:
...
| An identity provider | It authenticates people. Caracal governs what programs do. Keep your IdP for logins; you can federate it into Caracal later for attribution. |
...
flowchart LR
  ...
  Stack[Start the local stack]
  Setup[Create your first protected setup]
  Call[Make a protected call]
  Audit[Read the audit trail]

  Install --> Stack --> Setup --> Call --> Audit
```

1. [Install Caracal](./install-caracal/) - install one `caracal` executable and start the local stack with Docker.
2. [First Protected Call](./first-protected-call/) - give an AI agent scoped authority to call an LLM provider through the browser console, then send one request through the full path.
3. [Add SDK to Your App](./add-sdk-to-your-app/) - make the same protected call from TypeScript, Python, or Go.
