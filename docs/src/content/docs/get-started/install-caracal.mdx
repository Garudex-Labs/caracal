...
The install script resolves the latest release, downloads the right archive for your platform, verifies its checksum, and places `caracal` on your `PATH`. Pin a version by setting `CARACAL_VERSION` first.
...
```sh
tag=$(curl -fsSL https://api.github.com/repos/Garudex-Labs/caracal/releases/latest | grep -o '"tag_name": *"[^"]*"' | cut -d'"' -f4)
curl -fsSLO "https://github.com/Garudex-Labs/caracal/releases/download/${tag}/caracal-runtime-linux-amd64-${tag}.tar.gz"
...
tar -xzf "caracal-runtime-linux-amd64-${tag}.tar.gz"
...
```powershell
$tag = (Invoke-RestMethod https://api.github.com/repos/Garudex-Labs/caracal/releases/latest).tag_name
...
`caracal status --ready` exits successfully, [http://localhost:3001](http://localhost:3001) responds, your email is allowlisted, and one sign-in method is configured. Readiness proves the local services are up; nothing is protected yet - that is the next page.
