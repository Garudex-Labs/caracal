...
description: Make the protected call from First Protected Call again, this time from your own TypeScript, Python, or Go code.
pageType: workflow
---

import { Tabs, TabItem } from '@astrojs/starlight/components'

In [First Protected Call](/get-started/first-protected-call/) you proved the enforcement path with `caracal run` and `curl`. This page makes the same protected call from your own code. Pick one language; the three examples are equivalent.

The difference from the previous page: your program now talks to Caracal itself. Instead of `caracal run` fetching a token before launch, the SDK requests authority while your program runs - asking for exactly the permission each call needs, when it needs it. That is the shape real applications and agents use.
...
Because your program runs directly (not under `caracal run`), the SDK needs to know who it is and where Caracal is. It reads both from a small TOML file called a profile, or from `CARACAL_*` environment variables. You write this file yourself, using the values from First Protected Call - the zone ID, the application ID, and the application client secret you copied during guided setup:
...
On the local stack the SDK's built-in defaults already point at the right service URLs, so this is the whole file. For cloud or custom deployments you would add `sts_url`, `coordinator_url`, and `gateway_url`; the full schema is in [Configuration File](/runtime-console/config-file/).
...
Point the examples at the profile and choose what they call:
...
Run the example in your selected language. It should print the provider's JSON model list - the same response your `curl` call returned - and create matching authorization and Gateway action-result events in **Audit**. A constructor or profile error means no request reached Caracal; a policy denial appears in Audit with a request ID.

## What Each Call Does

All three examples follow the same four calls; only the naming convention changes per language:

- `new Caracal()` / `Caracal()` / `New()` loads the profile and prepares the client. It fails immediately if the configuration is missing or unreadable.
- `applicationTransport()` / `application_transport()` / `ApplicationTransport()` returns an HTTP transport pinned to one resource. Under the hood it does what `caracal run` did before launch, but per call: it asks Caracal for authority with exactly the scopes you list and gets a fresh, short-lived, replay-protected mandate for each request. To do this it opens a **session** - Caracal's record of one governed run of your program - and narrows the requested scope through a **delegation**, a hand-off that can only ever shrink permissions, never grow them. The SDK manages both for you; you will meet them properly in [Concepts](/concepts/).
- `gatewayRequest()` / `GatewayRequest()` builds the Gateway URL and the `X-Caracal-Resource` routing header - the same two things you assembled by hand in the `curl` call.
- `close()` / `aclose()` / `Close()` ends SDK-owned sessions and background work cleanly.

`applicationTransport()` is the smallest application-owned call path and the right starting point. Agent frameworks that already run under delegated authority use `session()` with `fetch(..., { scopes })` instead; read your language's [SDK guide](/guides/) before switching paths.

If policy denies the exchange, the SDK surfaces the error from Caracal's token service. Open **Audit** in the web console with the request ID to see the determining policy and diagnostics.
