...
On the local stack this is the whole file: the SDK defaults cover the STS, Coordinator, and Gateway URLs, and the client secret is read from the owner-only file you create at `<Caracal config dir>/runtime/<zone_id>/<application_id>/client-secret`, holding the secret shown once when the application was created. For cloud or custom deployments add `zone_url`, `coordinator_url`, `gateway_url`, and `app_client_secret_file`; see [Configuration File](/runtime-console/config-file/) for the full schema.
