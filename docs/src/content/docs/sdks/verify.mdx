...
`unauthorizedResponse` maps error codes to 401 or 403 and renders the same JSON error body as the framework adapters. Python and Go services cover this role with the [ASGI adapter](/sdks/adapters/asgi/) and the [net/http adapter](/sdks/adapters/nethttp/), which wrap their ecosystems' native request types.
