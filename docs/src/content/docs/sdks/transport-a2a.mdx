...
interface A2AOptions {
  ...
  retries?: number;                   // Retry count for recoverable transport failures; default: 2
  retryBaseMs?: number;               // Backoff base in milliseconds; default: 250
  retryTransientStatuses?: boolean;   // Retry 408, 425, 429, and 5xx responses; default: false
...
| `X-Caracal-Retry-Attempt` | Retry attempt number (`1`, `2`, ...), omitted on the first attempt |
...
| Condition | Behavior |
| --- | --- |
| HTTP 408, 425, 429, 5xx | Throw by default; retry after backoff only when `retryTransientStatuses` is `true` |
...
`timeoutMs` applies per attempt via `AbortController`. An attempt that exceeds the timeout counts as a recoverable transport failure and is retried. Retried target-agent requests include `X-Caracal-Retry-Attempt` so receivers can enforce idempotency, reject unsafe replays, or annotate execution audit records.
