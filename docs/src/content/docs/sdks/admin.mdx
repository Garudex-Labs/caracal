...
`@caracalai/admin` is the TypeScript client for Caracal's control-plane REST API. It is used by the Runtime and Terminal to manage zones, applications, policies, grants, and agent state. Use it directly when building tooling, scripts, or dashboards that interact with the Caracal control plane.
