...
`ensureGovernedUpstreams` composes them for the most common declaration: a set of upstream APIs, each with a sealed credential provider, a gateway-bound resource, and its application grants, converged in dependency order in one call.
  ...
  upstreams: [
    {
      ...
      resource: {
        ...
        gateway_application_id: agentAppId,
      },
