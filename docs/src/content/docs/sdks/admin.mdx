...
| `providerConnections` | `create`, `authorizeOAuth`, `revoke`                                                                                                                                                                                                                       |
...
Core groups and reconcilers are implemented in all three languages with idiomatic names. Do not infer that a newly introduced group exists in another language until its public package exports it; the raw [Admin API](/api/control-plane/) remains the wire source of truth.
