...
| `sessions` | `list` |
| `subjects` | `revoke` - the kill switch: one call revokes every live session record the subject holds, terminates the governed sessions riding them, revokes their delegations and the subject's provider connections, and feeds the revocation stream so in-flight tokens die before their `exp`. Idempotent. |
| `agentSessions` | `list` |
