...
| `client.FederateSubject(ctx, idToken, opts...)`        | Exchange an end user's identity token for an Authority record and return a `FederatedSubject` with `SubjectAuthorityRecordID`, `Token`, and `ExpiresInSeconds`. Setting `SubjectAuthorityRecordID` when starting a Session links attribution and lifecycle only; it does not by itself propagate the federated `sub` into later resource mandates. The returned mandate is the Subject's credential for supported approval and exchange paths.                                                                                                     |
...
The Go module ships as a single flat package: the envelope codec, context plumbing, Coordinator functions (`StartCoordinatorSession`, `CreateDelegation`, `HeartbeatSession`, `ListInboundDelegations`, `TerminateSession`), and Session primitives (`Session`, `StartSession`, `AttachSession`, `Delegate`) sit beside the `Caracal` client rather than behind a separate advanced entrypoint.
