...
| API | Purpose |
| --- | --- |
| `client.Transport(base, opts...)`                      | Return an `*http.Client` that injects Caracal headers. Set `CallOptions.Scopes` to mint the `use=gateway` mandate required at Gateway ingress and `CallOptions.ApprovalID` to consume an approved hold on that mint. Propagation defaults to `PropagationGatewayOnly`; use `PropagationAlways` only for a known Caracal-aware direct service chain. Gateway redirects are surfaced without automatically replaying the mandate.                                                                                                                                                    |
