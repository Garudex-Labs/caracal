...
| Operation                                                                    | Meaning                                                                                                                                      |
| ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `isRevoked(sid)` / `is_revoked(sid)` / `IsRevoked(sid)`                      | Return whether an identifier used as a revocation anchor is revoked. The compatibility parameter name `sid` accepts any supported anchor ID. |
| `markRevoked(sid, ttl)` / `mark_revoked(sid, ttl)` / `MarkRevoked(sid, ttl)` | Record an anchor ID as revoked for a TTL; `sid` is a compatibility parameter name, not a claim semantic.                                     |
