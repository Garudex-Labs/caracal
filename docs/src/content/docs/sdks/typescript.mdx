...
| `transport(options?)`                          | Return a fetch-compatible function that injects Caracal headers and Gateway routing. Pass `scopes` to mint the `use=gateway` mandate required at Gateway ingress, `timeoutMs` to bound the mint and request, and `propagation: "gateway-only"` to keep the context envelope off third-party hosts. Gateway redirects are surfaced without automatic replay.                                                                                                                                       |
...
Every request sent through `transport()` carries the context envelope: W3C `traceparent`/`tracestate` plus `caracal.*` baggage entries for Session, Delegation, and Subject authority record correlation. These are visible correlation identifiers, never credentials. The bearer is attached only inside the configured Gateway origin and base path, and Gateway strips `caracal.*` baggage before forwarding upstream. Use `propagation: "gateway-only"` when third-party hosts must not receive the correlation identifiers. Gateway-bound calls without `scopes` are valid only when the bound token is already a `use=gateway` mandate; lifecycle and resource tokens fail locally.
