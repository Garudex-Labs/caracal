...
| `await caracal.spawn_service(...)` | Start a long-lived service agent; auto-renews its lease and returns a handle with `heartbeat()` and `aclose()`. Pass `grant=Grant.narrow(...)` to bound the handle's authority. |
| `await caracal.delegate(...)` | Create a delegation edge to an existing agent session; returns the edge for the receiver to adopt. |
| `async with caracal.adopt_delegation(edge_id)` | Present a received delegation edge: bind a derived context carrying it. |
...
| `async with caracal.bind_from_headers(headers, allow_root=False, verifier=None)` | Bind inbound Caracal envelope headers; pass `verifier=` to enforce the bearer token before binding. |
...
Pass `verifier=` to **enforce at the boundary**. The callable receives the bearer token and must raise on failure; back it with `caracalai_identity.verify_token` so the application sees a request only after the mandate is proven. Return a `VerifiedClaims` object to stamp token-proven attribution (zone, application, agent session, delegation edge, hop) over the caller-supplied envelope headers. The SDK never inspects token internals itself.
...
- [MCP Auth Transport](/sdks/verify/)
