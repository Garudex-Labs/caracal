...
| Method | Purpose |
| --- | --- |
| `caracal.transport(as_application=False, ctx=None, scopes=None, approval_id=None, propagation="gateway-only", **kwargs)`       | Return an `httpx.AsyncClient` that injects Caracal headers and Gateway routing. Pass `ctx=` from thread pools or executors, `scopes=` to mint the `use=gateway` mandate required at Gateway ingress, and `approval_id=` to consume an approved hold on that mint. Use `propagation="always"` only for a known Caracal-aware direct service chain. Automatic redirects are rejected because request mandates are single-use.                                                                                                                                                                                                              |
