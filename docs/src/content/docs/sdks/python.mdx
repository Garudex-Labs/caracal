...
| Method | Purpose |
| --- | --- |
| `async with caracal.session(...)` | Run the block inside a governed session - a bounded identity Caracal establishes around whatever the block executes; pass `authority=Authority.narrow(...)` to bound its authority. |
| `await caracal.start_session(...)` | Start a governed session that outlives a block; auto-renews its lease and returns a handle with `heartbeat()` and `aclose()`. Pass `authority=Authority.narrow(...)` to bound the handle's authority. |
| `await caracal.delegate(...)` | Delegate a slice of the bound session's authority to an existing peer session; returns the delegation for the receiver to accept. |
| `async with caracal.accept_delegation(delegation_id)` | Present a received delegation: bind a derived context carrying it. |
...
| `caracal.transport(as_application=False, ctx=None, scopes=None, **kwargs)` | Return an `httpx.AsyncClient` that injects Caracal headers and Gateway routing. Pass `ctx=` from thread pools or executors; pass `scopes=` to authorize gateway calls with a scoped resource mandate instead of the raw subject token. |
...
| `caracal.mint_mandate(resource_id, scopes, ctx=None, ttl_seconds=None)` | Mint a cached resource mandate carrying the bound session and delegation; requires client-secret credentials. |
...
`on_event(hook)` reports every control-plane operation: `token.exchange` (with `resources`, `scopes`, and `cached` for cache hits), `approval.wait` (with `challenge_id` and the final `state`), and `coordinator.call` (with `method`, `path`, and `status`). Each event carries `ok` and `duration_ms`, ready to bridge into any metrics or tracing system. A hook that raises is ignored and never disturbs the operation that emitted the event.
