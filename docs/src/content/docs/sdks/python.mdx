...
| `caracal.transport(as_application=False, ctx=None, scopes=None, propagation="always", **kwargs)`            | Return an `httpx.AsyncClient` that injects Caracal headers and Gateway routing. Pass `ctx=` from thread pools or executors; pass `scopes=` to mint the `use=gateway` mandate required at Gateway ingress; pass `propagation="gateway-only"` to keep the context envelope off third-party hosts. Automatic redirects are rejected because request mandates are single-use.                                                                                                                                                                                                       |
| `caracal.sync_transport(as_application=False, ctx=None, scopes=None, **kwargs)`                             | Synchronous `httpx.Client` counterpart.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `caracal.application_transport(resource_id, scopes=[...], labels=None, mandate_ttl_seconds=None, **kwargs)` | Return an `httpx.AsyncClient` pinned to one resource, calling as the application's own identity. Provisioning starts one source/target Session pair and one narrowed Delegation per cache key; every request performs an uncached final mint and emits trace and authority-correlation headers. A cold call costs four provisioning calls plus the final SDK mint and Gateway STS exchange; a warm call still performs both per-request STS exchanges. `sync_application_transport(...)` is the synchronous counterpart. |
