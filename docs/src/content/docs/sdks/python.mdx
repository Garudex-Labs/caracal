...
| Method | Purpose |
| --- | --- |
| `async with caracal.session(...)` | Run the block inside a governed session - a bounded identity Caracal establishes around whatever the block executes; pass `authority=Authority.narrow(...)` to bound its authority, `task=` to record what the session is for (shown wherever the session is inspected), and `idempotency_key=` so a redelivered trigger resumes the session instead of minting a duplicate. |
| `await caracal.start_session(...)` | Start a governed session that outlives a block; auto-renews its lease and returns a handle with `heartbeat()` and `aclose()`. Pass `authority=Authority.narrow(...)` to bound the handle's authority; `task=` and `idempotency_key=` work exactly as in `session`. |
| `await caracal.attach_session(session_id, ...)` | Re-attach to a persisted service session after a restart: validates it with an immediate lease renewal and returns the same handle `start_session` does. |
...
| `caracal.federate_subject(id_token, ttl_seconds=None)` | Exchange an end user's identity token from a zone-trusted issuer for the user's subject session: returns a `FederatedSubject` with `subject_session_id`, `token`, and `expires_in_seconds`. Pass `subject_session_id=` when starting sessions on the user's behalf; the token is the user's own mandate for approval decisions. Never cached; requires client-secret credentials. |
