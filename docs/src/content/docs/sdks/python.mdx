...
| `caracal.application_transport(resource_id, scopes=[...], labels=None, mandate_ttl_seconds=None, **kwargs)` | Return an `httpx.AsyncClient` pinned to one resource, calling as the application's own identity: each mint cycle starts a source/target session pair, narrows a delegation to `scopes` on the resource, and caches the minted mandate until expiry. A cycle costs five control-plane calls and runs roughly once per mandate TTL (default 15 minutes); requests in between are served from cache. Requires client-secret credentials. `sync_application_transport(...)` is the synchronous counterpart. |
