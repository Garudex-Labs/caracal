...
| Session ID                 | OAuth client's compatibility name for an Authority record ID; sent as protocol `session_id`. It is not a governed Session ID. |
| Session ID                 | Protocol name for the governed Session ID; sent as `agent_session_id`.                                                        |
| Delegation ID              | Protocol name for Delegation ID; sent as `delegation_edge_id`.                                                                |
...
- `federateSubject(idToken, opts)` / `federate_subject(id_token, ...)` / `FederateSubject(ctx, idToken, opts)` - creates the Subject's Authority record and returns its compatibility `subjectSessionId` field. Never cached: each federation is an explicit identity event, and the record carries no resource authority.
