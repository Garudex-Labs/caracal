...
1. **Distribution.** Agent configurations - MCP servers, skills, hooks, prompts - live in per-tool config files with incompatible formats. Sharing a working setup means copy-pasting JSON between `~/.claude/`, `.kiro/`, `.cursor/`, and half a dozen other directories.
...
**Resources are the managed objects.** The registry stores Agents plus component Resources: MCP servers, Skills, Hooks, and Prompts. Each Resource has `namespace/slug` identity, versions, visibility, review status, lifecycle activity, and ownership metadata.
