...
Caracal exposes health probes, structured logs, and Prometheus metrics for monitoring itself.
...
```
http://localhost:8002 → Dashboards → Caracal → Caracal API Health
```
...
- **caracal-prometheus**: scrapes `/metrics` every 15s
- **caracal-grafana**: visualizes metrics (port 8002, login: admin/admin)
