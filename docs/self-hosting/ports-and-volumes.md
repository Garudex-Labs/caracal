...
| --- | --- | --- | --- |
| API | 8080 | `API_HOST_PORT` | HTTP / WebSocket |
| Web UI | 8000 | `WEB_HOST_PORT` | HTTP |
...
| Grafana, optional | 8002 | `GRAFANA_HOST_PORT` | HTTP |
...
* **Read-only root filesystem** on `caracal-api`, `caracal-worker`, `caracal-web` (tmpfs for `/tmp`).
...
* **Bridge network** (`caracal-net`) isolates inter-service traffic.
...
| From | To | URL |
| --- | --- | --- |
| API / worker | Postgres | `postgresql+asyncpg://postgres:postgres@caracal-db:5432/caracal` |
| API / worker | ClickHouse | `clickhouse://default:clickhouse@caracal-clickhouse:8123/caracal` |
| API / worker | Redis | `redis://caracal-redis:6379` |
| Web | API | `http://caracal-server:8080` |
