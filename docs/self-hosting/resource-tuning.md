...
Connection pool sizes, query limits, and timeout configuration. These settings control how Caracal connects to its backing stores (PostgreSQL, Redis, ClickHouse). Most deployments work fine with defaults. Tune when you see connection timeouts, pool exhaustion, or slow queries under load.
