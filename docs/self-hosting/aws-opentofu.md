...
End state: an Caracal install running in your own AWS account, fronted by an Application Load Balancer with HTTPS, with managed Postgres + Redis, ECS Fargate for the stateless app tier, and a single EC2 host for ClickHouse. Prometheus and Grafana are optional through `observability_stack`.

This is the recommended path for enterprise self-hosting on AWS. If you only want to evaluate Caracal, use [Docker Compose setup](docker-compose.md) instead.

## What gets provisioned

A single `tofu apply` creates:
...
| OpenTofu ≥ 1.6                                                                                                                         | `brew install opentofu` or use [tenv](https://tofuutils.github.io/tenv/)  |
...
| IAM principal with sufficient rights                                                                                                    | see [Required IAM permissions](aws-opentofu.md#required-iam-permissions)  |
| (Optional) Route 53 hosted zone                                                                                                         | required for HTTPS on a custom domain                                      |
| (Recommended) S3 bucket + DynamoDB table                                                                                                | remote OpenTofu state - see [Remote state](aws-opentofu.md#remote-state) |
...
Open that URL in your browser. The API and web tasks start a couple of minutes after the ALB targets register; refresh until you see the Caracal login page.

A ready-to-apply call of the module lives at [`infra/opentofu/aws/examples/minimal`](https://github.com/Garudex-Labs/caracal/blob/main/infra/opentofu/aws/examples/minimal/README.md).
...
The install comes up on the ALB's AWS-assigned hostname (e.g. `caracal-prod-alb-1234.us-east-1.elb.amazonaws.com`) over plain HTTP. Useful for evaluation; not for production.
...
OpenTofu requests an ACM certificate, validates it via DNS records in your hosted zone, attaches it to the ALB, and creates the alias A record pointing the domain at the ALB. There is no manual DNS step.
...
image_tag           = "v1.4.0"       # specific Caracal release; "latest" pulls main
```
...
Deploy Caracal into an existing VPC rather than having OpenTofu create a new one. Use this when you have Transit Gateway, shared-services VPCs, compliance requirements, or peered networks.
...
When `vpc_id` is set, OpenTofu skips creating VPC, subnets, IGW, NAT gateway, route tables, and VPC flow logs. All other resources (ALB, ECS, RDS, Redis, data host) are created inside your existing VPC.
...
The ALB SG must allow inbound TCP 80/443 from your desired CIDRs. The ECS SG must allow inbound TCP 8080 and 8000 from the ALB SG, with outbound to all.

A full working example lives at [`infra/opentofu/aws/examples/byovpc`](https://github.com/Garudex-Labs/caracal/blob/main/infra/opentofu/aws/examples/byovpc/README.md).

## Required IAM permissions

The IAM principal running OpenTofu needs permission to manage resources across these services. The simplest path is to attach the AWS-managed policies below; for a tighter custom policy, see [Hardened IAM policy](aws-opentofu.md#hardened-iam-policy).
...
# inside the session:
sudo journalctl -u caracal-bootstrap -f                                 # cloud-init / first-boot logs
sudo docker compose -f /opt/caracal/docker-compose.data.yml ps          # CH/Grafana/Prom status
sudo docker compose -f /opt/caracal/docker-compose.data.yml logs -f clickhouse
```
...
The OpenTofu run generates and stores `DB_PASSWORD`, `CLICKHOUSE_PASSWORD`, `SECRET_KEY`, and `GRAFANA_ADMIN_PASSWORD`, plus pre-built `DATABASE_URL`, `REDIS_URL`, and `CLICKHOUSE_URL` connection strings. ECS injects these into tasks at start; you never paste them in.

### Upgrade to a new Caracal release
...
By default, OpenTofu writes state to your laptop (`terraform.tfstate`). For a real install, configure remote state so the install can be managed by anyone on your team and state isn't lost.
...
2.  Uncomment and fill the backend block in `versions.tf`:
    ...
    backend "s3" {
      ...
      key            = "caracal/prod/terraform.tfstate"
      region         = "us-east-1"
      dynamodb_table = "your-tf-lock-table"
      encrypt        = true
    }
    ```
3. Re-run `tofu init` and answer "yes" when prompted to migrate state.
...
* [ ] Configure Caracal SSO. See [Authentication and SSO](authentication.md)
...
**`tofu apply` succeeds but the URL returns 502.** The api/web tasks are still starting. Watch the service:

```bash
aws ecs describe-services \
  --cluster $(tofu output -raw ecs_cluster_name) \
  --services caracal-prod-api \
...
**Init task fails.** Check the `/aws/ecs/caracal-prod/init` log group. The most common failures are:

* RDS not yet reachable (transient on first apply; re-running fixes it)
* Migration error (look at the entrypoint output)

To re-run by hand: `$(tofu output -raw init_run_task_command)`.

**ALB target health is `unhealthy`.**

* For `api`: health check hits `/readyz` on port 8080. Tail the api log group for startup errors.
* For `web`: health check hits `/` on port 8000. Check the web log group.
* For Grafana: health check hits `/api/health` on port 8002. SSM into the data host and check `docker compose ps`.

**ACM certificate stuck in `Pending validation`.** The DNS validation records were not created in your hosted zone. Verify `route53_zone_id` is correct and that the IAM principal had `route53:ChangeResourceRecordSets` permission at apply time. Re-run `tofu apply` after fixing.
