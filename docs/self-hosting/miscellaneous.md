...
Restrict which harnesses are available in the platform. When set, only the listed harnesses appear in install dropdowns, agent compatibility tags, and `caracal agent pull` target selection.

**Affects:** The harness dropdown on agent detail pages, component install commands, agent builder harness selection, and `caracal agent pull --harness` validation. Harnesses outside the allowlist are hidden from all users.
...
**CLI behavior:** `caracal agent pull` still requires `--harness`. The selected harness must be allowed by the server.
...
Limits telemetry to agents that are registered in the Caracal registry.
