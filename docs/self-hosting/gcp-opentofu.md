...
End state: Caracal running in your GCP project on Cloud Run, backed by Cloud SQL Postgres, Memorystore Redis, and a GCE instance for ClickHouse - with a Global HTTPS Load Balancer, managed SSL certificate, Secret Manager for credentials, and GCS for backups.
...
# 4. Run migrations
$(tofu output -raw init_job_run_command)
...
The install comes up on the Cloud Run default URL (e.g. `https://caracal-prod-api-xxxxx-uc.a.run.app`).
...
OpenTofu provisions a Global HTTPS Load Balancer with a Google-managed SSL certificate and creates the DNS A record.
...
```bash
$(tofu output -raw data_host_ssh_command)
# Inside:
...
```bash
gcloud logging read "resource.type=cloud_run_revision AND resource.labels.service_name=caracal-prod-api" \
...
gcloud secrets versions access latest \
  --secret="caracal-prod-SECRET_KEY"
```
...
- [ ] Set up OpenTofu remote state in GCS
...
**Init job fails.**
```bash
gcloud run jobs executions list --job=caracal-prod-init --region=us-central1
gcloud logging read "resource.type=cloud_run_job AND resource.labels.job_name=caracal-prod-init" --limit 20
```
