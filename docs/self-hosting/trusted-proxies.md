...
After saving, check that Caracal resolves the correct client IP:

```bash
curl -s https://caracal.yourcompany.com/api/health/ip \
...
A list of IP addresses or CIDR ranges that Caracal trusts to set forwarding headers (`X-Forwarded-For`, `X-Real-IP`, `X-Forwarded-Proto`).
...
|---------|-------------|-----|
| All users show same IP in audit logs | Proxy IP not trusted; Caracal records proxy IP as client | Add proxy's IP to trusted list |
...
| Users get `403 Untrusted proxy` | Client's IP is in a range Caracal doesn't recognize | Check if a new proxy/LB was added without updating this list |


## SSRF Protection {#ssrf-protection}

Server-Side Request Forgery (SSRF) protection prevents Caracal from making outbound requests to internal/private networks when processing user-supplied URLs (webhook destinations, IdP metadata URLs, custom API base URLs).
...
**When to set:** Your deployment hosts agent source code on an internal GitLab/Gitea/GitHub Enterprise instance that resolves to a private IP. Enable this so `caracal publish` and component installs can clone from internal repos.
