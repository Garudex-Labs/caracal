...
Every file Caracal reads or writes on the client (`~/.caracal/`) and in each harness's config directory.

## Client-side: `~/.caracal/`

| File | Purpose | Permissions |
| --- | --- | --- |
| `config.json` | CLI config (server URL, session token, access token, user info, timeout) | `0600` |
...
Authentication and identity fields are managed by `caracal auth`. The `config set` command accepts only `server_url`, `timeout`, `update_check`, `update_check_interval`, and `update_check_repo`. Supported environment variables override persisted values for the current invocation; see [Environment variables](environment-variables.md).

### Durable session outbox

Python session exporters persist each observed batch in `telemetry_buffer.db` before network delivery. OpenCode and Pi use per-session files under their native outbox directories because their TypeScript runtimes cannot call the Python SQLite engine. All follow the same protocol: pending data survives process restarts and failed attempts, and the source line advances only when the server's contiguous checkpoint covers the complete batch. Caracal does not silently evict unacknowledged records at capacity.
...
Use `caracal ops telemetry status` to inspect pending batch count, disk use, oldest pending time, and last successful acknowledgement.
...
`caracal doctor patch` and `caracal doctor cleanup` write each managed configuration file atomically and preserve unrelated entries. Use `--dry-run --output json` to review planned harness changes before writing. Agent Pull reports every created or merged file in its result.

## File permissions

Client-side files under `~/.caracal/` are created with mode `0600` (owner read/write only). This holds your access token, so don't loosen the permissions.

## Related

* [Environment variables](environment-variables.md) - supported runtime overrides
* [`caracal config`](../cli/config.md), CLI surface for editing
