...
| `CORS_ALLOWED_ORIGINS`   | `http://localhost:8000`        | Comma-separated allowed CORS origins                                                                                                                          |
...
| Variable              | Default                                                          | Description                                                 |
| --------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------- |
| `DATABASE_URL`        | `postgresql+asyncpg://postgres:postgres@localhost:5432/caracal` | PostgreSQL endpoint and source URL for `server migrate`. Supports `DATABASE_URL_FILE` in server configuration. |
...
| `CLICKHOUSE_URL`      | `clickhouse://localhost:8123/caracal`                           | ClickHouse endpoint and source URL for `server migrate`. Supports `CLICKHOUSE_URL_FILE` in server configuration. |
...
| `JWT_KEY_DIR`           | `~/.caracal/keys` (outside Docker) / `/data/keys` (Docker) | Directory for generated signing keys. **Back this up.** |
| `JWT_KEY_PASSWORD`      | unset                                                       | Optional private-key encryption password. Supports `JWT_KEY_PASSWORD_FILE`. |

### AWS (Bedrock)

> **Note:** Bedrock now supports [API keys](https://docs.aws.amazon.com/bedrock/latest/userguide/api-keys.html) - generate one from the AWS console and use it like any other provider. See [Insights LLM Setup](https://github.com/Garudex-Labs/caracal/blob/main/docs/insights-setup.md).
...
| ---------------------- | ------- | ------------------------- |
| `CARACAL_BIND_ADDRESS` | `127.0.0.1` in the server package | Host address for published ports. A non-loopback value is explicit remote exposure. |
| `CARACAL_SECRET_GID` | operator group from `setup.sh` | Supplementary container group allowed to read server-package files with mode `0640`. |
| `API_HOST_PORT`        | `8080`  | API (internal, behind LB) |
...
Only used when the optional Grafana overlay or OpenTofu `observability_stack = "grafana"` is enabled.
...
Read from the environment at invocation time. Override values in `~/.caracal/config.json` per invocation.

| Variable                | Default                        | Description                                                 |
| ----------------------- | ------------------------------ | ----------------------------------------------------------- |
| `CARACAL_SERVER_URL`   | from `~/.caracal/config.json` | Server URL                                                  |
| `CARACAL_ACCESS_TOKEN` | from `~/.caracal/config.json` | Access token (preferred for CI)                             |
| `CARACAL_TOKEN`        | from `~/.caracal/config.json` | Legacy CI token alias with highest precedence |
| `CARACAL_PASSWORD`     | unset                          | Password for credential or bootstrap login |
| `CARACAL_CURRENT_PASSWORD` | unset                      | Current password for non-interactive password changes |
| `CARACAL_NEW_PASSWORD` | unset                          | New password for non-interactive or mandatory password changes |
| `CARACAL_TIMEOUT`      | `30`                           | HTTP timeout in seconds |

The token and password variables support corresponding `_FILE` forms. Configuring both a direct variable and its file form is an error. When multiple token aliases are present, the legacy precedence remains `CARACAL_TOKEN`, then `CARACAL_ACCESS_TOKEN`.

Example CI usage:

```bash
export CARACAL_SERVER_URL=https://caracal.your-company.internal
export CARACAL_ACCESS_TOKEN=<token>
...
* [Config files](config-files.md), `~/.caracal/` file layout
