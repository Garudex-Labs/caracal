...
caracal registry hook submit

# From JSON file
caracal registry hook submit --from-file hook.json

# With a script file (content stored in registry)
caracal registry hook submit --script ./my-hook.sh

# As a draft (not submitted for review)
caracal registry hook submit --draft

# Submit an existing draft for review
caracal registry hook submit --submit <hook-id>

# With git source tracking
caracal registry hook submit --source-url https://github.com/org/hooks --source-ref main

# With install prerequisites
caracal registry hook submit --requires "apt-get install jq"
```
...
```bash
# All approved hooks
caracal registry hook list

# Filter by event
caracal registry hook list --event PreToolUse

# Search
caracal registry hook list --search "lint"

# JSON output
caracal registry hook list --output json
```

### Show hook details

```bash
caracal registry hook show <name-or-id>
caracal registry hook show <name-or-id> --output json
```
...
caracal registry hook install <name> --harness claude-code

# Install for Cursor
caracal registry hook install <name> --harness cursor

# Install for Kiro
caracal registry hook install <name> --harness kiro

# Install into a specific directory
caracal registry hook install <name> --harness claude-code --dir /path/to/project

# Raw server response, no file writes
caracal registry hook install <name> --harness claude-code --raw

# Install files and return a JSON operation result
caracal registry hook install <name> --harness claude-code --output json
```
...
```bash
caracal registry hook edit <name-or-id> --name "new-name" --output json
caracal registry hook edit <name-or-id> --description "updated" --output json
caracal registry hook edit <name-or-id> --from-file updates.json --output json
```
...
```bash
caracal registry hook submit --from-file block-rm-rf.json
caracal registry hook install block-rm-rf --harness claude-code
```
...
```bash
caracal registry hook install protect-files --harness claude-code
...
```bash
caracal registry hook submit --script ./log-tools.sh
...
EOF
caracal agent init --dir ./my-agent --name my-agent  # then agent add / build / publish

# Pull the agent - hook is auto-installed
caracal agent pull safe-coder --harness claude-code
...
| Caracal Event | Claude Code | Cursor | Kiro | Codex CLI |
|----------------|-------------|--------|------|-----------|
...
caracal registry hook install my-hook --harness claude-code  # → .claude/settings.json
caracal registry hook install my-hook --harness cursor       # → .cursor/hooks.json
caracal registry hook install my-hook --harness kiro         # → ~/.kiro/agents/my-hook.json
```
