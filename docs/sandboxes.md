...
Sandboxes are versioned execution environments registered in Caracal. When an agent has a sandbox component, Caracal installs an `caracal-sandbox` MCP server that exposes one callable tool per sandbox.
...
Docker is the common path. The other runtimes are local-runtime dispatchers: Caracal stores and installs the metadata, but the developer machine must have the corresponding runtime and artifact already available.
...
`source_url` + `source_ref` + `sandbox_path` can point at a Dockerfile or sandbox source tree, but Caracal does not build or publish images from that path today. Publish a built image/artifact ref in `image`.
...
```text
caracal agent pull my-agent --harness claude-code
    │
    ├── Registers "caracal-sandbox" MCP server
    │   └── Exposes run_sandbox_<name> as a callable tool
    │
    └── Agent calls run_sandbox_python_pytest(command="pytest tests/")
        └── MCP server → caracal sandbox run → local runtime → output
```

## Submit a sandbox

```bash
caracal registry sandbox submit \
  --name python-pytest \
  --version 1.0.0 \
...
Sandbox submission returns the direct server result in JSON mode. Standalone Sandbox installation is not supported. Add the returned sandbox UUID to an agent with `caracal agent add sandbox <sandbox-uuid>`.
...
```bash
caracal registry version publish sandbox python-pytest \
  --version 1.1.0 \
