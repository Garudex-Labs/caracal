...
AI failures rarely produce one reliable error code. Caracal preserves the session transcript so you can inspect what the user asked, which tools ran, what results the harness recorded, and how the agent responded.
...
The detail available for a tool call depends on the harness transcript. When present, Caracal displays the tool name, input, result, and error indicators. Caracal does not intercept MCP transport traffic or invent payload fields that the harness did not record.
