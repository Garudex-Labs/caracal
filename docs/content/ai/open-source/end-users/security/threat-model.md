...
Deterministic machine-facing contract for threat model in the `/ai/open-source/end-users/security/threat-model` route scope.

## Inputs

| name | type | required | source | notes |
| --- | --- | --- | --- | --- |
| Workspace context | object | yes | THREAT_MODEL.md | Workspace and principal scope for evaluation. |
| Authority context | object | conditional | THREAT_MODEL.md | Policy, mandate, and delegation data when required. |
| Operation payload | map | yes | THREAT_MODEL.md | Route-specific fields with canonical identifiers. |
...
- [/open-source/end-users/security/threat-model](/open-source/end-users/security/threat-model)
