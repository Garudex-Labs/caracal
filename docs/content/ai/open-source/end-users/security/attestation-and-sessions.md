...
Deterministic machine-facing contract for attestation and sessions in the `/ai/open-source/end-users/security/attestation-and-sessions` route scope.

## Inputs

| name | type | required | source | notes |
| --- | --- | --- | --- | --- |
| Workspace context | object | yes | packages/caracal-server/caracal/core/session_manager.py | Workspace and principal scope for evaluation. |
| Authority context | object | conditional | packages/caracal-server/caracal/core/session_manager.py | Policy, mandate, and delegation data when required. |
| Operation payload | map | yes | packages/caracal-server/caracal/core/session_manager.py | Route-specific fields with canonical identifiers. |
...
```json
{
  "operation": "attestation-and-sessions",
  "status": "ok",
  "result": {
    "id": "example-id"
  },
...
- [/open-source/end-users/security/attestation-and-sessions](/open-source/end-users/security/attestation-and-sessions)
