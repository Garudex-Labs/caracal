...
Deterministic machine-facing contract for master encryption key (mek) in the `/ai/open-source/end-users/concepts/master-encryption-key` route scope.
...
```json
{
  "operation": "master-encryption-key",
  "status": "ok",
  "result": {
    "id": "example-id"
  },
...
- [/open-source/end-users/concepts/master-encryption-key](/open-source/end-users/concepts/master-encryption-key)
