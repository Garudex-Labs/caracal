...
Deterministic machine-facing contract for policy in the `/ai/open-source/end-users/concepts/policy` route scope.

## Inputs

| name | type | required | source | notes |
| --- | --- | --- | --- | --- |
| Workspace context | object | yes | packages/caracal-server/caracal/core/authority.py | Workspace and principal scope for evaluation. |
| Authority context | object | conditional | packages/caracal-server/caracal/core/authority.py | Policy, mandate, and delegation data when required. |
| Operation payload | map | yes | packages/caracal-server/caracal/core/authority.py | Route-specific fields with canonical identifiers. |
