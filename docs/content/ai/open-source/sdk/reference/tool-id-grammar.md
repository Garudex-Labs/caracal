...
Deterministic machine-facing contract for tool id grammar in the `/ai/open-source/sdk/reference/tool-id-grammar` route scope.

## Inputs

| name | type | required | source | notes |
| --- | --- | --- | --- | --- |
| Workspace context | object | yes | packages/caracal-server/caracal/provider/definitions.py | Workspace and principal scope for evaluation. |
| Authority context | object | conditional | packages/caracal-server/caracal/provider/definitions.py | Policy, mandate, and delegation data when required. |
| Operation payload | map | yes | packages/caracal-server/caracal/provider/definitions.py | Route-specific fields with canonical identifiers. |
