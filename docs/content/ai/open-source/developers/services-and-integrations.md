...
Deterministic machine-facing contract for services and integrations in the `/ai/open-source/developers/services-and-integrations` route scope.

## Inputs

| name | type | required | source | notes |
| --- | --- | --- | --- | --- |
| Workspace context | object | yes | packages/caracal-server/caracal/db/ | Workspace and principal scope for evaluation. |
| Authority context | object | conditional | packages/caracal-server/caracal/db/ | Policy, mandate, and delegation data when required. |
| Operation payload | map | yes | packages/caracal-server/caracal/db/ | Route-specific fields with canonical identifiers. |
...
```json
{
  "operation": "services-and-integrations",
  "status": "ok",
  "result": {
    "id": "example-id"
  },
...
- [/open-source/developers/services-and-integrations](/open-source/developers/services-and-integrations)
