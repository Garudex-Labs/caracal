...
In OSS, provider credentials are resolved through `caracal/provider/credential_store.py` and the OSS vault. The broker is the only OSS execution path. Enterprise-ready provider packs and gateway vault binding stay in `caracalEnterprise/`.
...
- provider creation may be passthrough-only at first; scoped resource/action definitions are added later through enrich flows
...
- Provider definitions are optional for passthrough providers, but once scoped mode is enabled they become part of authority and policy validation.
