...
|---|---------|------|---------|
| 1 | Operator | `operator` | Operates the deployment and creates users |
| 2 | Organization Admin | `user` + org `admin` | Administers one organization |
...
| 6 | User B | `user` | Pulls agents, tests in harnesses |
...
## 1. Environment Setup
- [ ] `make reset NOCACHE=1` (destructive: wipes volumes and rebuilds without cache)
...
- [ ] Log in as Reviewer A via CLI (`caracal auth login`)
- [ ] List pending submissions in the web UI review queue
- [ ] Approve some components in the web UI review queue
- [ ] Reject some components with reasons in the web UI review queue
...
### Via CLI
- [ ] Create 3 agents via CLI (`caracal agent init` + `agent add` + `agent build` + `agent publish`)
- [ ] Save at least 1 agent as draft first, verify it appears in drafts
- [ ] Submit agents for review

## 8. Reviewer A - Review Agents via CLI
- [ ] Log in as Reviewer A via CLI
- [ ] List pending agent submissions in the web UI review queue
- [ ] Approve some agents in the web UI review queue
- [ ] Reject some agents with reasons in the web UI review queue
...
- [ ] Log in as User B via CLI (`caracal auth login`)
- [ ] Pull/install an agent via CLI (`caracal agent pull <agent> --harness <harness>`)
- [ ] Verify download count increases (0 → 1)

## 12. User C - Agent Pull & Downloads
- [ ] Log in as User C via CLI (`caracal auth login`)
- [ ] Pull/install the same agent via CLI (`caracal agent pull <agent> --harness <harness>`)
...
## 16. CLI -- Scan, Doctor & Patch
- [ ] Run `caracal scan` to discover harness configs (read-only)
- [ ] Run `caracal doctor patch --all-harnesses --dry-run` to preview instrumentation
- [ ] Run `caracal doctor patch --all-harnesses` to instrument harnesses
- [ ] Run `caracal doctor --output json` to check harness compatibility

## 17. Admin - Agent Registry Management
- [ ] Log in as Admin
- [ ] Go to agent registry
- [ ] Archive some agents
- [ ] Delete some agents

## 18. User C - Verify Registry Visibility
- [ ] Log in as User C
...
> **How to switch:** Log in as an operator -> Operator -> Telemetry & Data -> toggle "Only trace registered agents". The toggle affects new session ingestion.

Run sections 20–30 once with Case A, then reset and repeat with Case B.

---

## 19. Admin - Trace Privacy Toggle
...
## 20. User A - Edit Components & Publish New Versions
...
### Via CLI
- [ ] Log in as User A via CLI (`caracal auth login`)
- [ ] Run `caracal component version list <component-slug>` → verify current versions shown
- [ ] Run `caracal component version publish <component-slug>` → follow interactive prompts
- [ ] Verify CLI confirms version submitted for review

---

## 21. Reviewer B - Review Component Versions via UI (with Diff)

- [ ] Log in as Reviewer B
- [ ] Go to review queue → see pending component versions from Section 20
...
## 22. User A - Edit Agent & Release New Version
...
- [ ] Run `caracal agent versions <agent-slug>` → verify current versions shown
- [ ] Run `caracal agent release <agent-slug>` → follow prompts to publish new version
- [ ] Verify CLI confirms version submitted for review

---

## 23. Reviewer A - Review Agent Versions via CLI

- [ ] Log in as Reviewer A via CLI
- [ ] Open the web UI review queue → see pending agent version from Section 22
- [ ] Approve the new agent version in the web UI review queue

---

## 24. User B - Pull Updated Agent & Verify New Version

- [ ] Log in as User B via CLI (`caracal auth login`)
- [ ] Run `caracal agent pull <agent-slug> --harness <harness>` (note: command moved from `caracal pull`)
...
## 25. User A - Edit & Resubmit Rejected Items

- [ ] Log in as User A
- [ ] Navigate to the rejected component version from Section 21
...
## 26. Registered-Agents-Only Enforcement (Case B only)

> These steps only apply when running under **Case B** (registered-agents-only enabled).

- [ ] Log in as an operator -> Operator -> Telemetry & Data -> enable "Only trace registered agents"
- [ ] Switch to User B
...
- [ ] Run `caracal scan` → verify it warns/skips unregistered agents
- [ ] Run `caracal doctor patch --all-harnesses` → verify it only instruments registered agents
- [ ] Switch to Admin → verify traces only exist for registered agent activity

---

## 27. User A - Edit Pending Submissions (Edit Lock)
...
- [ ] Switch to Reviewer A → open the web UI review queue → verify agent is NOT listed
- [ ] Switch back to User A → save edits
- [ ] Switch to Reviewer A → open the web UI review queue → verify agent reappears

### Edit pending items via CLI
- [ ] Log in as User A via CLI
- [ ] Run edit command for a pending component (e.g. `caracal skill edit <slug>`)
