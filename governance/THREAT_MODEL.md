...
- **How we verify.** `go test ./services/gateway/...` covering SSRF metadata/link-local/multicast/unspecified/NAT64-embedded blocking, the private/loopback/CGNAT allow-by-default policy, egress-allowlist enforcement, redirect non-following (the 3xx is surfaced and the redirect target is never dialed or credentialed), header stripping, request-size, timeout, replay, revocation, and STS-failure cases.
