...
const PRIVATE_V4 = [/^10\./, /^127\./, /^169\.254\./, /^192\.168\./, /^172\.(1[6-9]|2[0-9]|3[0-1])\./]
...
export function checkApiReady(probe) {
  ...
  if (probe.error) {
    return fail(
      'readiness',
      id,
      `Admin API unreachable: ${probe.error}`,
  ...
  if (probe.status !== 200) {
    return fail(
      'readiness',
      id,
      `GET /ready returned ${probe.status}`,
...
export function checkGatewayReady(probe) {
  ...
  if (!probe) {
    return warn(
      'readiness',
      id,
      'Gateway not probed (PREFLIGHT_GATEWAY_URL not set)',
  ...
  if (probe.error) {
    return fail(
      'readiness',
      id,
      `Gateway unreachable: ${probe.error}`,
  ...
  if (probe.status !== 200) {
    ...
    return fail(
      'readiness',
      id,
      `GET /ready returned ${probe.status} (reason: ${reason})`,
...
export function checkBinding(resource, provider) {
  ...
  if (!resource.credential_provider_id) {
    return fail(
      'dependencies',
      id,
      `resource ${resource.identifier} has no credential provider bound`,
  ...
  if (application.expires_at) {
    ...
    if (expires <= now) {
      return fail(
        'dependencies',
        id,
        `application ${application.name} expired at ${application.expires_at}`,
    ...
    if (expires.getTime() - now.getTime() < dayMs) {
      return warn(
        'dependencies',
        id,
        `application ${application.name} expires within 24h (${application.expires_at})`,
...
export function checkProviderConfig(provider) {
  ...
  if (!provider) {
    return fail('configuration', id, 'no provider to evaluate', 'Resolve the resource binding failure first.')
  }
  ...
  if (missing.length > 0) {
    return fail(
      'configuration',
      id,
      `${provider.kind} provider is missing required config: ${missing.join(', ')}`,
  ...
  if (provider.kind === 'api_key') {
    if (config.auth_location === 'header' && !config.header_name) {
      return fail(
        'configuration',
        id,
        'api_key provider uses header auth but header_name is unset',
    ...
    if (config.auth_location === 'query' && !config.query_param_name) {
      return fail(
        'configuration',
        id,
        'api_key provider uses query auth but query_param_name is unset',
  ...
  if (OAUTH_KINDS.has(provider.kind) && Array.isArray(config.allowed_token_hosts) && config.allowed_token_hosts.length > 0) {
    ...
    if (host && !config.allowed_token_hosts.includes(host)) {
      return fail(
        'configuration',
        id,
        `token_endpoint host ${host} is not in allowed_token_hosts [${config.allowed_token_hosts.join(', ')}]`,
...
export function checkScopeCoverage(resource, requestedScopes) {
  ...
  if (uncovered.length > 0) {
    return fail(
      'configuration',
      id,
      `requested scope(s) not declared on the resource: ${uncovered.join(', ')}`,
...
export function checkRuntimeInjection(provider, requireInjection) {
  ...
  if (provider.config_json?.allow_runtime_injection !== true) {
    return fail(
      'configuration',
      id,
      `provider ${provider.identifier} does not set allow_runtime_injection=true`,
...
export async function checkTokenEndpointHost(provider, resolveHost) {
  ...
  if (!endpoint) {
    return fail(
      'connectivity',
      id,
      'OAuth provider has no token_endpoint',
  ...
  try {
    ...
    if (url.protocol !== 'https:') {
      return fail(
        'connectivity',
        id,
        `token_endpoint must be HTTPS: ${endpoint}`,
        "Use the provider's HTTPS token endpoint; STS refuses plaintext token exchanges.",
      )
    }
    host = url.hostname
  } catch {
    return fail('connectivity', id, `token_endpoint is not a valid URL: ${endpoint}`, 'Correct the token_endpoint URL on the provider.')
  }
  try {
    ...
    if (privateHit) {
      return fail(
        'connectivity',
        id,
        `${host} resolves to private address ${privateHit}`,
    ...
    return fail(
      'connectivity',
      id,
      `${host} resolution failed: ${err.message}`,
...
export async function checkCallbackReachable(provider, probe) {
  ...
  if (!redirect) {
    return fail(
      'connectivity',
      id,
      'authorization-code provider has no redirect_uri',
  ...
  try {
    ...
    return fail('connectivity', id, `redirect_uri is not a valid URI: ${redirect}`, 'Correct the redirect_uri on the provider.')
  }
  if (url.protocol !== 'https:') {
    return fail(
      'connectivity',
      id,
      `redirect_uri must be HTTPS: ${redirect}`,
  ...
  if (!reach.reachable) {
    return fail(
      'connectivity',
      id,
      `${url.origin} not reachable: ${reach.detail}`,
...
export async function checkUpstreamReachable(resource, probe, resolveHost) {
  ...
  try {
    ...
    return fail('connectivity', id, `upstream_url is not a valid URL: ${upstream}`, 'Correct the upstream_url on the resource.')
  }
  try {
    ...
    if (blockedHit) {
      return fail(
        'connectivity',
        id,
        `${url.hostname} resolves to blocked address ${blockedHit}`,
  ...
  if (!reach.reachable) {
    return fail(
      'connectivity',
      id,
      `${url.origin} not reachable from this host: ${reach.detail}`,
...
export function checkPolicyDecision(simulation) {
  ...
  if (!simulation) {
    return fail(
      'authorization',
      id,
      'no active policy set in the zone',
  ...
  if (warnings.length > 0) {
    return fail(
      'authorization',
      id,
      `simulation input rejected: ${warnings.join(', ')}`,
  ...
  if (!simulation.result) {
    ...
    return fail(
      'authorization',
      id,
      `policy was not executed: ${reason}`,
  ...
  return fail(
    'authorization',
    id,
    `active policy set returned ${decision ?? 'no decision'} (${status ?? 'unknown status'})`,
