...
async function main() {
  ...
  const requestedScopes = env('PREFLIGHT_SCOPES', '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
  ...
  if (!apiProbe.error) {
    ...
    simulation = resource ? await loadSimulation(apiUrl, token, zoneId, resource, applicationId, requestedScopes) : undefined
  }
