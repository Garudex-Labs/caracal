...
import { assessSimulation, awaitPropagation, iterate, pickDeniedInput, runRegressions, summarizeTrace, verdict } from '../iterate.mjs'
...
test('runRegressions compares each case against its expected decision', async () => {
  const transport = {
    simulate: async (_set, _version, input) => (input.principal.id === 'app-good' ? allowSimulation : denySimulation),
  }
  const cases = [
...
test('iterate holds activation when a regression fails', async () => {
  const transport = {
    explain: async () => deniedTrace,
    simulate: async (_set, _version, input) => (input.principal.id === 'app-1' ? allowSimulation : denySimulation),
    activate: async () => {
      throw new Error('must not activate')
    },
  }
