...
def test_streams_fail_closed_without_caracal_or_simulation(monkeypatch):
    ...
    with mock.patch.object(streams.caracal, "enabled", return_value=False):
        streams.start_streams()
    assert streams._threads == []
...
def test_streams_use_direct_path_only_in_simulation(monkeypatch):
    ...
    seen: list[tuple] = []
    with mock.patch.object(streams.caracal, "enabled", return_value=False), \
         mock.patch.object(streams, "_consume_direct", side_effect=lambda *a: seen.append(a)):
        streams.start_streams()
        _drain_threads()
    assert seen == [("http://pulse.test", "pk-test", os.getenv("LYNX_PULSE_SYMBOL", "USD/EUR"))]


def test_streams_route_through_gateway_when_caracal_enabled(monkeypatch):
    monkeypatch.setenv("LYNX_SIMULATION", "1")
    seen: list[tuple] = []
    with mock.patch.object(streams.caracal, "enabled", return_value=True), \
         mock.patch.object(streams, "_consume_governed", side_effect=lambda *a: seen.append(a)):
        streams.start_streams()
        _drain_threads()
    assert seen == [("USD/EUR",)]
