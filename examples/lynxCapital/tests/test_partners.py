...
def test_api_key_query_async_job(providerlab):
    ...
    assert doc["data"]["status"] == "processing"
        ...
        "inkwell-ocr", "get_extraction", {"documentId": doc["data"]["documentId"]}
    )
...
def test_bearer_standard_unbalanced_entry(providerlab):
    res = partners.call(
        "slate-ledger", "post_entry", {"lines": [{"debit": 100}, {"credit": 90}]}
    )
    assert res["status"] == 422
...
def test_bearer_standard_balanced_entry(providerlab):
    res = partners.call(
        "slate-ledger", "post_entry", {"lines": [{"debit": 100}, {"credit": 100}]}
    )
...
def test_oauth_cc_basic_convert(providerlab):
    ...
    assert (
        "client_buy_amount" in res["data"] and res["data"]["currency_pair"] == "EURUSD"
    )
...
def test_oauth_ac_pkce_initiate_payment(providerlab):
    acct = partners.call("halcyon-bank", "list_accounts", {})["data"]["items"][0][
        "accountId"
    ]
    res = partners.call(
        "halcyon-bank",
...
def test_internal_billing_create_and_404(providerlab):
    ...
    missing = partners.call(
        "core-billing", "get_invoice", {"invoiceId": "inv_does_not_exist"}
    )
    assert missing["status"] == 404
    ...
    customer = partners.call("core-billing", "list_customers", {"pageSize": 1})["data"][
        "items"
    ][0]
    remit = partners.call(
        "core-billing",
...
def test_mcp_compliance_and_onboarding(providerlab):
    listed = partners.call(
        "atlas-vendor", "list_vendors", {"status": "active", "pageSize": 5}
    )
    assert listed["status"] == 200 and listed["data"]["items"]
    vid = listed["data"]["items"][0]["id"]
    compliance = partners.call(
        "atlas-vendor", "get_compliance_status", {"vendorId": vid}
    )
    assert compliance["status"] == 200 and "clearedToPay" in compliance["data"]
    onboarding = partners.call(
        "atlas-vendor", "get_onboarding_status", {"vendorId": vid}
    )
...
def test_mcp_tool_error_surfaces(providerlab):
    res = partners.call(
        "atlas-vendor", "get_vendor_profile", {"vendorId": "VEND-00000"}
    )
...
def test_sdk_payout_unverified_recipient(providerlab):
    ...
    partners.call(
        "quetzal-payouts", "verify_recipient", {"recipientId": rec["data"]["id"]}
    )
    paid = partners.call(
        "quetzal-payouts",
        "create_payout",
...
def test_partner_operation_tool_emits_and_runs(providerlab):
    res = tool_fns.partner_operation(
        "run-1",
        "agent-1",
        "sabre-tax",
        "resolve_jurisdiction",
