...
def _client() -> TestClient:
    return TestClient(
        build_app(catalog.get("junction-procure")),
...
def _active_supplier(c: TestClient, token: str) -> dict:
    ...
    items = _api(
        c, token, "list_suppliers", {"status": "active", "pageSize": 100}
    ).json()["data"]["items"]
...
def test_supplier_listing_and_lookup():
    ...
    one = _api(c, token, "get_supplier", {"supplierId": supplier["supplierId"]}).json()[
        "data"
    ]
    ...
    assert (
        missing.status_code == 404 and missing.json()["error"] == "supplier_not_found"
    )
...
def test_commodity_catalog_is_unspsc_shaped():
    ...
    assert items and all(
        len(i["commodityCode"]) == 8 and i["commodityCode"].isdigit() for i in items
    )
...
def test_sub_threshold_requisition_is_auto_approved():
    ...
    req = _api(
        ...
        token,
        "create_requisition",
        {"department": "engineering", "amount": 900, "description": "Keyboard"},
    ).json()["data"]
...
def test_single_step_requisition_routes_for_approval():
    ...
    chain = _api(
        c, token, "get_approval_chain", {"requisitionId": req["requisitionId"]}
    ).json()["data"]
    step = chain["chain"][0]
    assert step["role"] == "Cost Center Manager"
    assert step["approvalLimit"] == 25000.0 and step["slaHours"] == 24 and step["dueBy"]
    approved = _api(
        c, token, "approve_requisition", {"requisitionId": req["requisitionId"]}
    ).json()["data"]
...
def test_draft_requisition_submits_into_workflow():
    ...
    submitted = _api(
        c, token, "submit_requisition", {"requisitionId": draft["requisitionId"]}
    ).json()["data"]
...
def test_delegated_approver_can_sign_off():
    ...
    token = _token(c)
    listed = _api(
        c, token, "list_requisitions", {"status": "pending_approval", "pageSize": 100}
    ).json()["data"]["items"]
    delegated = None
    for req in listed:
        chain = _api(
        ...
        step = next(
            (s for s in chain if s["status"] == "pending" and s.get("delegatedTo")),
            None,
        )
        if step:
            delegated = (req, step)
    ...
    assert (
        approved.json()["data"]["approval"]["chain"][step["step"] - 1]["decidedBy"]
        == step["delegatedTo"]
    )
...
def test_multi_step_chain_requires_every_signature():
    ...
    req = _api(
        ...
        token,
        "create_requisition",
        {"department": "it", "amount": 60000, "description": "Network refresh"},
    ).json()["data"]
...
def test_wrong_approver_is_rejected():
    ...
    assert (
        ...
        and denied.json()["error"] == "not_authorized_approver"
    )
...
def test_requisition_can_be_rejected():
    ...
    req = _api(
        ...
        token,
        "create_requisition",
        {"department": "marketing", "amount": 12000, "description": "Campaign"},
    ).json()["data"]
    rejected = _api(
        c,
        token,
        "reject_requisition",
        {"requisitionId": req["requisitionId"], "comment": "Deferred"},
    ).json()["data"]
    ...
    assert (
        denied.status_code == 409 and denied.json()["error"] == "requisition_rejected"
    )
...
def test_hard_limit_blocks_final_approval():
    ...
    req = _api(
        ...
        token,
        "create_requisition",
        {"department": "legal", "amount": 50_000_000, "description": "Acquisition"},
    ).json()["data"]
...
def test_budget_view_exposes_commitment_accounting():
    ...
    by_code = _api(c, token, "get_budget", {"costCenter": budget["costCenter"]}).json()[
        "data"
    ]
...
def test_full_procure_to_pay_flow_closes_and_spends_budget():
    ...
    before = _api(c, token, "get_budget", {"department": "operations"}).json()["data"][
        "spentAmount"
    ]
    ...
    req = _api(
        ...
        token,
        "create_requisition",
        {"department": "operations", "amount": 9000, "description": "Forklift parts"},
    ).json()["data"]
    _api(c, token, "approve_requisition", {"requisitionId": req["requisitionId"]})
    ...
    closed = _api(
        c, token, "get_requisition", {"requisitionId": req["requisitionId"]}
    ).json()["data"]
    assert closed["status"] == "closed"
    after = _api(c, token, "get_budget", {"department": "operations"}).json()["data"][
        "spentAmount"
    ]
...
def test_purchase_order_carries_realistic_header_fields():
    ...
    req = _api(
        ...
        token,
        "create_requisition",
        {"department": "operations", "amount": 9000, "description": "Pallet jacks"},
    ).json()["data"]
    _api(c, token, "approve_requisition", {"requisitionId": req["requisitionId"]})
    po = _api(
        c,
    ...
    assert po["shippingAmount"] == 150.0 and po["total"] == round(
        req["total"] + 150.0, 2
    )
...
def test_budget_view_flags_soft_limit_breach():
    ...
    assert budgets and all(
        "softLimitBreached" in b and "consumedAmount" in b for b in budgets
    )
    ...
    assert (
        ...
        and denied.json()["error"] == "requisition_not_approved"
    )
...
def test_purchase_order_rejects_inactive_supplier():
    ...
    inactive = next(
        (
            s
            for s in _api(c, token, "list_suppliers", {"pageSize": 100}).json()["data"][
                "items"
            ]
            if s["status"] != "active"
        ),
    ...
    req = _api(
        ...
        token,
        "create_requisition",
        {"department": "facilities", "amount": 7000, "description": "HVAC"},
    ).json()["data"]
    _api(c, token, "approve_requisition", {"requisitionId": req["requisitionId"]})
    denied = _api(
        c,
...
def test_purchase_order_blocks_supplier_with_expired_compliance():
    ...
    lapsed = next(
        (
            s
            for s in items
            if any(d["status"] == "expired" for d in s.get("complianceDocuments", []))
        ),
        None,
    )
    if lapsed is None:
    ...
    req = _api(
        ...
        token,
        "create_requisition",
        {"department": "facilities", "amount": 7000, "description": "HVAC filters"},
    ).json()["data"]
    _api(c, token, "approve_requisition", {"requisitionId": req["requisitionId"]})
    denied = _api(
        c,
    ...
    assert (
        denied.status_code == 409
        and denied.json()["error"] == "supplier_compliance_hold"
    )
...
def test_partial_receipt_keeps_order_open():
    ...
    partial = _api(
        ...
        token,
        "receive_order",
        {"poId": po["poId"], "lines": [{"lineNumber": 1, "quantityReceived": 4}]},
    ).json()["data"]
...
def test_lynxcapital_procurement_tools_reach_junction(providerlab):
    ...
    assert (
        suppliers["provider"] == "junction-procure" and suppliers["data"]["total"] >= 1
    )
    ...
    approved = tool_fns.approve_requisition(
        "run", "agent", req["data"]["requisitionId"]
    )
    assert approved["data"]["status"] == "approved"

    po = tool_fns.create_purchase_order(
        "run", "agent", req["data"]["requisitionId"], supplier_id
    )
