...
function summarize(ev) {
  ...
  switch (ev.kind) {
    case 'run_start':
      return `Run started  run_id=${ev.run_id.slice(0, 8)}`
    case 'run_end':
      return `Run ended  status=${p.status || '?'}`
    case 'error':
      return `Error: ${p.message || ''}`
    case 'agent_spawn':
      return `Spawn  ${role}  ${p.region || ''}  id=${agent}`
    case 'agent_start':
      return `Start  id=${agent}`
    case 'agent_end':
      return `End    id=${agent}`
    case 'agent_terminate':
      return `Term   id=${agent}  status=${p.status || '?'}`
    case 'delegation':
      return `Delegation  parent=${(p.parent_id || '').slice(0, 8)}  child=${(p.child_id || '').slice(0, 8)}`
    case 'tool_call':
      return `Tool call   ${p.tool_name}  agent=${agent}`
    case 'tool_result':
      return `Tool result ${p.tool_name}  agent=${agent}`
    case 'service_call':
      return `SVC call    ${p.service_id} -> ${p.action}`
    case 'service_result':
      return `SVC result  ${p.service_id}`
    case 'audit_record':
      return `Audit record  agent=${agent}`
    case 'chat_user':
      return `User  ${(p.text || '').slice(0, 80)}`
    case 'chat_token':
      return `Token  ${JSON.stringify(p.token || '')}`
    case 'chat_message':
      return `Message  ${(p.text || '').slice(0, 80)}`
    case 'llm_call':
      return `LLM call  ${p.model}  ${p.latency_ms}ms  in=${p.input_tokens}tok out=${p.output_tokens}tok  tools=${p.tool_calls}  agent=${agent}`
    case 'memory_update':
      return `Memory  agent=${agent}  tokens=${p.tokens_used}/${p.tokens_limit}  msgs=${p.message_count}  compactions=${p.compactions}`
    case 'memory_compaction':
      return `Memory compaction  agent=${agent}  ${p.tokens_before}->${p.tokens_after} tokens`
    case 'model_change':
      return `Model change  ${p.prior} -> ${p.model}`
    case 'plan_update':
      return `Plan  agent=${agent}  rev=${p.revision}  items=${(p.todos || []).length}`
    case 'file_write':
      return `file_write  ${p.path}  ${p.size}B  agent=${agent}`
    case 'file_read':
      return `file_read   ${p.path}  agent=${agent}`
    case 'run_cancelled':
