...
async def validate_setup():
    ...
    steps.append(
        _step(
            ...
            "The zone is set."
            if zone_ok
            else "Add CARACAL_ZONE_ID from the Caracal web console.",
        )
    )
    ...
    if len(credentialed) == len(model.applications):
        ...
        app_status = "missing"
        app_detail = (
            "Run scripts/provision.py and merge config/provisioned.env into .env."
        )
    steps.append(
        _step("applications", "Application boundaries", app_status, app_detail)
    )

    provisioned_providers, provisioned_resources = setup_catalog.provisioned_state()
    providers_ready = sum(
        1 for p in model.providers if p.identifier in provisioned_providers
    )
    if providers_ready == len(model.providers):
        provider_status = "passed"
        provider_detail = (
            f"All {len(model.providers)} credential providers are registered."
        )
        ...
        provider_detail = (
            "Register the partner credential providers with scripts/provision.py."
        )
    steps.append(
        _step("providers", "Credential providers", provider_status, provider_detail)
    )

    views_ready = sum(
        1 for r in model.resources if r.identifier in provisioned_resources
    )
    if views_ready == len(model.resources):
        ...
        view_status = "missing"
        view_detail = (
            "Create the per-application resource views with scripts/provision.py."
        )
