...
def cli(ctx, config: Optional[Path], workspace: Optional[str], log_level: str, verbose: bool):
    ...
    \b
    Quick Start:
      caracal init                    # Initialize Caracal
      caracal workspace create dev    # Create workspace
      caracal workspace use dev       # Switch workspace
      caracal principal register      # Register AI agent
      caracal policy create           # Create policy
      caracal authority mandate       # Issue mandate
    
    \b
    Common Commands:
      workspace    Manage workspaces (primary context)
      principal    Manage AI agent identities
      policy       Manage authority policies
      authority    Issue and manage mandates
      sync         Sync with enterprise (if connected)
