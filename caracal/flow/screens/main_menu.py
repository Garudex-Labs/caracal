...
def _show_status_header(console: Console) -> None:
    ...
    try:
        ...
        active_workspace = get_active_workspace_name(config_mgr)
        if active_workspace:
            status_items.append((Icons.WORKSPACE, f"Workspace: {active_workspace}", Colors.PRIMARY))
    except Exception:
        pass
...
def get_submenu_items(category: str) -> list[MenuItem]:
    ...
    submenus = {
        ...
        "policies": [
            ...
            MenuItem(key="view", label="View Policy Details", 
                    description="View detailed policy information", icon=""),
            MenuItem(key="edit", label="Edit Policy", 
                    description="Modify policy settings", icon=""),
            MenuItem(key="deactivate", label="Deactivate Policy", 
                    description="Deactivate an authority policy", icon=""),
            MenuItem(key="back", label="Back to Main Menu", 
                    description="", icon=Icons.ARROW_LEFT),
        ],
        "ledger": [
            MenuItem(key="recent", label="Recent Events", 
                    description="View recent authority ledger events", icon=""),
            MenuItem(key="filter", label="Filter Events", 
                    description="Search authority events with filters", icon=""),
            MenuItem(key="view", label="View Event Details", 
                    description="View details of a specific event", icon=""),
            MenuItem(key="export", label="Export Events", 
                    description="Export ledger events to file", icon=""),
            MenuItem(key="back", label="Back to Main Menu", 
                    description="", icon=Icons.ARROW_LEFT),
        ],
        "mandates": [
            ...
            MenuItem(key="view", label="View Mandate Details", 
                    description="View details of a specific mandate", icon=""),
        ...
        "delegation": [
            ...
            MenuItem(key="delegate", label="Delegate Mandate", 
                    description="Create a delegated mandate", icon=""),
            MenuItem(key="peer", label="Peer Delegate", 
                    description="Create a peer delegation", icon=""),
