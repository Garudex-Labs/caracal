...
```bash
caracal registry mcp my --output json
caracal registry skill my --output json
...
```bash
caracal registry mcp edit NAMESPACE/SLUG --from-file updates.json --output json
caracal registry mcp edit NAMESPACE/SLUG --name new-name --description 'New description' --output json
...
```bash
caracal registry version publish mcp NAMESPACE/SLUG --version 1.2.0 --description 'What changed' --output json
...
```bash
caracal registry mcp archive NAMESPACE/SLUG --yes --output json
...
```bash
caracal registry mcp transfer-owner NAMESPACE/SLUG @username --yes --output json
caracal registry skill transfer-owner NAMESPACE/SLUG @username --yes --output json
```
...
```bash
caracal registry mcp co-authors list NAMESPACE/SLUG --output json
caracal registry skill co-authors add NAMESPACE/SLUG @username --output json
caracal registry hook co-authors remove NAMESPACE/SLUG USER_UUID --output json
```
