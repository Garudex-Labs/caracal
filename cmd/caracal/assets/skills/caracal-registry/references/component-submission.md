...
```bash
printf '%s\n' '{"command":"npx","args":["-y","@example/mcp-server"]}' | caracal registry mcp submit --name my-mcp --category developer-tools --yes --output json
caracal registry mcp submit --git https://github.com/org/mcp-server --name my-mcp --category developer-tools --yes --output json
printf '%s\n' '{"command":"npx","args":["internal-mcp"]}' | caracal registry mcp submit --name internal-mcp --category developer-tools --visibility project --yes --output json
```
...
```bash
caracal registry skill submit --skill-md ./SKILL.md --git-url https://github.com/org/repo --git-ref main --name my-skill --description 'What it does' --task-type general --output json
```

Registry-direct delivery stores the supplied SKILL.md and optional script:

```bash
caracal registry skill submit --skill-md ./SKILL.md --delivery-mode registry_direct --name my-skill --description 'What it does' --task-type general --harness claude-code --output json
caracal registry skill submit --skill-md ./SKILL.md --script ./run.sh --delivery-mode registry_direct --name my-skill --description 'What it does' --task-type general --output json
```
...
```bash
caracal registry hook submit --name guard --description 'Guard prompts' --event UserPromptSubmit --handler-command './guard.sh' --execution-mode sync --timeout 10 --scope agent --harness claude-code --output json
caracal registry hook submit --from-file hook.json --output json
```
...
```bash
caracal registry prompt submit --name frontend-brief --description 'Frontend design brief' --category general --template 'Design {{component}} accessibly' --output json
...
```bash
caracal registry skill submit --skill-md ./SKILL.md --delivery-mode registry_direct --name my-skill --description 'What it does' --task-type general --draft --output json
caracal registry skill submit --submit NAMESPACE/SLUG --output json
```
