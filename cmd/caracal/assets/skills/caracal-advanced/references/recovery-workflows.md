...
Session delivery is project-scoped. Confirm `caracal use` reports an organization and project, or select one with `caracal use ORG/PROJECT`, before reconciling; batches queued under one project are never replayed under another.
...
```bash
caracal reconcile --harness claude-code --since 24 --dry-run --output json
```

Then run the smallest required scope:

```bash
caracal reconcile --harness claude-code --since 24 --output json
caracal reconcile --output json
```
...
```bash
caracal self upgrade --force --output json
caracal self upgrade --version 2.6.0 --force --output json
caracal self downgrade --list --output json
caracal self downgrade --version 2.5.0 --force --output json
...
Cursor uses the same frontmatter in an `.mdc` file. Preserve existing files unless the user explicitly authorizes replacement. Report the exact path and state that no Registry publication occurred. When service returns, use the normal `caracal agent create` or authoring workflow.
