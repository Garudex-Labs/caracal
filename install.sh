...
#
# Standalone installer that downloads, verifies, and extracts the Caracal runtime CLI release archive.
...
uninstall() {
    ...
    if [ -d "${INSTALL_PATH}" ]; then
        for binFile in caracal caracal.exe; do
...
installStaged "$([ "${os}" = windows ] && printf 'caracal.exe' || printf 'caracal')"
committed=1
...
ok "Caracal runtime is installed"
section "Next steps"
printf '  Release: %s (%s)\n' "${tag}" "${mode}"
