...
// sectionPreamble frames every section prompt.
const sectionPreamble = `You are writing part of a personal usage insights report for an AI coding agent's user.
...
// sectionPrompts are the per-section templates; {data_block} (and
// {previous_data_block} for regression detection) are substituted at
// build time.
var sectionPrompts = map[string]string{
...
- For registry components, include: ` + "`caracal agent add skill <id>`" + `
...
const synthesisPrompt = `You're writing an "At a Glance" section for a usage insights report. The goal is to help the user understand their patterns and improve how they work with their AI coding agent.
...
// deepSections get the sections model; the rest use the synthesis model.
var deepSections = map[string]bool{
	"suggestions":       true,
...
// sectionMaxTokens holds per-section output budgets; unlisted sections get
// the default.
var sectionMaxTokens = map[string]int{
	"fun_ending": 1024,
}

const defaultSectionTokens = 16384

// buildSectionPrompts renders every section prompt against the data block.
func buildSectionPrompts(dataBlock, previousDataBlock, offerBlock string) map[string]string {
	prompts := make(map[string]string, len(sectionPrompts))
	for name, template := range sectionPrompts {
		var built string
		switch name {
		case "regression_detection":
			built = strings.Replace(template, "{data_block}", dataBlock, 1)
			built = strings.Replace(built, "{previous_data_block}", previousDataBlock, 1)
		case "suggestions":
			built = strings.Replace(template, "{data_block}", dataBlock+offerBlock, 1)
		default:
			built = strings.Replace(template, "{data_block}", dataBlock, 1)
		}
		prompts[name] = sectionPreamble + built
	}
	return prompts
}

// unwrapSection pulls a section's payload out of the model response: the
// expected key when present, otherwise the response's single value.
func unwrapSection(name string, result map[string]any) any {
	if payload, ok := result[name]; ok {
		return payload
	}
	if len(result) > 0 {
	...
	if name == "fun_ending" {
		return map[string]any{"headline": "", "detail": ""}
	}
	return map[string]any{}
}

// generateSections runs the section prompts in parallel and one synthesis
// pass over their outputs, returning the combined narrative.
func (e *Engine) generateSections(ctx context.Context, dataBlock string, previousReport map[string]any, offer *catalogOffer) map[string]any {
	previousDataBlock := "## Previous Period Metrics\nNo previous period data available."
	if previousReport != nil {
		blob, err := json.MarshalIndent(previousReport, "", "  ")
		if err == nil {
			previousDataBlock = "## Previous Period Metrics\n" + string(blob)
		}
	}
	offerBlock := ""
	if offer != nil {
		offerBlock = offer.promptBlock()
	}

	deepModel := e.Config.String(ctx, "insights.model_sections", "")
	fastModel := e.Config.String(ctx, "insights.model_synthesis", "")

	catalogItems := 0
	if offer != nil {
		catalogItems = offer.itemCount()
	}
	slog.Info("insight sections starting",
		"deep_model", orDefault(deepModel), "fast_model", orDefault(fastModel), "catalog_items", catalogItems)

	prompts := buildSectionPrompts(dataBlock, previousDataBlock, offerBlock)
	...
	for name, prompt := range prompts {
		...
		go func(name, prompt string) {
			defer wg.Done()
			model := fastModel
			if deepSections[name] {
				model = deepModel
			}
			maxTokens := defaultSectionTokens
			if budget, ok := sectionMaxTokens[name]; ok {
				maxTokens = budget
			}
			result := e.callModel(ctx, prompt, model, maxTokens)
			if len(result) == 0 {
				slog.Warn("section produced no output", "section", name)
			}
			mu.Lock()
			narrative[name] = unwrapSection(name, result)
			mu.Unlock()
		}(name, prompt)
	}
	wg.Wait()

	sectionsJSON, err := json.MarshalIndent(narrative, "", "  ")
	if err != nil {
		sectionsJSON = []byte("{}")
	}
	synthesis := strings.Replace(synthesisPrompt, "{data_block}", dataBlock, 1)
	synthesis = strings.Replace(synthesis, "{sections_json}", string(sectionsJSON), 1)
	synthesisResult := e.callModel(ctx, synthesis, fastModel, 4096)
	if payload, ok := synthesisResult["at_a_glance"]; ok {
		narrative["at_a_glance"] = payload
	} else if len(synthesisResult) > 0 {
		narrative["at_a_glance"] = unwrapSection("at_a_glance", synthesisResult)
	} else {
		narrative["at_a_glance"] = map[string]any{}
	}
	return narrative
}
