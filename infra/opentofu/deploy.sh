...
#
# Unified Caracal AWS Deployment
# ================================
...
if [[ "${1:-}" == "--help" ]] || [[ "${1:-}" == "-h" ]]; then
  echo "Usage: $0"
  echo ""
  echo "Interactive guided deployment for Caracal on AWS."
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║         Caracal AWS Deployment                     ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════════════════╝${NC}"
...
validate_standard() {
  ...
  local full_name="caracal-${prefix}"
...
try:
    with urllib.request.urlopen("https://ghcr.io/token?scope=repository:garudex-labs/caracal-server:pull", timeout=10) as r:
        print(json.load(r).get("token",""))
except Exception:
    pass
' 2>/dev/null || echo "")
  if [ -n "$token" ]; then
    status=$(curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer $token" -H "Accept: application/vnd.docker.distribution.manifest.v2+json" "https://ghcr.io/v2/garudex-labs/caracal-server/manifests/$tag" 2>/dev/null || echo "000")
  else
    status="000"
  fi
  if [ "$status" = "200" ]; then
    pass "GHCR image caracal-server:$tag reachable"
  else
    warn "Cannot verify GHCR image caracal-server:$tag (HTTP $status)"
  fi
}
...
echo ""
$TF -chdir="$MODULE_DIR" apply tfplan 2>&1 | tee /tmp/caracal-apply-output.log
APPLY_EXIT=${PIPESTATUS[0]}

if [ "$APPLY_EXIT" -ne 0 ]; then
  echo ""

  if grep -qi "already exists\|AlreadyExists\|BucketAlreadyExists\|EntityAlreadyExists\|ResourceAlreadyExistsException\|ParameterAlreadyExists" /tmp/caracal-apply-output.log 2>/dev/null; then
    ...
    echo "  left resources behind (no state to track them)."
    echo ""
    echo -e "  ${BOLD}Options to fix:${NC}"
    echo ""
    echo "  1. Use a different name (easiest):"
    echo "     Edit $TFVARS and change:"
    echo "       name_prefix = \"caracal-v2\"   # or any unique name"
    ...
    echo ""
    echo "  3. Import them into OpenTofu state:"
    echo "     $TF -chdir=$MODULE_DIR import <resource_address> <resource_id>"
    echo "     (advanced - run for each conflicting resource)"
    echo ""
  else
    fail "$TF apply failed (see errors above)"
  fi
  rm -f /tmp/caracal-apply-output.log
  exit 1
fi
rm -f /tmp/caracal-apply-output.log
...
echo ""
echo "  Get started:"
echo -e "  ${BOLD}1.${NC} caracal config set server_url <app_url>"
echo -e "  ${BOLD}2.${NC} caracal auth login"
echo ""
