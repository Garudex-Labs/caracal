...
variable "image_tag" {
  description = "Tag of the Caracal images to deploy."
  type        = string
  default     = "latest"
}
...
variable "ecs_instance_type" {
  description = "EC2 instance type for the ECS cluster (runs API server, identity service, and web containers)."
  type        = string
  default     = "t3.large"
}
...
variable "auth_cpu" {
  description = "CPU units for the identity service task (only when sizing = custom)."
  type        = number
  default     = 256
}

variable "auth_memory" {
  description = "Memory (MB) for the identity service task (only when sizing = custom)."
  type        = number
  default     = 512
}

variable "auth_desired_count" {
  description = "Number of identity service tasks (only when sizing = custom)."
  type        = number
  default     = 1
}
