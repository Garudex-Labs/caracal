<!--
SPDX-FileCopyrightText: 2026 Ryan Madhuwala <rawx18.dev@gmail.com>
SPDX-License-Identifier: Apache-2.0
-->

# Caracal - GCP OpenTofu Module

Deploy a production-ready Caracal instance on Google Cloud Platform.

## Architecture

| Component | GCP Service |
|-----------|-------------|
| API server / Init | Cloud Run (v2) + Cloud Run Job (both run the `caracal-server` image; init applies migrations) |
| Identity service (auth) | Cloud Run (v2), `caracal-auth` image on port 3001 |
| Web frontend | Cloud Run (v2), `caracal-web` static SPA |
| PostgreSQL | Cloud SQL |
| Redis | Memorystore |
| ClickHouse, optional Prometheus and Grafana | GCE instance (Docker Compose) |
| Load balancer / TLS | Global HTTPS LB + Managed SSL Certificate |
| DNS | Cloud DNS |
| Secrets | Secret Manager |
| Backups | GCS |
| Logging | Cloud Logging (built-in) |

## Prerequisites

1. A GCP project with billing enabled
2. APIs enabled: `run.googleapis.com`, `sqladmin.googleapis.com`, `redis.googleapis.com`, `compute.googleapis.com`, `secretmanager.googleapis.com`, `dns.googleapis.com`, `vpcaccess.googleapis.com`
3. `gcloud` CLI authenticated
4. OpenTofu >= 1.6

Enable required APIs:
```bash
gcloud services enable \
  run.googleapis.com \
  sqladmin.googleapis.com \
  redis.googleapis.com \
  compute.googleapis.com \
  secretmanager.googleapis.com \
  dns.googleapis.com \
  vpcaccess.googleapis.com \
  servicenetworking.googleapis.com
```

## Quick Start

```bash
cp terraform.tfvars.example terraform.tfvars
# Edit terraform.tfvars with your project_id and settings

tofu init
tofu plan
tofu apply
```

After apply:
1. Run migrations: `gcloud run jobs execute caracal-prod-init --region=us-central1`
2. Access the app at the URL from `tofu output app_url`

## Custom Domain

Set `domain_name` and `dns_managed_zone_name` to enable the Global HTTPS Load Balancer with a managed SSL certificate. The module creates a DNS A record pointing to the LB IP.

With a custom domain the LB serves everything from one origin:

- `/api/auth/*` -> identity service (port 3001)
- `/api/*`, `/health`, `/healthz` -> API server (port 8080)
- everything else -> web frontend

Without a custom domain there is no LB: each Cloud Run service is reached at its own URL (`tofu output cloud_run_urls`). The app is served at the web URL, and the API server is configured to validate tokens against the identity service's own Cloud Run URL.

## ClickHouse Modes

- **self_hosted** (default): Deploys a GCE instance running ClickHouse, Grafana, and Prometheus via Docker Compose. Access via IAP SSH tunnel.
- **cloud**: Supply `clickhouse_cloud_url` and `clickhouse_cloud_password` to use ClickHouse Cloud. No GCE instance is created.

## Accessing the Data Host

```bash
gcloud compute ssh caracal-prod-data --zone=us-central1-a --tunnel-through-iap
```

## Outputs

| Output | Description |
|--------|-------------|
| `app_url` | Public URL |
| `cloud_run_urls` | Individual service URLs |
| `data_host_ssh_command` | IAP SSH command for ClickHouse host |
| `init_job_run_command` | Command to re-run migrations |
| `backups_bucket` | GCS backup bucket name |
