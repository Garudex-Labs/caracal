...
variable "internal_dns_zone" {
  description = "Private Route 53 zone for VPC-internal DNS (e.g. clickhouse.caracal.internal)."
  type        = string
  default     = "caracal.internal"
}

# ── DNS / TLS ──────────────────────────────────────────────────────────────

variable "domain_name" {
  description = "Public hostname for the install (e.g. caracal.example.com). Leave empty to expose only the ALB DNS name over HTTP."
  type        = string
  default     = ""
}
...
#
# Presets:
#   small  (~$150/mo) - 1× api, 1× auth, 1× web, t3.medium data, db.t4g.micro
#   medium (~$255/mo) - 2× api, 2× auth, 2× web, t3.large data, db.t4g.small
#   large  (~$600/mo) - 3× api, 2× auth, 3× web, r6i.xlarge data, db.r6g.large
...
# ── ECS Fargate (api / auth / web / init) ──────────────────────────────────

variable "image_repo_server" {
  description = "Container image repository for the API server + init (they share an image)."
  type        = string
  default     = "ghcr.io/garudex-labs/caracal-server"
}

variable "image_repo_auth" {
  description = "Container image repository for the identity service."
  type        = string
...
variable "image_tag" {
  description = "Tag of the Caracal images to deploy. Bump and re-apply to roll out a new release."
  type        = string
  default     = "latest"
}
...
variable "auth_cpu" {
  description = "Fargate CPU units for the identity service task."
  type        = number
  default     = 256
}

variable "auth_memory" {
  description = "Fargate memory (MB) for the identity service task."
  type        = number
  default     = 512
}

variable "auth_desired_count" {
  description = "Baseline number of identity service tasks."
  type        = number
  default     = 2
}

variable "auth_autoscale_min" {
  description = "Minimum number of identity service tasks under autoscaling."
  type        = number
  default     = 2
}

variable "auth_autoscale_max" {
  description = "Maximum number of identity service tasks under autoscaling."
  type        = number
  default     = 6
}
...
variable "backup_bucket_force_destroy" {
  description = "Allow OpenTofu to destroy the backups bucket even if it contains objects. Set true only for non-prod."
  type        = bool
  default     = false
}
...
variable "enable_public_ops_paths" {
  description = "Let /docs, /redoc, /openapi.json requests through the ALB instead of answering 403. The API server does not serve these paths; they fall through to the web SPA. Default false blocks them."
  type        = bool
  default     = false
}
...
# The ECS SG must allow inbound 8080/8001/8000 from the ALB SG.
