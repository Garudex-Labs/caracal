...
Deploy Caracal into an existing VPC instead of having OpenTofu create one.
...
- **ECS SG** must allow inbound TCP 8080 (API server), 8001 (identity service), and 8000 (web) from the ALB security group, and outbound to all (for image pulls and AWS APIs)
