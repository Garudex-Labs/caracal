...
# ── ECS task ENIs (api/auth/web) ───────────────────────────────────────────
resource "aws_security_group" "ecs_tasks" {
  count       = local.create_ecs_sg ? 1 : 0
  name        = "${local.name}-ecs-tasks"
  description = "Fargate task ENIs for api/auth/web. Inbound only from ALB."
  vpc_id      = local.vpc_id

  ingress {
    description     = "API server HTTP from ALB"
    from_port       = 8080
    to_port         = 8080
    protocol        = "tcp"
  ...
  ingress {
    description     = "Identity service HTTP from ALB"
    from_port       = 8001
    to_port         = 8001
    protocol        = "tcp"
    security_groups = [local.alb_sg_id]
...
resource "aws_security_group" "redis" {
  ...
  description = "Redis reachable from ECS tasks."
  vpc_id      = local.vpc_id
