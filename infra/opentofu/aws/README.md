...
# Caracal on AWS - OpenTofu

Production-shaped self-hosted Caracal in your own AWS account. One `tofu apply` provisions:
...
- **ECS Fargate cluster** - `api` (Go API server, port 8080), `auth` (identity service, port 8001), and `web` as separate services with target-tracking CPU autoscaling. `init` runs as a one-shot `RunTask` on every image bump and applies Postgres + ClickHouse migrations
...
- **SSM Parameter Store**: generated DB / ClickHouse / `SECRET_KEY` / `AUTH_INTERNAL_SECRET` / `BETTER_AUTH_SECRET` / optional Grafana passwords, plus pre-built connection URLs injected into ECS tasks
...
```
                ...
                Internet → │   ALB (443)   │
                      └──┬─────┬─────┬──────────┘
          /api/auth/*    │     │     │/grafana/* optional
        ┌────────────────┘     │     │        default
        │        /api/*        ▼     ▼              ▼
        ▼              ┌──────────────┐  ┌────────────────┐  ┌──────────┐
  ┌──────────────┐     │ ECS Fargate  │  │  EC2 data host │  │   ECS    │
  │ ECS Fargate  │     │ api service  │  │  (single AZ)   │  │   web    │
  │ auth :8001   │     │ :8080 2..10× │  │  ClickHouse    │  │ 2..6×    │
  │  2..4×       │     └─────┬────────┘  │  optional      │  └──────────┘
  └──────┬───────┘           │           │  observability │
         │                   │           └────────┬───────┘
         │                   │                    ▼
...
The stateless app tier (api/auth/web) lives on Fargate across both AZs with autoscaling and rolling deploys; background jobs run inside the API server process. The stateful data tier lives on a single EC2 with EBS so ClickHouse keeps its disk across instance replacements. Prometheus and Grafana are enabled with `observability_stack`. Real ClickHouse HA is out of scope; set `clickhouse_mode = "cloud"` and point at ClickHouse Cloud when you need it.

## Prerequisites

| Tool | Version | Install |
|------|---------|---------|
| OpenTofu | ≥ 1.6 | `brew install opentofu` or [tenv](https://tofuutils.github.io/tenv/) |
...
Any of these is fine. OpenTofu reads the standard AWS SDK credential chain:
...
aws sso login --profile caracal-prod
export AWS_PROFILE=caracal-prod

# Or short-lived session creds
eval "$(aws configure export-credentials --profile caracal-prod --format env)"
```
...
This creates the S3 bucket + DynamoDB lock table that hold OpenTofu state. Without it, state lives only on your laptop. Losing the file orphans live AWS resources.

```bash
cd infra/opentofu/aws/bootstrap
tofu init
tofu apply
tofu output -raw backend_config   # copy this
```

Paste the output into the `backend "s3" {}` block in `infra/opentofu/aws/versions.tf` (it ships commented out). Details: [`bootstrap/README.md`](bootstrap/README.md).
...
```bash
tofu init        # picks up the S3 backend from step 2
...
If you skipped step 2, run `tofu init -migrate-state` after configuring the backend and OpenTofu will move local state into S3.

#### Watching the apply in the AWS Console

While OpenTofu runs, you can watch resources come up live. Replace `<REGION>` in the URLs below with the region from your `terraform.tfvars`. The names below assume `name_prefix = "caracal"` and `environment = "prod"` - if you changed those, substitute accordingly. The pattern is `<name_prefix>-<environment>-<resource>`).

| Resource | Console link (replace `<REGION>`) | What to look for |
|---|---|---|
| RDS Postgres | `https://<REGION>.console.aws.amazon.com/rds/home?region=<REGION>#databases:` | `caracal-prod-pg` → Status: `Available` (passes through `Creating` → `Backing-up` → `Modifying`) |
| ElastiCache Redis | `https://<REGION>.console.aws.amazon.com/elasticache/home?region=<REGION>#/redis` | `caracal-prod-redis` → Status: `available` |
| ECS Fargate | `https://<REGION>.console.aws.amazon.com/ecs/v2/clusters?region=<REGION>` | Cluster `caracal-prod-cluster` → Services tab: `api`/`auth`/`web` showing `Running 2/2` |
| ALB | `https://<REGION>.console.aws.amazon.com/ec2/home?region=<REGION>#LoadBalancers:` | `caracal-prod-alb` → State: `Active` |
| EC2 data host | `https://<REGION>.console.aws.amazon.com/ec2/home?region=<REGION>#Instances:` | `caracal-prod-data-host` → Status check: `2/2 checks passed` |
| VPC | `https://<REGION>.console.aws.amazon.com/vpc/home?region=<REGION>#vpcs:` | `caracal-prod-vpc` |
| S3 (state + backups) | `https://s3.console.aws.amazon.com/s3/buckets` | `caracal-tf-state-<account_id>` and `caracal-prod-backups-<account_id>` (S3 console is global) |
| CloudWatch logs | `https://<REGION>.console.aws.amazon.com/cloudwatch/home?region=<REGION>#logsV2:log-groups` | `/aws/ecs/caracal-prod/api`, `/aws/ecs/caracal-prod/auth`, `/aws/ecs/caracal-prod/web`, `/aws/ec2/caracal-prod/data-host`, `/aws/elasticache/caracal-prod/redis/slow`, `/aws/vpc/caracal-prod/flow-logs` |
...
watch -n 10 "aws rds describe-db-instances \
  --query 'DBInstances[?DBInstanceIdentifier==\`caracal-prod-pg\`].[DBInstanceStatus,Endpoint.Address]' \
  --output table"

# Redis
watch -n 10 "aws elasticache describe-replication-groups \
  --replication-group-id caracal-prod-redis \
  --query 'ReplicationGroups[0].Status' --output text"

# ECS services
watch -n 10 "aws ecs describe-services \
  --cluster caracal-prod-cluster \
  --services caracal-prod-api caracal-prod-auth caracal-prod-web \
...
```bash
tofu output app_url                        # https://caracal.example.com (or ALB DNS in HTTP mode)
tofu output ecs_cluster_name               # caracal-prod-cluster
tofu output data_host_ssm_session_command  # SSM session into the data host
```

The api takes ~2–3 minutes to be reachable after `apply` finishes (ECS task startup + ALB health checks). `curl -fsS $(tofu output -raw app_url)/api/v1/config/version` should return 200.

A working module-call example lives at [`examples/minimal`](examples/minimal/).

## What credentials and inputs are required?

The module generates all application secrets (Postgres password, ClickHouse password, `SECRET_KEY`, `AUTH_INTERNAL_SECRET`, `BETTER_AUTH_SECRET`, Grafana admin) and stores them in SSM Parameter Store as `SecureString`s. ECS task definitions reference them via the `secrets` block, so they're injected as environment variables at task start, never written to disk.
...
**Tail an ECS service:**
```bash
aws logs tail /aws/ecs/caracal-prod/api --follow
```
...
aws ecs update-service \
  ...
  --service caracal-prod-api \
  --force-new-deployment
```

**Re-run migrations:**
```bash
$(tofu output -raw init_run_task_command)
```

**Read a generated secret:**
```bash
aws ssm get-parameter --with-decryption \
  --name /caracal-prod/SECRET_KEY \
...
| Fargate auth 2× (0.25 vCPU / 0.5 GB) | always-on | $15 |
...
Drop to single-AZ RDS and a single Redis node for staging by setting `environment != "prod"`.
...
> **Migrating an existing Terraform-managed install?** OpenTofu reads the same
> state format. Keep your existing `backend "s3"` block and state key exactly
> as they are, run `tofu init` once, and review `tofu plan` - it should show
> only tag-value updates (`ManagedBy = "OpenTofu"`), no create/destroy.

### Tear down
```bash
tofu destroy
```
The bootstrap module's bucket + table have `prevent_destroy = true`. Destroying the main module won't touch them. To remove them too, edit `bootstrap/main.tf` to drop the lifecycle blocks, then `tofu destroy` in `bootstrap/`. Don't do this until you're sure no Caracal install in the account still needs the state.
...
- **OpenTofu state**: S3 versioning is on - recover prior state with `aws s3api list-object-versions` + `cp --version-id`.

## Production hardening checklist

Before using in front of customers:

- [x] Switch the OpenTofu backend to S3 + DynamoDB - done by the `bootstrap/` module
...
**`No valid credential sources found` on `tofu plan`** - your shell auth (e.g. AWS CLI v2 `aws login` profile) isn't visible to the AWS SDK that OpenTofu uses. Export the resolved creds:
...
**App returns 502 right after apply**: ECS tasks need ~2 min to pass ALB health checks. `aws logs tail /aws/ecs/caracal-prod/api --follow` shows what the api is doing.
