...
output "state_bucket" {
  description = "Name of the S3 bucket holding OpenTofu state. Plug into the main module's backend block as `bucket`."
...
output "backend_config" {
  ...
  value       = <<-EOT
    backend "s3" {
      ...
      key            = "caracal/${var.name_prefix}/terraform.tfstate"
      region         = "${var.region}"
