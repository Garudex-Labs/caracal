...
# The identity service rule must win over the general api rule, so it gets a
...
# Blocked by default (SEC-018); set enable_public_ops_paths=true to let them
# fall through to the web SPA.
