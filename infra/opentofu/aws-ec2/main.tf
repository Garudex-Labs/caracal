...
resource "aws_instance" "caracal" {
  ...
  user_data = <<-EOF
    ...
    # Signal completion
    touch /var/run/caracal-startup-complete
  EOF
...
resource "aws_eip" "caracal" {
  ...
  tags = {
    Name = "caracal-${var.name}-eip"
  }
}

resource "aws_security_group" "caracal" {
  name        = "caracal-${var.name}"
  description = "Caracal instance - HTTP/HTTPS inbound"
