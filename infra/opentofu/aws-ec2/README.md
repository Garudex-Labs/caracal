...
# Caracal on AWS EC2 - Single Instance

Deploy Caracal on a single EC2 instance with everything running in Docker. Simple, self-contained, no managed services required.
...
All Caracal components (API, web frontend, worker, PostgreSQL, Redis, ClickHouse, Grafana, nginx) run as Docker containers on the single instance.
...
That's it. The deploy script clones Caracal, configures it, builds Docker images, and starts everything. It takes 8-12 minutes on first run (Docker build from scratch).
...
docker compose --env-file .env logs -f caracal-server
```
...
```bash
aws ec2 stop-instances --instance-ids $(tofu output -raw instance_id) --region $(tofu output -raw region)
```

Start it back:

```bash
aws ec2 start-instances --instance-ids $(tofu output -raw instance_id) --region $(tofu output -raw region)
...
A fresh deployment has no accounts. The first `caracal auth login` against the
new server bootstraps the administrator account with credentials you choose:

```bash
caracal auth login --server <URL>
```
