...
echo ""
echo "=== Migration: released set is frozen; one open migration per release line ==="
# released.sha256 records every migration file shipped in the latest stable
# minor or major release (regenerated by freezeMigrations.sh when one ships).
# Released files are immutable: upgrades between stable releases replay them by
# recorded filename, so an edit would silently diverge existing installs.
# Everything else must be exactly one up/down pair with the next sequential
# prefix: the open migration for the upcoming stable release, amended in place
# through that release's rc train instead of accreting per-rc migrations.
...
if ! (cd "${MIGRATIONS_DIR}" && sha256sum --check --quiet released.sha256); then
    echo "FAIL: a released migration was edited or removed; released files are frozen, put schema changes in the open migration for the next release" >&2
    exit 1
fi
...
if [ "${#open_files[@]}" -eq 0 ]; then
    ...
    if [ "$(printf '%s\n' "${open_names}" | wc -l)" -ne 1 ]; then
        echo "FAIL: more than one open migration; amend the existing open migration during the rc train instead of adding another" >&2
        printf '  open: %s\n' "${open_files[@]}" >&2
        exit 1
    fi
