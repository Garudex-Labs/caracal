...
INSTALL_DIR="${CARACAL_INSTALL_DIR:-/opt/caracal}"
...
info "Caracal Server Setup"
prompt_with_default FRONTEND_URL "Frontend URL" "http://localhost:8000"
prompt_with_default CARACAL_BIND_ADDRESS "HTTP bind address" "${previous_bind:-127.0.0.1}"
if [ "$CARACAL_BIND_ADDRESS" != "127.0.0.1" ] && [ "$CARACAL_BIND_ADDRESS" != "::1" ]; then
...
BETTER_AUTH_SECRET=$(existing_or_generated_secret better_auth_secret BETTER_AUTH_SECRET)
AUTH_INTERNAL_SECRET=$(existing_or_generated_secret auth_internal_secret AUTH_INTERNAL_SECRET)
...
write_secret auth_internal_secret "$AUTH_INTERNAL_SECRET"
...
case "$DATABASE_URL" in "" | *'$'*) DATABASE_URL="postgresql://postgres:$POSTGRES_PASSWORD@caracal-db:5432/caracal" ;; esac
case "$CLICKHOUSE_URL" in "" | *'$'*) CLICKHOUSE_URL="clickhouse://default:$CLICKHOUSE_PASSWORD@caracal-clickhouse:8123/caracal" ;; esac
REDIS_URL="${REDIS_URL:-redis://caracal-redis:6379}"
write_secret database_url "$DATABASE_URL"
write_secret clickhouse_url "$CLICKHOUSE_URL"
write_secret redis_url "$REDIS_URL"
# The identity service's pg driver needs the plain postgresql:// scheme even
# when an upgraded install still carries the legacy +asyncpg dialect.
write_secret auth_database_url "${DATABASE_URL/postgresql+asyncpg:\/\//postgresql://}"
...
CARACAL_BIND_ADDRESS=$CARACAL_BIND_ADDRESS
CARACAL_SECRET_GID=$SECRET_GID
FRONTEND_URL=$FRONTEND_URL
CORS_ALLOWED_ORIGINS=$FRONTEND_URL
BETTER_AUTH_URL=$FRONTEND_URL
AUTH_TRUSTED_ORIGINS=$FRONTEND_URL
EOF
chmod 600 "$ENV_FILE"
...
info "Starting Caracal services"
cd "$INSTALL_DIR"
docker compose "${profile_args[@]}" "${compose_args[@]}" --env-file .env up -d --wait --wait-timeout 300

docker compose "${profile_args[@]}" "${compose_args[@]}" restart caracal-lb

info "Caracal is running"
info "Dashboard: $FRONTEND_URL"
info "HTTP bind address: $CARACAL_BIND_ADDRESS"
info "Config: $ENV_FILE"
printf '\nNo accounts exist yet. The first "caracal auth login" against this server\nbootstraps the administrator account with credentials you choose.\n'
if [ "$OBSERVABILITY_STACK" = "grafana" ]; then
