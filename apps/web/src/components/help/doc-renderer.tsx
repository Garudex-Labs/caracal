		...
		<pre className="mb-4 relative group">
			{codeText && (
				...
				>
					{copied ? (
						<>
							<svg className="h-3 w-3 text-success" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path d="M5 13l4 4L19 7" /></svg>
...
const GITHUB_DOCS_BASE = "https://github.com/Garudex-Labs/caracal/blob/main/docs/";
...
function resolveDocHref(href: string | undefined, basePath?: string): string | undefined {
	...
	return `${GITHUB_DOCS_BASE}${docPath}${hash ? `#${hash}` : ""}`;
}
...
function createComponents(basePath?: string): Components {
	...
	strong: ({ children }) => {
		...
		if (text.startsWith("Checkpoint:")) return <span className="text-[15px]"><span className="inline-flex items-center gap-1 font-semibold text-success">✓ Checkpoint:</span> {text.slice(11).trim()}</span>;
	...
	blockquote: ({ children }) => {
		...
		const styles = {
			info: "border-l-[3px] border-primary/60 bg-primary/5",
			warning: "border-l-[3px] border-warning/60 bg-warning/5",
			tip: "border-l-[3px] border-success/60 bg-success/5",
			danger: "border-l-[3px] border-destructive/60 bg-destructive/5",
		};
		const icons = {
			...
			tip: <Lightbulb className="h-3.5 w-3.5 text-success shrink-0 mt-0.5" />,
			danger: <ShieldAlert className="h-3.5 w-3.5 text-destructive shrink-0 mt-0.5" />,
		};
...
export function DocRenderer({ content, basePath }: { content: string; basePath?: string }) {
