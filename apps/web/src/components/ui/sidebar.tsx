...
/* ─── Sidebar (NO position:fixed - normal flex child) ─── */
...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  if (collapsible === "none") {
    return (
      <div className={cn("flex h-full w-[--sidebar-width] shrink-0 flex-col bg-sidebar text-sidebar-foreground", className)} ref={ref} {...props}>
        {children}
      </div>
    )
  }
  ...
  // Desktop: normal flow element, not fixed
  const isCollapsed = state === "collapsed"
  const width = collapsible === "icon" && isCollapsed ? "var(--sidebar-width-icon)" : "var(--sidebar-width)"
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          "absolute inset-y-0 right-0 z-20 hidden w-1 cursor-col-resize transition-colors hover:bg-sidebar-border sm:flex",
          className,
        )}
        {...props}
      />
...
const SidebarInset = React.forwardRef<HTMLDivElement, React.ComponentProps<"main">>(
  ({ className, onScroll, ...props }, ref) => {
    ...
    // Reveal the auto-hidden scrollbar while scrolling; recede when idle.
    const handleScroll = React.useCallback<React.UIEventHandler<HTMLElement>>(
      (e) => {
        ...
        idleTimer.current = window.setTimeout(() => delete el.dataset.scrolling, 800)
      },
      [onScroll],
    )
    return (
      <main
        ...
        className={cn("scrollbar-autohide relative flex min-h-0 min-w-0 w-full flex-1 flex-col overflow-y-auto bg-background [scrollbar-gutter:stable]", className)}
        {...props}
      />
    )
  },
...
>(({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, ...props }, ref) => {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="right" align="center" hidden={state !== "collapsed" || isMobile} {...tipProps} />
    </Tooltip>
  )
})
  ...
  ({ className, ...props }, ref) => (
    <div
      ...
      className={cn(
        ...
        // Without a vertical offset, `absolute` falls back to the static
        // position, which is BELOW the menu button; the badge then lands on the
        // next row and reads as belonging to the wrong item. Same offsets as
        // SidebarMenuAction directly above.
        "peer-data-[size=sm]/menu-button:top-1 peer-data-[size=default]/menu-button:top-1.5 peer-data-[size=lg]/menu-button:top-2.5",
        "group-data-[collapsible=icon]:hidden",
        className,
      )}
