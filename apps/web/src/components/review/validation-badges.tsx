...
export function ValidationBadge({ item }: { item: ReviewItem }) {
  ...
  if (item.mcp_validated) {
    if (hasWarnings) {
      return (
        <span className="inline-flex items-center gap-1 text-[10px] text-warning bg-warning/10 border border-warning/25 rounded px-1.5 py-0.5">
...
export function ValidationDetails({ results }: { results?: McpValidationResult[] }) {
  ...
  return (
    <div className="mt-2 p-2 rounded bg-warning/5 border border-warning/15 space-y-1">
