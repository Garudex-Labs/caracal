...
import { AuthApiError, fetchEnabledProviders, signIn, signUpFirstOperator } from "@/platform/auth";
...
function SignInPage() {
  ...
  useEffect(() => {
    ...
    fetchEnabledProviders().then((enabled) => {
      ...
      setInviteAvailable(enabled.bootstrapInvite === true);
    });
    return () => {
      active = false;
    };
  ...
  async function onFirstOperatorSubmit(event: React.FormEvent) {
    event.preventDefault();
    setBusy(true);
    setError(null);
    try {
      await signUpFirstOperator({ name, email, password, inviteCode });
    } catch (submitError) {
      setBusy(false);
      setError(
        submitError instanceof AuthApiError && submitError.code === "registration_not_permitted"
          ? t.inviteInvalid
          : "Could not create the operator account.",
      );
      return;
    }
    const { error: signInError } = await signIn.email({ email, password, rememberMe: remember });
  ...
  return (
    <AuthSplitLayout
      title={firstOperator ? t.firstOperatorTitle : t.signInTitle}
      subtitle={firstOperator ? t.firstOperatorSubtitle : t.signInSubtitle}
    ...
    >
      {firstOperator ? (
        <form onSubmit={onFirstOperatorSubmit} className="flex flex-col gap-4">
          <p className="text-sm text-muted-foreground">{t.firstOperatorHint}</p>
          <Field
            label={t.nameLabel}
            autoComplete="name"
          ...
          <Field
            ...
            placeholder="Paste the code from caracal invite"
            required
            value={inviteCode}
          ...
          <Button type="submit" loading={busy} className="w-full">
            {busy ? "Creating account…" : t.firstOperatorCta}
          </Button>
          <button
            type="button"
            onClick={() => {
              setFirstOperator(false);
              setError(null);
            }}
