...
export function usePurgeDeletedAgent() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => registry.purgeDeletedAgent(id, { confirm: "permanently delete" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["registry", "agents"] });
      toast.success("Agent permanently deleted");
    },
    onError: (err: Error) => {
      toast.error(err.message || "Failed to permanently delete agent");
    },
  });
}
