 ...
 * what commands take (``caracal agent pull richard/reviewer``), but it reads
 * poorly in the UI, so listings render the bare name with the owning namespace
 * underneath as ``@richard``. Keep using ``qualified_name`` verbatim for anything
 * copy-pasted into a shell.
 */

/**
 * Canonical namespace charset: 3-32 chars, starting and ending alphanumeric,
 * hyphens and dots inside. Mirrors `caracal-server/services/shared/namespace_rules.py` - keep
 * the two in sync.
 */
 ...
 * ``qualified_name`` so payloads that only carry the qualified form still
 * render a handle.
 */
...
/** Single-line form for breadcrumbs and document titles, e.g. ``reviewer @richard``. */
