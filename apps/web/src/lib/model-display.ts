 ...
 *
 * Reads pre-computed display fields attached to catalog rows. Falls back
 * to raw model_id when display data isn't available.
 */
...
/** The pre-computed `display` field attached to catalog rows. */
interface ServerModelDisplay {
  primary: string;
  secondary: string | null;
  is_rolling: boolean;
}
...
export function formatModel(input: {
  ...
  display?: ServerModelDisplay | null;
