...
// Parse Discord-style search: actor:richard outcome:denied sensitivity:high
// Supports quoted values: actor:"Richard Hendricks" action:"agent.pull"
...
export default function AuditLogPage() {
  const { search: searchParam } = useSearch({ from: "/operator/audit-log" });
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `caracal_audit-log_${new Date().toISOString().replace(/[-:]/g, "").slice(0, 15)}Z.csv`;
      a.click();
