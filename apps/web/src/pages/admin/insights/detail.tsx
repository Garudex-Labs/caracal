		...
		<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
			...
			{outcomes && Object.keys(outcomes).length > 0 && (
				<div className="rounded-lg border border-border bg-card p-4">
					...
					{renderRecordChart(outcomes, "bg-warning")}
				</div>
			)}
...
function SuggestionsSection({
	...
	const toggleConfig = (i: number) => {
		setSelectedConfigs((s) => { const n = new Set(s); if (n.has(i)) n.delete(i); else n.add(i); return n; });
	};
	const toggleFeature = (i: number) => {
		setSelectedFeatures((s) => { const n = new Set(s); if (n.has(i)) n.delete(i); else n.add(i); return n; });
	};
	const togglePattern = (i: number) => {
		setSelectedPatterns((s) => { const n = new Set(s); if (n.has(i)) n.delete(i); else n.add(i); return n; });
	};
...
function ReportContent({ report }: { report: InsightReport }) {
	...
	const rich = metrics?.rich;
	// facets_summary is stored inside aggregated_data by the batch job
	const facetsSummary = (report.aggregated_data?.facets_summary || report.facets_summary) as Record<string, unknown> | undefined;
