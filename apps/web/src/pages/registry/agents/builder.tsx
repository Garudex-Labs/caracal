...
const DRAFT_STORAGE_KEY = "caracal_agent_draft";
...
export interface AgentBuilderProps {
  /** Agent id when editing an existing agent (contextual /edit route). */
  editId?: string;
  /** Draft id to resume (create route ?draft=). */
  draftId?: string;
  /** Canonical ref of the agent being edited, for contextual back-navigation. */
  backTo?: { namespace: string; slug: string };
}

export default function AgentBuilderPage(props: AgentBuilderProps) {
  return (
    <Suspense>
...
function AgentBuilderInner({ editId, draftId: draftParam, backTo }: AgentBuilderProps) {
  ...
  // project-shared agent and invalid for a public one, so a change of
  // visibility has to revalidate.
  }, [selectedComponents, visibility]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    autoSaveTimerRef.current = setTimeout(() => {
      ...
      try {
        const draft = {
          ...
          // project-shared draft silently reverts to a public agent, which is a
          // change of audience the user never asked for.
          visibility,
  ...
  }, [name, description, version, modelName, modelsByHarness, selectedComponents, systemPrompt, successCriteria, draftId, visibility, isEditMode]);
  ...
  async function handlePublish() {
    ...
    try {
      ...
      if (draftId) {
        ...
        // Authoring flows into review: land on the agent's own review surface.
        router.navigate({ to: "/agents/$agentId", params: { agentId: draftId }, search: { view: "review" } });
      } else {
        const created = await registry.create("agents", body);
        toast.success("Agent submitted for review.");
        router.navigate({ to: "/agents/$agentId", params: { agentId: created.id }, search: { view: "review" } });
      }
    } catch (e) {
  ...
  async function handleUpdateWithVersion(selectedVersion: string) {
    ...
    try {
      ...
      router.navigate({ to: "/agents/$agentId", params: { agentId: editId }, search: { view: "review" } });
    } catch (e) {
  ...
  return (
    <>
      <PageHeader
        title={isEditMode ? `Edit ${existingAgent?.name ?? "Agent"}` : "New Agent"}
        breadcrumbs={[
          { label: "Resources", href: "/resources" },
          ...(backTo
            ? [{ label: existingAgent?.name ?? backTo.slug, href: `/agents/${backTo.namespace}/${backTo.slug}` }]
            : []),
          { label: isEditMode ? "Edit" : "New" },
        ]}
        helpKey="agents.builder"
      />
