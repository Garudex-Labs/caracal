...
import { useAiEngineModelProviders, useAiEngineModels } from "@/hooks/use-api";
...
export function InsightsSection({ entries, onSave, onRevoke, refetch }: InsightsSectionProps) {
	...
	const providerCatalog = useAiEngineModelProviders();
	const modelCatalog = useAiEngineModels(provider);
	...
	const handleTestConnection = useCallback(async () => {
		...
		try {
			...
			for (const { model, label } of modelsToTest) {
				const result = await admin.testAiEngineConnection({ model });
				if (!result.success) {
					setTestResult({
						...result,
	...
	const statusConfig = {
		...
		partial: { color: "bg-warning", label: "Partially configured" },
	...
	return (
		<section className="mb-6 mt-2">
			...
			<div className="flex items-center justify-between mb-1">
				<h3 className={cn("text-sm font-semibold uppercase tracking-wider text-foreground/80 flex items-center gap-1.5", helpCtx.helpActive && "cursor-help ring-2 ring-primary/60 ring-offset-2 ring-offset-background rounded-sm")} onClick={(event) => { if ((event.ctrlKey || event.metaKey) && helpCtx.openHelp({ sectionTitle: "AI Engine" })) event.preventDefault(); }}>
					<Activity className="h-3.5 w-3.5" />
					AI Engine
					<button type="button" className="text-muted-foreground hover:text-primary" onClick={(event) => { event.preventDefault(); event.stopPropagation(); helpCtx.openHelp({ sectionTitle: "AI Engine" }); }} aria-label="Open AI Engine documentation">
			...
			<p className="text-xs text-foreground/60 mb-3">
				Configure the shared LLM backend used by deployment-managed report generation.
			</p>
