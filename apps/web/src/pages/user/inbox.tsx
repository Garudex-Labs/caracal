 ...
 * destination renders - `?type=` picks the component type, `?view=` picks the
 * resource workspace section (e.g. the in-context change review). Link takes
 * those as a `search` object, not baked into `to`, so a raw string would
 * navigate to the path with the params dropped.
 */
const INTERNAL_ROOTS = new Set(["agents", "components", "resources", "settings"]);
