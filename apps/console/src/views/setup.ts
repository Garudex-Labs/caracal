  ...
  private openProviderPage(app: App): void {
    ...
    app.push(new FormView({
      ...
      fields: [
        ...
        { key: 'provider_token_params', label: 'OAuth token parameters', kind: 'list', default: this.values.provider_token_params ?? '', dependsOn: { provider_mode: 'create', provider_kind: ['oauth2_authorization_code', 'oauth2_client_credentials'] }, advanced: true, info: guidedInfo('OAuth token parameters', 'Optional provider-specific parameters sent to the token endpoint.', 'tenant=hooli', 'Comma-separated key=value pairs; token credentials and grants are managed by Caracal.', 'Use this only when the provider documents extra token endpoint parameters.') },
