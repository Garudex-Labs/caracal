...
//
// Writes apps/runtime/src/runtime/version.gen.ts with the dev, rc, or stable runtime identity and prints the version or short SHA on stdout.
...
if (releaseVersion && /(^|[+-])dev\./.test(releaseVersion)) {
  process.stderr.write(`stampVersion: refusing dev-suffixed CARACAL_RELEASE_VERSION '${releaseVersion}'\n`)
  process.exit(1)
}
...
const body = `// Copyright (C) 2026 Garudex Labs.  All Rights Reserved.
// Caracal, a product of Garudex Labs
//
// Stamped runtime identity. Generated by scripts/stampVersion.mjs.
...
const versionFile = resolve(runtimeRoot, 'src/runtime/version.gen.ts')
