...
//
// `caracal invite <email>`: mints the one-time invite code that authorizes the first operator registration.
...
export function inviteCommand(argv: string[] = []): void {
  ...
  process.stdout.write('\n')
  process.stdout.write(`${style.label('Shown once and stored only as a hash. Open the web console sign-in page,')}\n`)
  process.stdout.write(`${style.label(`create the first operator account as ${invite.email}, and enter this code.`)}\n`)
...
function inviteHelp(): never {
  return showHelp([
    ...
    '',
    'Mints a one-time invite code that authorizes the first operator account on',
    'this stack. The code is printed once; only its hash is stored, in',
    "the stack's secrets directory. Minting again replaces any live invite.",
    '',
    'Flags:',
