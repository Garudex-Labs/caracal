  ...
  return [
    {
      ...
      content: systemPrompt(
        ...
        [
          ...
          'RESOURCE VS PROVIDER. A resource is the protected target and its scopes; a provider is only the',
          'external credential source the Gateway brokers for a resource. When a target authenticates by',
          'verifying Caracal’s own signed mandate — auth mode caracal_mandate, an internal service or a',
          'Caracal-protected API — there is no external credential to broker, so define the resource alone',
          'and do NOT connect a provider. Connect a provider only when the upstream requires a brokered',
          'external credential (oauth2_authorization_code, oauth2_client_credentials, api_key,',
          'bearer_token). Never add a provider step to a caracal_mandate or internal-service target.',
